 ...
 */
namespace ConfigTransformer202205128\SebastianBergmann\Diff\Output;
 ...
 */
final class UnifiedDiffOutputBuilder extends \ConfigTransformer202205128\SebastianBergmann\Diff\Output\AbstractChunkOutputBuilder
{
    /**
     * @var bool
     */
    ...
    private function writeDiffHunks($output, array $diff) : void
    {
        ...
        if (0 === $diff[$upperLimit - 1][1]) {
            ...
            if ("\n" !== $lc) {
                \array_splice($diff, $upperLimit, 0, [["\n\\ No newline at end of file\n", \ConfigTransformer202205128\SebastianBergmann\Diff\Differ::NO_LINE_END_EOF_WARNING]]);
            }
        } else {
        ...
        foreach ($diff as $i => $entry) {
            ...
            $sameCount = 0;
            if ($entry[1] === \ConfigTransformer202205128\SebastianBergmann\Diff\Differ::NO_LINE_END_EOF_WARNING) {
                continue;
            }
            if (\false === $hunkCapture) {
                $hunkCapture = $i;
            }
            if (\ConfigTransformer202205128\SebastianBergmann\Diff\Differ::ADDED === $entry[1]) {
                ++$toRange;
            }
            if (\ConfigTransformer202205128\SebastianBergmann\Diff\Differ::REMOVED === $entry[1]) {
                ++$fromRange;
            }
        }
        if (\false === $hunkCapture) {
    ...
    private function writeHunk(array $diff, int $diffStartIndex, int $diffEndIndex, int $fromStart, int $fromRange, int $toStart, int $toRange, $output) : void
    {
        ...
        for ($i = $diffStartIndex; $i < $diffEndIndex; ++$i) {
            if ($diff[$i][1] === \ConfigTransformer202205128\SebastianBergmann\Diff\Differ::ADDED) {
                \fwrite($output, '+' . $diff[$i][0]);
            } elseif ($diff[$i][1] === \ConfigTransformer202205128\SebastianBergmann\Diff\Differ::REMOVED) {
                \fwrite($output, '-' . $diff[$i][0]);
            } elseif ($diff[$i][1] === \ConfigTransformer202205128\SebastianBergmann\Diff\Differ::OLD) {
                \fwrite($output, ' ' . $diff[$i][0]);
            } elseif ($diff[$i][1] === \ConfigTransformer202205128\SebastianBergmann\Diff\Differ::NO_LINE_END_EOF_WARNING) {
                \fwrite($output, "\n");
                // $diff[$i][0]
            } else {
