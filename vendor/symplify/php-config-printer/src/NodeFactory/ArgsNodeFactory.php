...
use ConfigTransformer2021061210\Symplify\PhpConfigPrinter\ExprResolver\StringExprResolver;
use ConfigTransformer2021061210\Symplify\PhpConfigPrinter\ExprResolver\TaggedReturnsCloneResolver;
use ConfigTransformer2021061210\Symplify\PhpConfigPrinter\ExprResolver\TaggedServiceResolver;
final class ArgsNodeFactory
{
    ...
    public function __construct(\ConfigTransformer2021061210\Symplify\PhpConfigPrinter\ExprResolver\StringExprResolver $stringExprResolver, \ConfigTransformer2021061210\Symplify\PhpConfigPrinter\ExprResolver\TaggedReturnsCloneResolver $taggedReturnsCloneResolver, \ConfigTransformer2021061210\Symplify\PhpConfigPrinter\ExprResolver\TaggedServiceResolver $taggedServiceResolver)
    {
    ...
    public function createFromValuesAndWrapInArray($values) : array
    {
        if (\is_array($values)) {
            ...
            $items = [new \ConfigTransformer2021061210\PhpParser\Node\Expr\ArrayItem($expr)];
            $array = new \ConfigTransformer2021061210\PhpParser\Node\Expr\Array_($items);
        }
        return [new \ConfigTransformer2021061210\PhpParser\Node\Arg($array)];
    }
    /**
     * @return Arg[]
     */
    public function createFromValues($values, bool $skipServiceReference = \false, bool $skipClassesToConstantReference = \false) : array
    {
        ...
        if ($values instanceof \ConfigTransformer2021061210\PhpParser\Node) {
            if ($values instanceof \ConfigTransformer2021061210\PhpParser\Node\Arg) {
                return [$values];
            }
            if ($values instanceof \ConfigTransformer2021061210\PhpParser\Node\Expr) {
                return [new \ConfigTransformer2021061210\PhpParser\Node\Arg($values)];
            }
        }
        if (\is_string($values)) {
            $expr = $this->resolveExpr($values);
            return [new \ConfigTransformer2021061210\PhpParser\Node\Arg($expr)];
        }
        throw new \ConfigTransformer2021061210\Symplify\PhpConfigPrinter\Exception\NotImplementedYetException();
    }
    public function resolveExpr($value, bool $skipServiceReference = \false, bool $skipClassesToConstantReference = \false) : \ConfigTransformer2021061210\PhpParser\Node\Expr
    {
        if (\is_string($value)) {
            return $this->stringExprResolver->resolve($value, $skipServiceReference, $skipClassesToConstantReference);
        }
        if ($value instanceof \ConfigTransformer2021061210\PhpParser\Node\Expr) {
            return $value;
        }
        ...
        if (\is_array($value)) {
            ...
            return new \ConfigTransformer2021061210\PhpParser\Node\Expr\Array_($arrayItems);
        }
        return \ConfigTransformer2021061210\PhpParser\BuilderHelpers::normalizeValue($value);
    }
    private function resolveExprFromArray(array $values) : \ConfigTransformer2021061210\PhpParser\Node\Expr\Array_
    {
        $arrayItems = [];
        foreach ($values as $key => $value) {
            ...
            if (!\is_int($key)) {
                ...
                $arrayItem = new \ConfigTransformer2021061210\PhpParser\Node\Expr\ArrayItem($expr, $keyExpr);
            } else {
                $arrayItem = new \ConfigTransformer2021061210\PhpParser\Node\Expr\ArrayItem($expr);
            }
            $arrayItems[] = $arrayItem;
        }
        return new \ConfigTransformer2021061210\PhpParser\Node\Expr\Array_($arrayItems);
    }
    private function createServiceReferenceFromTaggedValue(\ConfigTransformer2021061210\Symfony\Component\Yaml\Tag\TaggedValue $taggedValue) : \ConfigTransformer2021061210\PhpParser\Node\Expr
    {
        ...
        return new \ConfigTransformer2021061210\PhpParser\Node\Expr\FuncCall(new \ConfigTransformer2021061210\PhpParser\Node\Name($taggedValue->getTag()), $args);
    }
    /**
     * @param mixed[] $value
     * @return ArrayItem[]
    ...
    private function resolveArrayItems(array $value, bool $skipClassesToConstantReference) : array
    {
        ...
        foreach ($value as $nestedKey => $nestedValue) {
            ...
            if (!\is_int($nestedKey) || $nestedKey !== $naturalKey) {
                ...
                $arrayItem = new \ConfigTransformer2021061210\PhpParser\Node\Expr\ArrayItem($valueExpr, $keyExpr);
            } else {
                $arrayItem = new \ConfigTransformer2021061210\PhpParser\Node\Expr\ArrayItem($valueExpr);
            }
            $arrayItems[] = $arrayItem;
            ++$naturalKey;
        }
