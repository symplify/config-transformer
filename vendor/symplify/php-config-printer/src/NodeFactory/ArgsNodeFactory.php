...
final class ArgsNodeFactory
{
    ...
    public function __construct(\ConfigTransformer202206075\Symplify\PhpConfigPrinter\ExprResolver\StringExprResolver $stringExprResolver, \ConfigTransformer202206075\Symplify\PhpConfigPrinter\ExprResolver\TaggedReturnsCloneResolver $taggedReturnsCloneResolver, \ConfigTransformer202206075\Symplify\PhpConfigPrinter\ExprResolver\TaggedServiceResolver $taggedServiceResolver, \ConfigTransformer202206075\Symplify\PhpConfigPrinter\NodeFactory\NewValueObjectFactory $newValueObjectFactory)
    {
    ...
    public function createFromValuesAndWrapInArray($values) : array
    {
        if (\is_array($values)) {
            ...
            $items = [new \ConfigTransformer202206075\PhpParser\Node\Expr\ArrayItem($expr)];
            $array = new \ConfigTransformer202206075\PhpParser\Node\Expr\Array_($items);
        }
        return [new \ConfigTransformer202206075\PhpParser\Node\Arg($array)];
    }
    /**
     * @return mixed[]|Arg[]
     * @param mixed $values
    ...
    public function createFromValues($values, bool $skipServiceReference = \false, bool $skipClassesToConstantReference = \false, bool $isForConfig = \false) : array
    {
        ...
        if ($values instanceof \ConfigTransformer202206075\PhpParser\Node) {
            if ($values instanceof \ConfigTransformer202206075\PhpParser\Node\Arg) {
                return [$values];
            }
            if ($values instanceof \ConfigTransformer202206075\PhpParser\Node\Expr) {
                return [new \ConfigTransformer202206075\PhpParser\Node\Arg($values)];
            }
        }
        if (\is_string($values)) {
            $expr = $this->resolveExpr($values);
            return [new \ConfigTransformer202206075\PhpParser\Node\Arg($expr)];
        }
        throw new \ConfigTransformer202206075\Symplify\PhpConfigPrinter\Exception\NotImplementedYetException();
    }
    /**
     * @param mixed $value
     */
    public function resolveExpr($value, bool $skipServiceReference = \false, bool $skipClassesToConstantReference = \false) : \ConfigTransformer202206075\PhpParser\Node\Expr
    {
        if (\is_string($value)) {
            return $this->stringExprResolver->resolve($value, $skipServiceReference, $skipClassesToConstantReference);
        }
        if ($value instanceof \ConfigTransformer202206075\PhpParser\Node\Expr) {
            return $value;
        }
        ...
        if (\is_array($value)) {
            ...
            return new \ConfigTransformer202206075\PhpParser\Node\Expr\Array_($arrayItems);
        }
        if (\is_object($value)) {
     ...
     * @param mixed[] $values
     */
    public function resolveExprFromArray(array $values) : \ConfigTransformer202206075\PhpParser\Node\Expr\Array_
    {
        $arrayItems = [];
        foreach ($values as $key => $value) {
            ...
            if (!\is_int($key)) {
                ...
                $arrayItem = new \ConfigTransformer202206075\PhpParser\Node\Expr\ArrayItem($expr, $keyExpr);
            } else {
                $arrayItem = new \ConfigTransformer202206075\PhpParser\Node\Expr\ArrayItem($expr);
            }
     ...
     * @param mixed $key
     */
    private function resolveArgs(array $args, $key, \ConfigTransformer202206075\PhpParser\Node\Expr $expr, bool $isForConfig) : array
    {
        if (\is_string($key) && $isForConfig) {
            $key = $this->resolveExpr($key);
            $args[] = new \ConfigTransformer202206075\PhpParser\Node\Arg(new \ConfigTransformer202206075\PhpParser\Node\Expr\ArrayItem($expr, $key));
            return $args;
        }
        if (!\is_int($key) && $this->isPhpNamedArguments) {
            $args[] = new \ConfigTransformer202206075\PhpParser\Node\Arg($expr, \false, \false, [], new \ConfigTransformer202206075\PhpParser\Node\Identifier($key));
            return $args;
        }
        $args[] = new \ConfigTransformer202206075\PhpParser\Node\Arg($expr);
        return $args;
    }
    private function createServiceReferenceFromTaggedValue(\ConfigTransformer202206075\Symfony\Component\Yaml\Tag\TaggedValue $taggedValue) : \ConfigTransformer202206075\PhpParser\Node\Expr
    {
        ...
        switch ($taggedValue->getTag()) {
            case 'tagged_iterator':
                $name = new \ConfigTransformer202206075\PhpParser\Node\Name\FullyQualified(\ConfigTransformer202206075\Symplify\PhpConfigPrinter\ValueObject\FunctionName::TAGGED_ITERATOR);
                break;
            case 'tagged_locator':
                $name = new \ConfigTransformer202206075\PhpParser\Node\Name\FullyQualified(\ConfigTransformer202206075\Symplify\PhpConfigPrinter\ValueObject\FunctionName::TAGGED_LOCATOR);
                break;
            default:
                $name = new \ConfigTransformer202206075\PhpParser\Node\Name($taggedValue->getTag());
                break;
        }
        $args = $this->createFromValues($taggedValue->getValue());
        return new \ConfigTransformer202206075\PhpParser\Node\Expr\FuncCall($name, $args);
    }
    /**
     * @param mixed[] $value
     * @return ArrayItem[]
    ...
    private function resolveArrayItems(array $value, bool $skipClassesToConstantReference) : array
    {
        ...
        foreach ($value as $nestedKey => $nestedValue) {
            ...
            if (!\is_int($nestedKey) || $nestedKey !== $naturalKey) {
                ...
                $arrayItem = new \ConfigTransformer202206075\PhpParser\Node\Expr\ArrayItem($valueExpr, $keyExpr);
            } else {
                $arrayItem = new \ConfigTransformer202206075\PhpParser\Node\Expr\ArrayItem($valueExpr);
            }
            $arrayItems[] = $arrayItem;
            ++$naturalKey;
        }
