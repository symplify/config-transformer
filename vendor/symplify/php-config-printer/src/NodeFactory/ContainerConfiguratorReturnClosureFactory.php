...
namespace ConfigTransformer202202064\Symplify\PhpConfigPrinter\NodeFactory;

use ConfigTransformer202202064\PhpParser\Node\Expr\Assign;
...
use ConfigTransformer202202064\PhpParser\Node\Stmt\Return_;
use ConfigTransformer202202064\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface;
use ConfigTransformer202202064\Symplify\PhpConfigPrinter\PhpParser\NodeFactory\ConfiguratorClosureNodeFactory;
use ConfigTransformer202202064\Symplify\PhpConfigPrinter\ValueObject\VariableMethodName;
...
final class ContainerConfiguratorReturnClosureFactory
{
     ...
     */
    public function createFromYamlArray(array $arrayData) : \ConfigTransformer202202064\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \ConfigTransformer202202064\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     * @return Stmt[]
     */
    ...
    private function createInitializeAssign(string $variableMethodName) : \ConfigTransformer202202064\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \ConfigTransformer202202064\PhpParser\Node\Expr\Variable($variableMethodName);
        ...
        $assign = new \ConfigTransformer202202064\PhpParser\Node\Expr\Assign($servicesVariable, new \ConfigTransformer202202064\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $variableMethodName));
        return new \ConfigTransformer202202064\PhpParser\Node\Stmt\Expression($assign);
    }
    /**
     * @return mixed[]
     */
    ...
    {
        if ($key === \ConfigTransformer202202064\Symplify\PhpConfigPrinter\ValueObject\YamlKey::SERVICES) {
            $nodes[] = $this->createInitializeAssign(\ConfigTransformer202202064\Symplify\PhpConfigPrinter\ValueObject\VariableMethodName::SERVICES);
            return $nodes;
        }
        if ($key === \ConfigTransformer202202064\Symplify\PhpConfigPrinter\ValueObject\YamlKey::PARAMETERS) {
            $nodes[] = $this->createInitializeAssign(\ConfigTransformer202202064\Symplify\PhpConfigPrinter\ValueObject\VariableMethodName::PARAMETERS);
            return $nodes;
        }
        return $nodes;
    }
     ...
     */
    private function resolveExpression(string $key, $nestedKey, $nestedValues) : ?\ConfigTransformer202202064\PhpParser\Node\Stmt\Expression
    {
