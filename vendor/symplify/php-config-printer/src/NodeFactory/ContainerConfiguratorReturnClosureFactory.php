...
namespace ConfigTransformer2021120610\Symplify\PhpConfigPrinter\NodeFactory;

use ConfigTransformer2021120610\PhpParser\Node\Expr\Assign;
...
use ConfigTransformer2021120610\PhpParser\Node\Stmt\Return_;
use ConfigTransformer2021120610\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface;
use ConfigTransformer2021120610\Symplify\PhpConfigPrinter\PhpParser\NodeFactory\ConfiguratorClosureNodeFactory;
use ConfigTransformer2021120610\Symplify\PhpConfigPrinter\ValueObject\VariableMethodName;
...
final class ContainerConfiguratorReturnClosureFactory
{
     ...
     */
    public function __construct(\ConfigTransformer2021120610\Symplify\PhpConfigPrinter\PhpParser\NodeFactory\ConfiguratorClosureNodeFactory $configuratorClosureNodeFactory, array $caseConverters, \ConfigTransformer2021120610\Symplify\PhpConfigPrinter\NodeFactory\ContainerNestedNodesFactory $containerNestedNodesFactory)
    {
     ...
     */
    public function createFromYamlArray(array $arrayData) : \ConfigTransformer2021120610\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \ConfigTransformer2021120610\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     * @return Stmt[]
     */
    ...
    private function createInitializeAssign(string $variableMethodName) : \ConfigTransformer2021120610\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \ConfigTransformer2021120610\PhpParser\Node\Expr\Variable($variableMethodName);
        ...
        $assign = new \ConfigTransformer2021120610\PhpParser\Node\Expr\Assign($servicesVariable, new \ConfigTransformer2021120610\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $variableMethodName));
        return new \ConfigTransformer2021120610\PhpParser\Node\Stmt\Expression($assign);
    }
    /**
     * @return mixed[]
     */
    ...
    {
        if ($key === \ConfigTransformer2021120610\Symplify\PhpConfigPrinter\ValueObject\YamlKey::SERVICES) {
            $nodes[] = $this->createInitializeAssign(\ConfigTransformer2021120610\Symplify\PhpConfigPrinter\ValueObject\VariableMethodName::SERVICES);
            return $nodes;
        }
        if ($key === \ConfigTransformer2021120610\Symplify\PhpConfigPrinter\ValueObject\YamlKey::PARAMETERS) {
            $nodes[] = $this->createInitializeAssign(\ConfigTransformer2021120610\Symplify\PhpConfigPrinter\ValueObject\VariableMethodName::PARAMETERS);
            return $nodes;
        }
        return $nodes;
    }
     ...
     */
    private function resolveExpression(string $key, $nestedKey, $nestedValues) : ?\ConfigTransformer2021120610\PhpParser\Node\Stmt\Expression
    {
