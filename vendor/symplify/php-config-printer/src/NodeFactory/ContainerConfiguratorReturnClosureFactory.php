...
use ConfigTransformer202206055\PhpParser\Node\Stmt\Return_;
use Symfony\Component\DependencyInjection\Loader\Configurator\ContainerConfigurator;
use ConfigTransformer202206055\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface;
use ConfigTransformer202206055\Symplify\PhpConfigPrinter\PhpParser\NodeFactory\ConfiguratorClosureNodeFactory;
use ConfigTransformer202206055\Symplify\PhpConfigPrinter\ValueObject\VariableMethodName;
...
final class ContainerConfiguratorReturnClosureFactory
{
     ...
     */
    public function __construct(\ConfigTransformer202206055\Symplify\PhpConfigPrinter\PhpParser\NodeFactory\ConfiguratorClosureNodeFactory $configuratorClosureNodeFactory, array $caseConverters, \ConfigTransformer202206055\Symplify\PhpConfigPrinter\NodeFactory\ContainerNestedNodesFactory $containerNestedNodesFactory)
    {
     ...
     */
    public function createFromYamlArray(array $arrayData, string $containerConfiguratorClass = \Symfony\Component\DependencyInjection\Loader\Configurator\ContainerConfigurator::class) : \ConfigTransformer202206055\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts, $containerConfiguratorClass);
        return new \ConfigTransformer202206055\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     ...
     */
    private function createInitializeAssign(string $variableMethodName) : \ConfigTransformer202206055\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \ConfigTransformer202206055\PhpParser\Node\Expr\Variable($variableMethodName);
        $containerConfiguratorVariable = new \ConfigTransformer202206055\PhpParser\Node\Expr\Variable(\ConfigTransformer202206055\Symplify\PhpConfigPrinter\ValueObject\VariableName::CONTAINER_CONFIGURATOR);
        $assign = new \ConfigTransformer202206055\PhpParser\Node\Expr\Assign($servicesVariable, new \ConfigTransformer202206055\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $variableMethodName));
        return new \ConfigTransformer202206055\PhpParser\Node\Stmt\Expression($assign);
    }
    /**
    ...
    {
        if ($key === \ConfigTransformer202206055\Symplify\PhpConfigPrinter\ValueObject\YamlKey::SERVICES) {
            $nodes[] = $this->createInitializeAssign(\ConfigTransformer202206055\Symplify\PhpConfigPrinter\ValueObject\VariableMethodName::SERVICES);
            return $nodes;
        }
        if ($key === \ConfigTransformer202206055\Symplify\PhpConfigPrinter\ValueObject\YamlKey::PARAMETERS) {
            $nodes[] = $this->createInitializeAssign(\ConfigTransformer202206055\Symplify\PhpConfigPrinter\ValueObject\VariableMethodName::PARAMETERS);
            return $nodes;
        }
        return $nodes;
    }
     ...
     */
    private function resolveExpression(string $key, $nestedKey, $nestedValues) : ?\ConfigTransformer202206055\PhpParser\Node\Stmt\Expression
    {
