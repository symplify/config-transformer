...
namespace ConfigTransformer2022022410\Symplify\PhpConfigPrinter\NodeFactory;

use ConfigTransformer2022022410\PhpParser\Node\Expr\Assign;
...
use ConfigTransformer2022022410\PhpParser\Node\Stmt\Return_;
use ConfigTransformer2022022410\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface;
use ConfigTransformer2022022410\Symplify\PhpConfigPrinter\PhpParser\NodeFactory\ConfiguratorClosureNodeFactory;
use ConfigTransformer2022022410\Symplify\PhpConfigPrinter\ValueObject\VariableMethodName;
...
final class ContainerConfiguratorReturnClosureFactory
{
     ...
     */
    public function __construct(\ConfigTransformer2022022410\Symplify\PhpConfigPrinter\PhpParser\NodeFactory\ConfiguratorClosureNodeFactory $configuratorClosureNodeFactory, array $caseConverters, \ConfigTransformer2022022410\Symplify\PhpConfigPrinter\NodeFactory\ContainerNestedNodesFactory $containerNestedNodesFactory)
    {
     ...
     */
    public function createFromYamlArray(array $arrayData) : \ConfigTransformer2022022410\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \ConfigTransformer2022022410\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
    ...
    private function createInitializeAssign(string $variableMethodName) : \ConfigTransformer2022022410\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \ConfigTransformer2022022410\PhpParser\Node\Expr\Variable($variableMethodName);
        ...
        $assign = new \ConfigTransformer2022022410\PhpParser\Node\Expr\Assign($servicesVariable, new \ConfigTransformer2022022410\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $variableMethodName));
        return new \ConfigTransformer2022022410\PhpParser\Node\Stmt\Expression($assign);
    }
    /**
    ...
    {
        if ($key === \ConfigTransformer2022022410\Symplify\PhpConfigPrinter\ValueObject\YamlKey::SERVICES) {
            $nodes[] = $this->createInitializeAssign(\ConfigTransformer2022022410\Symplify\PhpConfigPrinter\ValueObject\VariableMethodName::SERVICES);
            return $nodes;
        }
        if ($key === \ConfigTransformer2022022410\Symplify\PhpConfigPrinter\ValueObject\YamlKey::PARAMETERS) {
            $nodes[] = $this->createInitializeAssign(\ConfigTransformer2022022410\Symplify\PhpConfigPrinter\ValueObject\VariableMethodName::PARAMETERS);
            return $nodes;
        }
        return $nodes;
    }
     ...
     */
    private function resolveExpression(string $key, $nestedKey, $nestedValues) : ?\ConfigTransformer2022022410\PhpParser\Node\Stmt\Expression
    {
