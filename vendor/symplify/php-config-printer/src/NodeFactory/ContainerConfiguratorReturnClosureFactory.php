...
use ConfigTransformer202107055\PhpParser\Node\Expr\Assign;
...
use ConfigTransformer202107055\PhpParser\Node\Stmt\Return_;
use ConfigTransformer202107055\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface;
use ConfigTransformer202107055\Symplify\PhpConfigPrinter\PhpParser\NodeFactory\ConfiguratorClosureNodeFactory;
...
final class ContainerConfiguratorReturnClosureFactory
{
     ...
     */
    public function __construct(\ConfigTransformer202107055\Symplify\PhpConfigPrinter\PhpParser\NodeFactory\ConfiguratorClosureNodeFactory $configuratorClosureNodeFactory, array $caseConverters, \ConfigTransformer202107055\Symplify\PhpConfigPrinter\NodeFactory\ContainerNestedNodesFactory $containerNestedNodesFactory)
    {
     ...
     */
    public function createFromYamlArray(array $arrayData) : \ConfigTransformer202107055\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \ConfigTransformer202107055\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     * @return Stmt[]
     */
    ...
    private function createNodesFromCaseConverters(array $yamlData) : array
    {
        ...
        foreach ($yamlData as $key => $values) {
            ...
            foreach ($values as $nestedKey => $nestedValues) {
                ...
                if (!$expression instanceof \ConfigTransformer202107055\PhpParser\Node\Stmt\Expression) {
                    continue;
                }
                $nodes[] = $expression;
            }
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \ConfigTransformer202107055\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \ConfigTransformer202107055\PhpParser\Node\Expr\Variable($variableName);
        ...
        $assign = new \ConfigTransformer202107055\PhpParser\Node\Expr\Assign($servicesVariable, new \ConfigTransformer202107055\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \ConfigTransformer202107055\PhpParser\Node\Stmt\Expression($assign);
    }
    /**
     * @return mixed[]
     */
    ...
    {
        if ($key === \ConfigTransformer202107055\Symplify\PhpConfigPrinter\ValueObject\YamlKey::SERVICES) {
            $nodes[] = $this->createInitializeAssign(\ConfigTransformer202107055\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES, \ConfigTransformer202107055\Symplify\PhpConfigPrinter\ValueObject\MethodName::SERVICES);
        }
        if ($key === \ConfigTransformer202107055\Symplify\PhpConfigPrinter\ValueObject\YamlKey::PARAMETERS) {
            $nodes[] = $this->createInitializeAssign(\ConfigTransformer202107055\Symplify\PhpConfigPrinter\ValueObject\VariableName::PARAMETERS, \ConfigTransformer202107055\Symplify\PhpConfigPrinter\ValueObject\MethodName::PARAMETERS);
        }
        return $nodes;
    }
    /**
     ...
     */
    private function resolveExpression(string $key, $nestedKey, $nestedValues) : ?\ConfigTransformer202107055\PhpParser\Node\Stmt\Expression
    {
