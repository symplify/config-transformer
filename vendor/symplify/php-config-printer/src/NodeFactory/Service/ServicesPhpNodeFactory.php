...
final class ServicesPhpNodeFactory
{
     ...
     */
    public function createResource(string $serviceKey, array $serviceValues) : \ConfigTransformer2022051310\PhpParser\Node\Stmt\Expression
    {
        ...
        $excludeMethodCall = new \ConfigTransformer2022051310\PhpParser\Node\Expr\MethodCall($decoratedMethodCall, self::EXCLUDE, $args);
        return new \ConfigTransformer2022051310\PhpParser\Node\Stmt\Expression($excludeMethodCall);
    }
    /**
     * @param mixed[] $serviceValues
     */
    private function createServicesLoadMethodCall(string $serviceKey, array $serviceValues) : \ConfigTransformer2022051310\PhpParser\Node\Expr\MethodCall
    {
        ...
        $args = [];
        $args[] = new \ConfigTransformer2022051310\PhpParser\Node\Arg(new \ConfigTransformer2022051310\PhpParser\Node\Scalar\String_($serviceKey));
        $args[] = new \ConfigTransformer2022051310\PhpParser\Node\Arg($this->commonNodeFactory->createAbsoluteDirExpr($resource));
        return new \ConfigTransformer2022051310\PhpParser\Node\Expr\MethodCall($servicesVariable, 'load', $args);
    }
}
