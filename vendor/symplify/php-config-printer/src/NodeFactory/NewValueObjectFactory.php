...
final class NewValueObjectFactory
{
    ...
    public function create(object $valueObject)
    {
        ...
        if ($valueObject instanceof \ConfigTransformer202206079\MyCLabs\Enum\Enum) {
            return new \ConfigTransformer202206079\PhpParser\Node\Expr\StaticCall(new \ConfigTransformer202206079\PhpParser\Node\Name\FullyQualified($valueObjectClass), $valueObject->getKey());
        }
        // assumption that constructor parameters share the same value as property names
        $propertyValues = $this->resolvePropertyValuesFromValueObject($valueObjectClass, $valueObject);
        $args = $this->createArgs($propertyValues);
        return new \ConfigTransformer202206079\PhpParser\Node\Expr\New_(new \ConfigTransformer202206079\PhpParser\Node\Name\FullyQualified($valueObjectClass), $args);
    }
    /**
     * @return mixed[]
     */
    ...
    private function createArgs(array $propertyValues) : array
    {
        ...
        foreach ($propertyValues as $propertyValue) {
            if (\is_object($propertyValue)) {
                ...
                $args[] = new \ConfigTransformer202206079\PhpParser\Node\Arg($nestedValueObject);
            } elseif (\is_array($propertyValue)) {
                $args[] = new \ConfigTransformer202206079\PhpParser\Node\Arg(new \ConfigTransformer202206079\PhpParser\Node\Expr\Array_($this->createArgs($propertyValue)));
            } else {
                $args[] = new \ConfigTransformer202206079\PhpParser\Node\Arg(\ConfigTransformer202206079\PhpParser\BuilderHelpers::normalizeValue($propertyValue));
            }
        }
        return $args;
    }
