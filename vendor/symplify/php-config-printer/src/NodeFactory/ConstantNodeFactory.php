...
use ConfigTransformer202201268\Symplify\PhpConfigPrinter\Dummy\YamlContentProvider;
/**
...
final class ConstantNodeFactory
{
    ...
    public function __construct(\ConfigTransformer202201268\Symplify\PhpConfigPrinter\Dummy\YamlContentProvider $yamlContentProvider)
    {
     ...
     */
    public function createConstantIfValue(string $value) : ?\ConfigTransformer202201268\PhpParser\Node\Expr
    {
        if (\strpos($value, '::') !== \false) {
            ...
            return new \ConfigTransformer202201268\PhpParser\Node\Expr\ClassConstFetch(new \ConfigTransformer202201268\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\ConfigTransformer202201268\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \ConfigTransformer202201268\PhpParser\Node\Expr\ConstFetch(new \ConfigTransformer202201268\PhpParser\Node\Name($constantName));
        }
        return null;
    }
    /**
