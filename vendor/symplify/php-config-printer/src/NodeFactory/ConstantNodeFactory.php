...
use ConfigTransformer2022030710\PhpParser\Node\Expr\ConstFetch;
use ConfigTransformer2022030710\PhpParser\Node\Name;
use ConfigTransformer2022030710\PhpParser\Node\Name\FullyQualified;
use ConfigTransformer2022030710\Symplify\PhpConfigPrinter\Dummy\YamlContentProvider;
/**
...
final class ConstantNodeFactory
{
    ...
    public function __construct(\ConfigTransformer2022030710\Symplify\PhpConfigPrinter\Dummy\YamlContentProvider $yamlContentProvider)
    {
     ...
     */
    public function createConstantIfValue(string $value) : ?\ConfigTransformer2022030710\PhpParser\Node\Expr
    {
        if (\strpos($value, '::') !== \false) {
            ...
            return new \ConfigTransformer2022030710\PhpParser\Node\Expr\ClassConstFetch(new \ConfigTransformer2022030710\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\ConfigTransformer2022030710\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \ConfigTransformer2022030710\PhpParser\Node\Expr\ConstFetch(new \ConfigTransformer2022030710\PhpParser\Node\Name($constantName));
        }
        return null;
    }
    /**
