...
use ConfigTransformer202106282\PhpParser\Node\Expr\ConstFetch;
use ConfigTransformer202106282\PhpParser\Node\Name;
use ConfigTransformer202106282\PhpParser\Node\Name\FullyQualified;
use ConfigTransformer202106282\Symplify\PhpConfigPrinter\Contract\YamlFileContentProviderInterface;
/**
...
final class ConstantNodeFactory
{
    ...
    public function __construct(\ConfigTransformer202106282\Symplify\PhpConfigPrinter\Contract\YamlFileContentProviderInterface $yamlFileContentProvider)
    {
     ...
     */
    public function createConstantIfValue(string $value) : ?\ConfigTransformer202106282\PhpParser\Node\Expr
    {
        if (\strpos($value, '::') !== \false) {
            ...
            return new \ConfigTransformer202106282\PhpParser\Node\Expr\ClassConstFetch(new \ConfigTransformer202106282\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\ConfigTransformer202106282\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \ConfigTransformer202106282\PhpParser\Node\Expr\ConstFetch(new \ConfigTransformer202106282\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
