...
final class ConstantNodeFactory
{
    ...
    public function __construct(\ConfigTransformer202108251\Symplify\PhpConfigPrinter\Contract\YamlFileContentProviderInterface $yamlFileContentProvider)
    {
     ...
     */
    public function createConstantIfValue(string $value) : ?\ConfigTransformer202108251\PhpParser\Node\Expr
    {
        if (\strpos($value, '::') !== \false) {
            ...
            return new \ConfigTransformer202108251\PhpParser\Node\Expr\ClassConstFetch(new \ConfigTransformer202108251\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\ConfigTransformer202108251\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \ConfigTransformer202108251\PhpParser\Node\Expr\ConstFetch(new \ConfigTransformer202108251\PhpParser\Node\Name($constantName));
        }
        return null;
    }
    /**
