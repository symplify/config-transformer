...
final class ClassServiceCaseConverter implements \ConfigTransformer202106130\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    /**
    ...
    {
        $args = $this->argsNodeFactory->createFromValues([$key, $values[\ConfigTransformer202106130\Symplify\PhpConfigPrinter\ValueObject\YamlKey::CLASS_KEY]]);
        $setMethodCall = new \ConfigTransformer202106130\PhpParser\Node\Expr\MethodCall(new \ConfigTransformer202106130\PhpParser\Node\Expr\Variable(\ConfigTransformer202106130\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \ConfigTransformer202106130\Symplify\PhpConfigPrinter\ValueObject\MethodName::SET, $args);
        unset($values[\ConfigTransformer202106130\Symplify\PhpConfigPrinter\ValueObject\YamlKey::CLASS_KEY]);
        $setMethodCall = $this->serviceOptionNodeFactory->convertServiceOptionsToNodes($values, $setMethodCall);
        return new \ConfigTransformer202106130\PhpParser\Node\Stmt\Expression($setMethodCall);
    }
    public function match(string $rootKey, $key, $values) : bool
    {
        ...
        return !isset($values[\ConfigTransformer202106130\Symplify\PhpConfigPrinter\ValueObject\YamlKey::ALIAS]);
    }
}
