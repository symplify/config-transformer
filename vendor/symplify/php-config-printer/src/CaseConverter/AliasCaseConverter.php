...
use ConfigTransformer2022052210\Nette\Utils\Strings;
...
final class AliasCaseConverter implements \ConfigTransformer2022052210\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    /**
    ...
    public function convertToMethodCall($key, $values) : \ConfigTransformer2022052210\PhpParser\Node\Stmt\Expression
    {
        ...
        $fullClassName = \ConfigTransformer2022052210\Nette\Utils\Strings::before($key, ' $');
        ...
        if (\is_string($values) && $values[0] === '@') {
            ...
            $methodCall = new \ConfigTransformer2022052210\PhpParser\Node\Expr\MethodCall($servicesVariable, \ConfigTransformer2022052210\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
            return new \ConfigTransformer2022052210\PhpParser\Node\Stmt\Expression($methodCall);
        }
        if (\is_array($values)) {
    ...
    public function match(string $rootKey, $key, $values) : bool
    {
        ...
        if (\ConfigTransformer2022052210\Nette\Utils\Strings::match($key, self::NAMED_ALIAS_REGEX)) {
            return \true;
        }
        if (!\is_string($values)) {
            return \false;
     ...
     */
    private function createAliasNode(string $key, string $fullClassName, $serviceValues) : \ConfigTransformer2022052210\PhpParser\Node\Expr\MethodCall
    {
        $args = [];
        $classConstFetch = $this->commonNodeFactory->createClassReference($fullClassName);
        \ConfigTransformer2022052210\Nette\Utils\Strings::match($key, self::ARGUMENT_NAME_REGEX);
        $argumentName = '$' . \ConfigTransformer2022052210\Nette\Utils\Strings::after($key, '$');
        $concat = new \ConfigTransformer2022052210\PhpParser\Node\Expr\BinaryOp\Concat($classConstFetch, new \ConfigTransformer2022052210\PhpParser\Node\Scalar\String_(' ' . $argumentName));
        $args[] = new \ConfigTransformer2022052210\PhpParser\Node\Arg($concat);
        $serviceName = \ltrim($serviceValues, '@');
        $args[] = new \ConfigTransformer2022052210\PhpParser\Node\Arg(new \ConfigTransformer2022052210\PhpParser\Node\Scalar\String_($serviceName));
        return new \ConfigTransformer2022052210\PhpParser\Node\Expr\MethodCall(new \ConfigTransformer2022052210\PhpParser\Node\Expr\Variable(\ConfigTransformer2022052210\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \ConfigTransformer2022052210\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
    }
    /**
     * @param mixed $values
     */
    private function createFromClassLike(string $key, $values, \ConfigTransformer2022052210\PhpParser\Node\Expr\Variable $servicesVariable) : \ConfigTransformer2022052210\PhpParser\Node\Stmt\Expression
    {
        $classReference = $this->commonNodeFactory->createClassReference($key);
        $argValues = [];
        $argValues[] = $classReference;
        $argValues[] = $values[\ConfigTransformer2022052210\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS] ?? $values;
        $args = $this->argsNodeFactory->createFromValues($argValues, \true);
        $methodCall = new \ConfigTransformer2022052210\PhpParser\Node\Expr\MethodCall($servicesVariable, \ConfigTransformer2022052210\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
        return new \ConfigTransformer2022052210\PhpParser\Node\Stmt\Expression($methodCall);
    }
    private function createFromAlias(string $serviceName, string $key, \ConfigTransformer2022052210\PhpParser\Node\Expr\Variable $servicesVariable) : \ConfigTransformer2022052210\PhpParser\Node\Expr\MethodCall
    {
        ...
        return new \ConfigTransformer2022052210\PhpParser\Node\Expr\MethodCall($servicesVariable, \ConfigTransformer2022052210\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
    }
    /**
     * @param mixed[] $values
     */
    private function createFromArrayValues(array $values, string $key, \ConfigTransformer2022052210\PhpParser\Node\Expr\Variable $servicesVariable) : \ConfigTransformer2022052210\PhpParser\Node\Stmt\Expression
    {
        if (isset($values[\ConfigTransformer2022052210\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS])) {
            $methodCall = $this->createFromAlias($values[\ConfigTransformer2022052210\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS], $key, $servicesVariable);
            unset($values[\ConfigTransformer2022052210\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS]);
        } else {
            throw new \ConfigTransformer2022052210\Symplify\SymplifyKernel\Exception\ShouldNotHappenException();
        }
