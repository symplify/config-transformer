...
final class ConfiguredServiceCaseConverter implements \ConfigTransformer202106114\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    /**
     * @var ArgsNodeFactory
     */
    ...
    public function convertToMethodCall($key, $values) : \ConfigTransformer202106114\PhpParser\Node\Stmt\Expression
    {
        ...
        if (isset($values[\ConfigTransformer202106114\Symplify\PhpConfigPrinter\ValueObject\YamlKey::CLASS_KEY])) {
            $valuesForArgs[] = $values[\ConfigTransformer202106114\Symplify\PhpConfigPrinter\ValueObject\YamlKey::CLASS_KEY];
        }
        $args = $this->argsNodeFactory->createFromValues($valuesForArgs);
        $methodCall = new \ConfigTransformer202106114\PhpParser\Node\Expr\MethodCall(new \ConfigTransformer202106114\PhpParser\Node\Expr\Variable(\ConfigTransformer202106114\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \ConfigTransformer202106114\Symplify\PhpConfigPrinter\ValueObject\MethodName::SET, $args);
    ...
    public function match(string $rootKey, $key, $values) : bool
    {
        ...
        if ($key === \ConfigTransformer202106114\Symplify\PhpConfigPrinter\ValueObject\YamlKey::_DEFAULTS) {
            return \false;
        }
        if ($key === \ConfigTransformer202106114\Symplify\PhpConfigPrinter\ValueObject\YamlKey::_INSTANCEOF) {
            return \false;
        }
        if (isset($values[\ConfigTransformer202106114\Symplify\PhpConfigPrinter\ValueObject\YamlKey::RESOURCE])) {
            return \false;
        }
    ...
    private function isAlias($values) : bool
    {
        ...
        return \ConfigTransformer202106114\Nette\Utils\Strings::startsWith($values, '@');
    }
}
