...
final class ConfiguredServiceCaseConverter implements \ConfigTransformer202109216\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    /**
    ...
    public function convertToMethodCall($key, $values) : \ConfigTransformer202109216\PhpParser\Node\Stmt\Expression
    {
        ...
        if (isset($values[\ConfigTransformer202109216\Symplify\PhpConfigPrinter\ValueObject\YamlKey::CLASS_KEY])) {
            $valuesForArgs[] = $values[\ConfigTransformer202109216\Symplify\PhpConfigPrinter\ValueObject\YamlKey::CLASS_KEY];
        }
    ...
    public function match($rootKey, $key, $values) : bool
    {
        ...
        if ($key === \ConfigTransformer202109216\Symplify\PhpConfigPrinter\ValueObject\YamlKey::_DEFAULTS) {
            return \false;
        }
        if ($key === \ConfigTransformer202109216\Symplify\PhpConfigPrinter\ValueObject\YamlKey::_INSTANCEOF) {
            return \false;
        }
        if (isset($values[\ConfigTransformer202109216\Symplify\PhpConfigPrinter\ValueObject\YamlKey::RESOURCE])) {
            return \false;
        }
