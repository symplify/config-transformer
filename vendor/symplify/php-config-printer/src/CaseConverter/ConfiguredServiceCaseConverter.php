...
final class ConfiguredServiceCaseConverter implements \ConfigTransformer2021113010\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    /**
    ...
    public function convertToMethodCall($key, $values) : \ConfigTransformer2021113010\PhpParser\Node\Stmt\Expression
    {
        ...
        if (isset($values[\ConfigTransformer2021113010\Symplify\PhpConfigPrinter\ValueObject\YamlKey::CLASS_KEY])) {
            $valuesForArgs[] = $values[\ConfigTransformer2021113010\Symplify\PhpConfigPrinter\ValueObject\YamlKey::CLASS_KEY];
        }
    ...
    public function match($rootKey, $key, $values) : bool
    {
        ...
        if ($key === \ConfigTransformer2021113010\Symplify\PhpConfigPrinter\ValueObject\YamlKey::_DEFAULTS) {
            return \false;
        }
        if ($key === \ConfigTransformer2021113010\Symplify\PhpConfigPrinter\ValueObject\YamlKey::_INSTANCEOF) {
            return \false;
        }
        if (isset($values[\ConfigTransformer2021113010\Symplify\PhpConfigPrinter\ValueObject\YamlKey::RESOURCE])) {
            return \false;
        }
