...
final class ConfiguredServiceCaseConverter implements \ConfigTransformer202110016\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    /**
    ...
    public function convertToMethodCall($key, $values) : \ConfigTransformer202110016\PhpParser\Node\Stmt\Expression
    {
        ...
        if (isset($values[\ConfigTransformer202110016\Symplify\PhpConfigPrinter\ValueObject\YamlKey::CLASS_KEY])) {
            $valuesForArgs[] = $values[\ConfigTransformer202110016\Symplify\PhpConfigPrinter\ValueObject\YamlKey::CLASS_KEY];
        }
    ...
    public function match($rootKey, $key, $values) : bool
    {
        ...
        if ($key === \ConfigTransformer202110016\Symplify\PhpConfigPrinter\ValueObject\YamlKey::_DEFAULTS) {
            return \false;
        }
        if ($key === \ConfigTransformer202110016\Symplify\PhpConfigPrinter\ValueObject\YamlKey::_INSTANCEOF) {
            return \false;
        }
        if (isset($values[\ConfigTransformer202110016\Symplify\PhpConfigPrinter\ValueObject\YamlKey::RESOURCE])) {
            return \false;
        }
