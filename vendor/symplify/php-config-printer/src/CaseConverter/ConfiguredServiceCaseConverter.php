...
final class ConfiguredServiceCaseConverter implements \ConfigTransformer2021070510\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    /**
    ...
    public function convertToMethodCall($key, $values) : \ConfigTransformer2021070510\PhpParser\Node\Stmt\Expression
    {
        ...
        if (isset($values[\ConfigTransformer2021070510\Symplify\PhpConfigPrinter\ValueObject\YamlKey::CLASS_KEY])) {
            $valuesForArgs[] = $values[\ConfigTransformer2021070510\Symplify\PhpConfigPrinter\ValueObject\YamlKey::CLASS_KEY];
        }
        $args = $this->argsNodeFactory->createFromValues($valuesForArgs);
        $methodCall = new \ConfigTransformer2021070510\PhpParser\Node\Expr\MethodCall(new \ConfigTransformer2021070510\PhpParser\Node\Expr\Variable(\ConfigTransformer2021070510\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \ConfigTransformer2021070510\Symplify\PhpConfigPrinter\ValueObject\MethodName::SET, $args);
    ...
    public function match(string $rootKey, $key, $values) : bool
    {
        ...
        if ($key === \ConfigTransformer2021070510\Symplify\PhpConfigPrinter\ValueObject\YamlKey::_DEFAULTS) {
            return \false;
        }
        if ($key === \ConfigTransformer2021070510\Symplify\PhpConfigPrinter\ValueObject\YamlKey::_INSTANCEOF) {
            return \false;
        }
        if (isset($values[\ConfigTransformer2021070510\Symplify\PhpConfigPrinter\ValueObject\YamlKey::RESOURCE])) {
            return \false;
        }
