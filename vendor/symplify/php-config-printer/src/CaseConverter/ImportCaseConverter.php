...
use ConfigTransformer202206079\Symplify\PhpConfigPrinter\Sorter\YamlArgumentSorter;
 ...
 * imports: <---
 */
final class ImportCaseConverter implements \ConfigTransformer202206079\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    /**
    ...
    public function __construct(\ConfigTransformer202206079\Symplify\PhpConfigPrinter\Sorter\YamlArgumentSorter $yamlArgumentSorter, \ConfigTransformer202206079\Symplify\PhpConfigPrinter\NodeFactory\CommonNodeFactory $commonNodeFactory)
    {
    ...
    {
        return $rootKey === \ConfigTransformer202206079\Symplify\PhpConfigPrinter\ValueObject\YamlKey::IMPORTS;
    }
    /**
     * @param mixed $key
     * @param mixed $values
    ...
    {
        if (\is_array($values)) {
            $arguments = $this->yamlArgumentSorter->sortArgumentsByKeyIfExists($values, [\ConfigTransformer202206079\Symplify\PhpConfigPrinter\ValueObject\YamlKey::RESOURCE => '', 'type' => null, \ConfigTransformer202206079\Symplify\PhpConfigPrinter\ValueObject\YamlKey::IGNORE_ERRORS => \false]);
     ...
     */
    private function createImportMethodCall(array $arguments) : \ConfigTransformer202206079\PhpParser\Node\Stmt\Expression
    {
        ...
        $methodCall = new \ConfigTransformer202206079\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, 'import', $args);
    ...
    private function shouldSkipDefaultValue(string $name, $value, array $arguments) : bool
    {
        ...
        if ($name === \ConfigTransformer202206079\Symplify\PhpConfigPrinter\ValueObject\YamlKey::IGNORE_ERRORS && $value === \false) {
            return \true;
        }
        ...
        if (!isset($arguments[\ConfigTransformer202206079\Symplify\PhpConfigPrinter\ValueObject\YamlKey::IGNORE_ERRORS])) {
            return \false;
        }
        return $arguments[\ConfigTransformer202206079\Symplify\PhpConfigPrinter\ValueObject\YamlKey::IGNORE_ERRORS] === \false;
    }
    /**
     * @param mixed $value
     * @return mixed
    ...
    private function replaceImportedFileSuffix($value)
    {
        ...
        return \ConfigTransformer202206079\Nette\Utils\Strings::replace($value, self::INPUT_SUFFIX_REGEX, '.php');
    }
    /**
     * @param mixed $value
     */
    private function resolveExpr($value) : \ConfigTransformer202206079\PhpParser\Node\Expr
    {
        if (\is_bool($value)) {
        ...
        if ($value === 'not_found') {
            return new \ConfigTransformer202206079\PhpParser\Node\Scalar\String_('not_found');
        }
        if (\is_string($value) && \strpos($value, '::') !== \false) {
            [$className, $constantName] = \explode('::', $value);
            return new \ConfigTransformer202206079\PhpParser\Node\Expr\ClassConstFetch(new \ConfigTransformer202206079\PhpParser\Node\Name\FullyQualified($className), $constantName);
        }
