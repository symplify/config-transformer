...
final class InstanceOfNestedCaseConverter
{
    ...
    public function __construct(\ConfigTransformer202108297\Symplify\PhpConfigPrinter\NodeFactory\CommonNodeFactory $commonNodeFactory, \ConfigTransformer202108297\Symplify\PhpConfigPrinter\NodeFactory\Service\ServiceOptionNodeFactory $serviceOptionNodeFactory)
    {
    ...
    public function convertToMethodCall($key, $values) : \ConfigTransformer202108297\PhpParser\Node\Stmt\Expression
    {
        ...
        $args = [new \ConfigTransformer202108297\PhpParser\Node\Arg($classConstFetch)];
        $instanceofMethodCall = new \ConfigTransformer202108297\PhpParser\Node\Expr\MethodCall($servicesVariable, \ConfigTransformer202108297\Symplify\PhpConfigPrinter\ValueObject\MethodName::INSTANCEOF, $args);
        $decoreatedInstanceofMethodCall = $this->serviceOptionNodeFactory->convertServiceOptionsToNodes($values, $instanceofMethodCall);
        return new \ConfigTransformer202108297\PhpParser\Node\Stmt\Expression($decoreatedInstanceofMethodCall);
    }
    public function isMatch(string $rootKey, $subKey) : bool
    {
        ...
        return $subKey === \ConfigTransformer202108297\Symplify\PhpConfigPrinter\ValueObject\YamlKey::_INSTANCEOF;
    }
}
