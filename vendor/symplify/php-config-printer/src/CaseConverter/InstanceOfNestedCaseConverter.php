...
final class InstanceOfNestedCaseConverter
{
    ...
    public function __construct(\ConfigTransformer2021071010\Symplify\PhpConfigPrinter\NodeFactory\CommonNodeFactory $commonNodeFactory, \ConfigTransformer2021071010\Symplify\PhpConfigPrinter\NodeFactory\Service\ServiceOptionNodeFactory $serviceOptionNodeFactory)
    {
    ...
    public function convertToMethodCall($key, $values) : \ConfigTransformer2021071010\PhpParser\Node\Stmt\Expression
    {
        ...
        $args = [new \ConfigTransformer2021071010\PhpParser\Node\Arg($classConstFetch)];
        $instanceofMethodCall = new \ConfigTransformer2021071010\PhpParser\Node\Expr\MethodCall($servicesVariable, \ConfigTransformer2021071010\Symplify\PhpConfigPrinter\ValueObject\MethodName::INSTANCEOF, $args);
        $instanceofMethodCall = $this->serviceOptionNodeFactory->convertServiceOptionsToNodes($values, $instanceofMethodCall);
        return new \ConfigTransformer2021071010\PhpParser\Node\Stmt\Expression($instanceofMethodCall);
    }
    public function isMatch(string $rootKey, $subKey) : bool
    {
        ...
        return $subKey === \ConfigTransformer2021071010\Symplify\PhpConfigPrinter\ValueObject\YamlKey::_INSTANCEOF;
    }
}
