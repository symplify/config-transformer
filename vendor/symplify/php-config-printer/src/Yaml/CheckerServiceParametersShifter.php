...
namespace ConfigTransformer2021061110\Symplify\PhpConfigPrinter\Yaml;

use ConfigTransformer2021061110\Nette\Utils\Strings;
use ConfigTransformer2021061110\Symplify\PackageBuilder\Strings\StringFormatConverter;
/**
...
final class CheckerServiceParametersShifter
{
    ...
    {
        $this->stringFormatConverter = new \ConfigTransformer2021061110\Symplify\PackageBuilder\Strings\StringFormatConverter();
    }
    /**
    ...
    private function processServices(array $services) : array
    {
        foreach ($services as $serviceName => $serviceDefinition) {
            ...
            if (\ConfigTransformer2021061110\Nette\Utils\Strings::endsWith($serviceName, 'Fixer')) {
                $services = $this->processFixer($services, $serviceName, $serviceDefinition);
            }
            if (\ConfigTransformer2021061110\Nette\Utils\Strings::endsWith($serviceName, 'Sniff')) {
    ...
    {
        if (\ConfigTransformer2021061110\Nette\Utils\Strings::endsWith($checker, 'Fixer')) {
            return \true;
        }
        return \ConfigTransformer2021061110\Nette\Utils\Strings::endsWith($checker, 'Sniff');
    }
    /**
     * @param mixed[] $services
    ...
    private function escapeValue($value)
    {
        ...
        return \ConfigTransformer2021061110\Nette\Utils\Strings::replace($value, '#^@#', '@@');
    }
}
