...
namespace ConfigTransformer202106120\Symplify\PhpConfigPrinter\Yaml;

use ConfigTransformer202106120\Nette\Utils\Strings;
use ConfigTransformer202106120\Symplify\PackageBuilder\Strings\StringFormatConverter;
/**
...
final class CheckerServiceParametersShifter
{
    ...
    {
        $this->stringFormatConverter = new \ConfigTransformer202106120\Symplify\PackageBuilder\Strings\StringFormatConverter();
    }
    /**
    ...
    private function processServices(array $services) : array
    {
        foreach ($services as $serviceName => $serviceDefinition) {
            ...
            if (\ConfigTransformer202106120\Nette\Utils\Strings::endsWith($serviceName, 'Fixer')) {
                $services = $this->processFixer($services, $serviceName, $serviceDefinition);
            }
            if (\ConfigTransformer202106120\Nette\Utils\Strings::endsWith($serviceName, 'Sniff')) {
    ...
    {
        if (\ConfigTransformer202106120\Nette\Utils\Strings::endsWith($checker, 'Fixer')) {
            return \true;
        }
        return \ConfigTransformer202106120\Nette\Utils\Strings::endsWith($checker, 'Sniff');
    }
    /**
     * @param mixed[] $services
    ...
    private function escapeValue($value)
    {
        ...
        return \ConfigTransformer202106120\Nette\Utils\Strings::replace($value, '#^@#', '@@');
    }
}
