...
namespace ConfigTransformer2021061810\Symplify\PhpConfigPrinter\ExprResolver;
...
use ConfigTransformer2021061810\PhpParser\Node\Expr\FuncCall;
...
use ConfigTransformer2021061810\Symplify\Astral\ValueObject\AttributeKey;
use ConfigTransformer2021061810\Symplify\PhpConfigPrinter\Configuration\SymfonyFunctionNameProvider;
use ConfigTransformer2021061810\Symplify\PhpConfigPrinter\NodeFactory\CommonNodeFactory;
use ConfigTransformer2021061810\Symplify\PhpConfigPrinter\NodeFactory\ConstantNodeFactory;
...
final class StringExprResolver
{
    ...
    public function __construct(\ConfigTransformer2021061810\Symplify\PhpConfigPrinter\NodeFactory\ConstantNodeFactory $constantNodeFactory, \ConfigTransformer2021061810\Symplify\PhpConfigPrinter\NodeFactory\CommonNodeFactory $commonNodeFactory, \ConfigTransformer2021061810\Symplify\PhpConfigPrinter\Configuration\SymfonyFunctionNameProvider $symfonyFunctionNameProvider)
    {
        $this->constantNodeFactory = $constantNodeFactory;
        $this->commonNodeFactory = $commonNodeFactory;
        $this->symfonyFunctionNameProvider = $symfonyFunctionNameProvider;
    }
    public function resolve(string $value, bool $skipServiceReference, bool $skipClassesToConstantReference) : \ConfigTransformer2021061810\PhpParser\Node\Expr
    {
        if ($value === '') {
            return new \ConfigTransformer2021061810\PhpParser\Node\Scalar\String_($value);
        }
        ...
        if (\strncmp($value, '@=', \strlen('@=')) === 0) {
            ...
            return new \ConfigTransformer2021061810\PhpParser\Node\Expr\FuncCall(new \ConfigTransformer2021061810\PhpParser\Node\Name\FullyQualified(\ConfigTransformer2021061810\Symplify\PhpConfigPrinter\ValueObject\FunctionName::EXPR), $args);
        }
    ...
    private function keepNewline(string $value) : \ConfigTransformer2021061810\PhpParser\Node\Scalar\String_
    {
        $string = new \ConfigTransformer2021061810\PhpParser\Node\Scalar\String_($value);
        $string->setAttribute(\ConfigTransformer2021061810\Symplify\Astral\ValueObject\AttributeKey::KIND, \ConfigTransformer2021061810\PhpParser\Node\Scalar\String_::KIND_DOUBLE_QUOTED);
        return $string;
    }
    private function isFilePath(string $value) : bool
    {
        return (bool) \ConfigTransformer2021061810\Nette\Utils\Strings::match($value, self::TWIG_HTML_XML_SUFFIX_REGEX);
    }
    /**
    ...
    private function resolveServiceReferenceExpr(string $value, bool $skipServiceReference, string $functionName) : \ConfigTransformer2021061810\PhpParser\Node\Expr
    {
        $value = \ltrim($value, '@');
        ...
        $args = [new \ConfigTransformer2021061810\PhpParser\Node\Arg($expr)];
        return new \ConfigTransformer2021061810\PhpParser\Node\Expr\FuncCall(new \ConfigTransformer2021061810\PhpParser\Node\Name\FullyQualified($functionName), $args);
    }
}
