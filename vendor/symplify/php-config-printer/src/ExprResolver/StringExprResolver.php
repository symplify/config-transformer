...
use ConfigTransformer202108236\PhpParser\BuilderHelpers;
use ConfigTransformer202108236\PhpParser\Node\Arg;
use ConfigTransformer202108236\PhpParser\Node\Expr;
use ConfigTransformer202108236\PhpParser\Node\Expr\ClassConstFetch;
...
use ConfigTransformer202108236\Symplify\Astral\ValueObject\AttributeKey;
use ConfigTransformer202108236\Symplify\PhpConfigPrinter\Configuration\SymfonyFunctionNameProvider;
use ConfigTransformer202108236\Symplify\PhpConfigPrinter\NodeFactory\CommonNodeFactory;
use ConfigTransformer202108236\Symplify\PhpConfigPrinter\NodeFactory\ConstantNodeFactory;
use ConfigTransformer202108236\Symplify\PhpConfigPrinter\ValueObject\FunctionName;
final class StringExprResolver
{
    ...
    public function __construct(\ConfigTransformer202108236\Symplify\PhpConfigPrinter\NodeFactory\ConstantNodeFactory $constantNodeFactory, \ConfigTransformer202108236\Symplify\PhpConfigPrinter\NodeFactory\CommonNodeFactory $commonNodeFactory, \ConfigTransformer202108236\Symplify\PhpConfigPrinter\Configuration\SymfonyFunctionNameProvider $symfonyFunctionNameProvider)
    {
        $this->constantNodeFactory = $constantNodeFactory;
        $this->commonNodeFactory = $commonNodeFactory;
        $this->symfonyFunctionNameProvider = $symfonyFunctionNameProvider;
    }
    public function resolve(string $value, bool $skipServiceReference, bool $skipClassesToConstantReference) : \ConfigTransformer202108236\PhpParser\Node\Expr
    {
        if ($value === '') {
            return new \ConfigTransformer202108236\PhpParser\Node\Scalar\String_($value);
        }
        ...
        if (\strncmp($value, '@=', \strlen('@=')) === 0) {
            ...
            return new \ConfigTransformer202108236\PhpParser\Node\Expr\FuncCall(new \ConfigTransformer202108236\PhpParser\Node\Name\FullyQualified(\ConfigTransformer202108236\Symplify\PhpConfigPrinter\ValueObject\FunctionName::EXPR), $args);
        }
        ...
        return \ConfigTransformer202108236\PhpParser\BuilderHelpers::normalizeValue($value);
    }
    private function keepNewline(string $value) : \ConfigTransformer202108236\PhpParser\Node\Scalar\String_
    {
        $string = new \ConfigTransformer202108236\PhpParser\Node\Scalar\String_($value);
        $string->setAttribute(\ConfigTransformer202108236\Symplify\Astral\ValueObject\AttributeKey::KIND, \ConfigTransformer202108236\PhpParser\Node\Scalar\String_::KIND_DOUBLE_QUOTED);
        return $string;
    }
    private function isFilePath(string $value) : bool
    {
        return (bool) \ConfigTransformer202108236\Nette\Utils\Strings::match($value, self::TWIG_HTML_XML_SUFFIX_REGEX);
    }
    /**
    ...
    private function resolveServiceReferenceExpr(string $value, bool $skipServiceReference, string $functionName) : \ConfigTransformer202108236\PhpParser\Node\Expr
    {
        $value = \ltrim($value, '@');
