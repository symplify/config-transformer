...
use ConfigTransformer202107154\PhpParser\Node\Expr\ClassConstFetch;
...
use ConfigTransformer202107154\Symplify\Astral\ValueObject\AttributeKey;
use ConfigTransformer202107154\Symplify\PhpConfigPrinter\Configuration\SymfonyFunctionNameProvider;
use ConfigTransformer202107154\Symplify\PhpConfigPrinter\NodeFactory\CommonNodeFactory;
use ConfigTransformer202107154\Symplify\PhpConfigPrinter\NodeFactory\ConstantNodeFactory;
...
final class StringExprResolver
{
    ...
    public function __construct(\ConfigTransformer202107154\Symplify\PhpConfigPrinter\NodeFactory\ConstantNodeFactory $constantNodeFactory, \ConfigTransformer202107154\Symplify\PhpConfigPrinter\NodeFactory\CommonNodeFactory $commonNodeFactory, \ConfigTransformer202107154\Symplify\PhpConfigPrinter\Configuration\SymfonyFunctionNameProvider $symfonyFunctionNameProvider)
    {
        $this->constantNodeFactory = $constantNodeFactory;
        $this->commonNodeFactory = $commonNodeFactory;
        $this->symfonyFunctionNameProvider = $symfonyFunctionNameProvider;
    }
    public function resolve(string $value, bool $skipServiceReference, bool $skipClassesToConstantReference) : \ConfigTransformer202107154\PhpParser\Node\Expr
    {
        if ($value === '') {
            return new \ConfigTransformer202107154\PhpParser\Node\Scalar\String_($value);
        }
        ...
        if (\strncmp($value, '@=', \strlen('@=')) === 0) {
            ...
            $args = [new \ConfigTransformer202107154\PhpParser\Node\Arg($expr)];
            return new \ConfigTransformer202107154\PhpParser\Node\Expr\FuncCall(new \ConfigTransformer202107154\PhpParser\Node\Name\FullyQualified(\ConfigTransformer202107154\Symplify\PhpConfigPrinter\ValueObject\FunctionName::EXPR), $args);
        }
        ...
        return \ConfigTransformer202107154\PhpParser\BuilderHelpers::normalizeValue($value);
    }
    private function keepNewline(string $value) : \ConfigTransformer202107154\PhpParser\Node\Scalar\String_
    {
        $string = new \ConfigTransformer202107154\PhpParser\Node\Scalar\String_($value);
        $string->setAttribute(\ConfigTransformer202107154\Symplify\Astral\ValueObject\AttributeKey::KIND, \ConfigTransformer202107154\PhpParser\Node\Scalar\String_::KIND_DOUBLE_QUOTED);
        return $string;
    }
    private function isFilePath(string $value) : bool
    {
        return (bool) \ConfigTransformer202107154\Nette\Utils\Strings::match($value, self::TWIG_HTML_XML_SUFFIX_REGEX);
    }
    /**
    ...
    private function resolveServiceReferenceExpr(string $value, bool $skipServiceReference, string $functionName) : \ConfigTransformer202107154\PhpParser\Node\Expr
    {
        $value = \ltrim($value, '@');
        ...
        $args = [new \ConfigTransformer202107154\PhpParser\Node\Arg($expr)];
        return new \ConfigTransformer202107154\PhpParser\Node\Expr\FuncCall(new \ConfigTransformer202107154\PhpParser\Node\Name\FullyQualified($functionName), $args);
    }
}
