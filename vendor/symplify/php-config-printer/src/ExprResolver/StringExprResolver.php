...
use ConfigTransformer2022022410\Symplify\Astral\ValueObject\AttributeKey;
use ConfigTransformer2022022410\Symplify\PhpConfigPrinter\NodeFactory\CommonNodeFactory;
use ConfigTransformer2022022410\Symplify\PhpConfigPrinter\NodeFactory\ConstantNodeFactory;
...
final class StringExprResolver
{
    ...
    public function __construct(\ConfigTransformer2022022410\Symplify\PhpConfigPrinter\NodeFactory\ConstantNodeFactory $constantNodeFactory, \ConfigTransformer2022022410\Symplify\PhpConfigPrinter\NodeFactory\CommonNodeFactory $commonNodeFactory)
    {
        $this->constantNodeFactory = $constantNodeFactory;
        $this->commonNodeFactory = $commonNodeFactory;
    }
    public function resolve(string $value, bool $skipServiceReference, bool $skipClassesToConstantReference) : \ConfigTransformer2022022410\PhpParser\Node\Expr
    {
        if ($value === '') {
            return new \ConfigTransformer2022022410\PhpParser\Node\Scalar\String_($value);
        }
        ...
        if (\strncmp($value, '@=', \strlen('@=')) === 0) {
            ...
            return new \ConfigTransformer2022022410\PhpParser\Node\Expr\FuncCall(new \ConfigTransformer2022022410\PhpParser\Node\Name\FullyQualified(\ConfigTransformer2022022410\Symplify\PhpConfigPrinter\ValueObject\FunctionName::EXPR), $args);
        }
        // is service reference
        if (\strncmp($value, '@', \strlen('@')) === 0 && !$this->isFilePath($value)) {
            return $this->resolveServiceReferenceExpr($value, $skipServiceReference, \ConfigTransformer2022022410\Symplify\PhpConfigPrinter\ValueObject\FunctionName::SERVICE);
        }
        return \ConfigTransformer2022022410\PhpParser\BuilderHelpers::normalizeValue($value);
    }
    private function keepNewline(string $value) : \ConfigTransformer2022022410\PhpParser\Node\Scalar\String_
    {
        $string = new \ConfigTransformer2022022410\PhpParser\Node\Scalar\String_($value);
        $string->setAttribute(\ConfigTransformer2022022410\Symplify\Astral\ValueObject\AttributeKey::KIND, \ConfigTransformer2022022410\PhpParser\Node\Scalar\String_::KIND_DOUBLE_QUOTED);
        return $string;
    }
    private function isFilePath(string $value) : bool
    {
        return (bool) \ConfigTransformer2022022410\Nette\Utils\Strings::match($value, self::TWIG_HTML_XML_SUFFIX_REGEX);
    }
    /**
    ...
    private function resolveServiceReferenceExpr(string $value, bool $skipServiceReference, string $functionName) : \ConfigTransformer2022022410\PhpParser\Node\Expr
    {
        $value = \ltrim($value, '@');
        ...
        $args = [new \ConfigTransformer2022022410\PhpParser\Node\Arg($expr)];
        return new \ConfigTransformer2022022410\PhpParser\Node\Expr\FuncCall(new \ConfigTransformer2022022410\PhpParser\Node\Name\FullyQualified($functionName), $args);
    }
}
