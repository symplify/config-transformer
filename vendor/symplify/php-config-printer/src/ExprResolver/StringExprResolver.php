...
use ConfigTransformerPrefix202605\Nette\Utils\Strings;
use ConfigTransformerPrefix202605\PhpParser\BuilderHelpers;
use ConfigTransformerPrefix202605\PhpParser\Node\Arg;
use ConfigTransformerPrefix202605\PhpParser\Node\Expr;
use ConfigTransformerPrefix202605\PhpParser\Node\Expr\ClassConstFetch;
use ConfigTransformerPrefix202605\PhpParser\Node\Expr\FuncCall;
...
final class StringExprResolver
{
    ...
    public function resolve(string $value, bool $skipServiceReference, bool $skipClassesToConstantReference, bool $isRoutingImport = \false) : Expr
    {
        ...
        if (\strncmp($value, '@', \strlen('@')) === 0 && !$this->isFilePath($value)) {
            return $this->resolveServiceReferenceExpr($value, $skipServiceReference, ReferenceFunctionNameResolver::resolve(), $isRoutingImport);
        }
