...
use ConfigTransformer202112193\Symplify\Astral\ValueObject\AttributeKey;
use ConfigTransformer202112193\Symplify\PhpConfigPrinter\NodeFactory\CommonNodeFactory;
use ConfigTransformer202112193\Symplify\PhpConfigPrinter\NodeFactory\ConstantNodeFactory;
...
final class StringExprResolver
{
    ...
    public function __construct(\ConfigTransformer202112193\Symplify\PhpConfigPrinter\NodeFactory\ConstantNodeFactory $constantNodeFactory, \ConfigTransformer202112193\Symplify\PhpConfigPrinter\NodeFactory\CommonNodeFactory $commonNodeFactory)
    {
        $this->constantNodeFactory = $constantNodeFactory;
        $this->commonNodeFactory = $commonNodeFactory;
    }
    public function resolve(string $value, bool $skipServiceReference, bool $skipClassesToConstantReference) : \ConfigTransformer202112193\PhpParser\Node\Expr
    {
        if ($value === '') {
            return new \ConfigTransformer202112193\PhpParser\Node\Scalar\String_($value);
        }
        ...
        if (\strncmp($value, '@=', \strlen('@=')) === 0) {
            ...
            return new \ConfigTransformer202112193\PhpParser\Node\Expr\FuncCall(new \ConfigTransformer202112193\PhpParser\Node\Name\FullyQualified(\ConfigTransformer202112193\Symplify\PhpConfigPrinter\ValueObject\FunctionName::EXPR), $args);
        }
        // is service reference
        if (\strncmp($value, '@', \strlen('@')) === 0 && !$this->isFilePath($value)) {
            return $this->resolveServiceReferenceExpr($value, $skipServiceReference, \ConfigTransformer202112193\Symplify\PhpConfigPrinter\ValueObject\FunctionName::SERVICE);
        }
        return \ConfigTransformer202112193\PhpParser\BuilderHelpers::normalizeValue($value);
    }
    private function keepNewline(string $value) : \ConfigTransformer202112193\PhpParser\Node\Scalar\String_
    {
        $string = new \ConfigTransformer202112193\PhpParser\Node\Scalar\String_($value);
        $string->setAttribute(\ConfigTransformer202112193\Symplify\Astral\ValueObject\AttributeKey::KIND, \ConfigTransformer202112193\PhpParser\Node\Scalar\String_::KIND_DOUBLE_QUOTED);
        return $string;
    }
    private function isFilePath(string $value) : bool
    {
        return (bool) \ConfigTransformer202112193\Nette\Utils\Strings::match($value, self::TWIG_HTML_XML_SUFFIX_REGEX);
    }
    /**
    ...
    private function resolveServiceReferenceExpr(string $value, bool $skipServiceReference, string $functionName) : \ConfigTransformer202112193\PhpParser\Node\Expr
    {
        $value = \ltrim($value, '@');
