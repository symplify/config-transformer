...
namespace ConfigTransformer2022052210\Symplify\PhpConfigPrinter\PhpParser\NodeFactory;

use ConfigTransformer2022052210\PhpParser\Node\Arg;
use ConfigTransformer2022052210\PhpParser\Node\Expr;
use ConfigTransformer2022052210\PhpParser\Node\Expr\Array_;
use ConfigTransformer2022052210\PhpParser\Node\Expr\ArrayItem;
...
use ConfigTransformer2022052210\Symplify\Astral\NodeValue\NodeValueResolver;
use ConfigTransformer2022052210\Symplify\PhpConfigPrinter\Naming\VariableNameResolver;
...
final class ConfiguratorClosureNodeFactory
{
     ...
     * @param Stmt[] $stmts
     */
    public function createContainerClosureFromStmts(array $stmts, string $containerConfiguratorClass) : \ConfigTransformer2022052210\PhpParser\Node\Expr\Closure
    {
     ...
     * @param Stmt[] $stmts
     */
    public function createRoutingClosureFromStmts(array $stmts) : \ConfigTransformer2022052210\PhpParser\Node\Expr\Closure
    {
        $param = $this->createRoutingConfiguratorParam();
        return $this->createClosureFromParamAndStmts($param, $stmts);
    }
    private function createContainerConfiguratorParam(string $containerConfiguratorClass) : \ConfigTransformer2022052210\PhpParser\Node\Param
    {
        $variableName = $this->variableNameResolver->resolveFromType($containerConfiguratorClass);
        $containerConfiguratorVariable = new \ConfigTransformer2022052210\PhpParser\Node\Expr\Variable($variableName);
        $fullyQualified = new \ConfigTransformer2022052210\PhpParser\Node\Name\FullyQualified($containerConfiguratorClass);
        return new \ConfigTransformer2022052210\PhpParser\Node\Param($containerConfiguratorVariable, null, $fullyQualified);
    }
    private function createRoutingConfiguratorParam() : \ConfigTransformer2022052210\PhpParser\Node\Param
    {
        $containerConfiguratorVariable = new \ConfigTransformer2022052210\PhpParser\Node\Expr\Variable(\ConfigTransformer2022052210\Symplify\PhpConfigPrinter\ValueObject\VariableName::ROUTING_CONFIGURATOR);
        // @note must be string to avoid prefixing class
        $classNameFullyQualified = new \ConfigTransformer2022052210\PhpParser\Node\Name\FullyQualified('ConfigTransformer2022052210\\Symfony\\Component\\Routing\\Loader\\Configurator\\RoutingConfigurator');
        return new \ConfigTransformer2022052210\PhpParser\Node\Param($containerConfiguratorVariable, null, $classNameFullyQualified);
    }
    /**
     * @param Stmt[] $stmts
     */
    private function createClosureFromParamAndStmts(\ConfigTransformer2022052210\PhpParser\Node\Param $param, array $stmts) : \ConfigTransformer2022052210\PhpParser\Node\Expr\Closure
    {
        $stmts = $this->mergeStmtsFromSameClosure($stmts);
        $closure = new \ConfigTransformer2022052210\PhpParser\Node\Expr\Closure(['params' => [$param], 'stmts' => $stmts, 'static' => \true]);
        $closure->returnType = new \ConfigTransformer2022052210\PhpParser\Node\Identifier('void');
        return $closure;
    }
    /**
    ...
    private function mergeStmtsFromSameClosure(array $stmts) : array
    {
        ...
        foreach ($stmts as $stmtKey => $stmt) {
            ...
            $stmt = $stmt->expr;
            if (!$stmt instanceof \ConfigTransformer2022052210\PhpParser\Node\Expr\MethodCall) {
                continue;
            }
            ...
            if (!$secondArgOrVariadicPlaceholder instanceof \ConfigTransformer2022052210\PhpParser\Node\Arg) {
                continue;
            }
    ...
    private function replaceArrayArgWithMergedArrayItems(array $extensionNodesByExtensionName, array $stmts) : array
    {
        foreach ($extensionNodesByExtensionName as $extensionNodes) {
            ...
            if (!$expression instanceof \ConfigTransformer2022052210\PhpParser\Node\Stmt\Expression) {
                continue;
            }
            $methodCall = $expression->expr;
            if (!$methodCall instanceof \ConfigTransformer2022052210\PhpParser\Node\Expr\MethodCall) {
                continue;
            }
            $array = new \ConfigTransformer2022052210\PhpParser\Node\Expr\Array_($newArrayItems);
            $methodCall->args[1] = new \ConfigTransformer2022052210\PhpParser\Node\Arg($array);
        }
        return $stmts;
    }
    /**
    ...
    private function resolveMergedArrayItems(array $extensionExprs) : array
    {
        ...
        foreach ($extensionExprs as $extensionExpr) {
            foreach ($extensionExpr as $singleExtensionExpr) {
                if (!$singleExtensionExpr instanceof \ConfigTransformer2022052210\PhpParser\Node\Expr\Array_) {
                    continue;
                }
    ...
    private function matchExtensionName(\ConfigTransformer2022052210\PhpParser\Node\Expr\MethodCall $methodCall) : ?string
    {
        if (!$this->simpleNameResolver->isName($methodCall->name, 'extension')) {
            return null;
        }
        $firstArg = $methodCall->args[0];
        if (!$firstArg instanceof \ConfigTransformer2022052210\PhpParser\Node\Arg) {
            return null;
        }
