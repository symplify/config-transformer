...
namespace ConfigTransformer202106114\Symplify\PhpConfigPrinter\NodeTraverser;
...
use ConfigTransformer202106114\PhpParser\NodeTraverser;
use ConfigTransformer202106114\Symplify\PhpConfigPrinter\NodeVisitor\ImportFullyQualifiedNamesNodeVisitor;
final class ImportFullyQualifiedNamesNodeTraverser
{
    ...
    public function __construct(\ConfigTransformer202106114\Symplify\PhpConfigPrinter\NodeVisitor\ImportFullyQualifiedNamesNodeVisitor $importFullyQualifiedNamesNodeVisitor, \ConfigTransformer202106114\PhpParser\BuilderFactory $builderFactory)
    {
    ...
    private function addUseImportsToNamespace(array $nodes, array $nameImports) : array
    {
        ...
        $useImports[] = new \ConfigTransformer202106114\PhpParser\Node\Stmt\Nop();
    ...
    {
        $nodeTraverser = new \ConfigTransformer202106114\PhpParser\NodeTraverser();
    ...
    private function createUses(array $nameImports) : array
    {
        ...
        foreach ($nameImports as $nameImport) {
            $shortNameImport = \ConfigTransformer202106114\Nette\Utils\Strings::after($nameImport, '\\', -1);
            if (\function_exists($nameImport) || $shortNameImport === 'ref') {
                $useBuilder = $this->builderFactory->useFunction(new \ConfigTransformer202106114\PhpParser\Node\Name($nameImport));
                /** @var Use_ $use */
                $use = $useBuilder->getNode();
                $useImports[] = $use;
            } else {
                $useBuilder = $this->builderFactory->use(new \ConfigTransformer202106114\PhpParser\Node\Name($nameImport));
                /** @var Use_ $use */
                $use = $useBuilder->getNode();
                $useImports[] = $use;
            }
