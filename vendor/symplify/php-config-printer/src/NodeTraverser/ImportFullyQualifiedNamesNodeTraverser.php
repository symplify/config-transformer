...
namespace ConfigTransformer2021061810\Symplify\PhpConfigPrinter\NodeTraverser;
...
use ConfigTransformer2021061810\Symplify\PhpConfigPrinter\NodeVisitor\ImportFullyQualifiedNamesNodeVisitor;
final class ImportFullyQualifiedNamesNodeTraverser
{
    ...
    public function __construct(\ConfigTransformer2021061810\Symplify\PhpConfigPrinter\NodeVisitor\ImportFullyQualifiedNamesNodeVisitor $importFullyQualifiedNamesNodeVisitor, \ConfigTransformer2021061810\PhpParser\BuilderFactory $builderFactory)
    {
    ...
    private function addUseImportsToNamespace(array $nodes, array $nameImports) : array
    {
        ...
        $useImports[] = new \ConfigTransformer2021061810\PhpParser\Node\Stmt\Nop();
    ...
    private function createUses(array $nameImports) : array
    {
        ...
        foreach ($nameImports as $nameImport) {
            $shortNameImport = \ConfigTransformer2021061810\Nette\Utils\Strings::after($nameImport, '\\', -1);
            if (\function_exists($nameImport) || $shortNameImport === 'ref') {
                $useBuilder = $this->builderFactory->useFunction(new \ConfigTransformer2021061810\PhpParser\Node\Name($nameImport));
                /** @var Use_ $use */
                $use = $useBuilder->getNode();
                $useImports[] = $use;
            } else {
                $useBuilder = $this->builderFactory->use(new \ConfigTransformer2021061810\PhpParser\Node\Name($nameImport));
                /** @var Use_ $use */
                $use = $useBuilder->getNode();
                $useImports[] = $use;
            }
