...
namespace ConfigTransformer202112191\Symplify\PhpConfigPrinter\NodeTraverser;
...
use ConfigTransformer202112191\Symplify\PhpConfigPrinter\NodeVisitor\ImportFullyQualifiedNamesNodeVisitor;
use ConfigTransformer202112191\Symplify\PhpConfigPrinter\Sorter\FullyQualifiedImportSorter;
...
final class ImportFullyQualifiedNamesNodeTraverser
{
    ...
    public function __construct(\ConfigTransformer202112191\PhpParser\NodeVisitor\ParentConnectingVisitor $parentConnectingVisitor, \ConfigTransformer202112191\Symplify\PhpConfigPrinter\NodeVisitor\ImportFullyQualifiedNamesNodeVisitor $importFullyQualifiedNamesNodeVisitor, \ConfigTransformer202112191\Symplify\PhpConfigPrinter\Sorter\FullyQualifiedImportSorter $fullyQualifiedImportSorter, \ConfigTransformer202112191\PhpParser\BuilderFactory $builderFactory)
    {
    ...
    private function addUseImportsToNamespace(array $nodes, array $imports) : array
    {
        ...
        return \array_merge($useImports, [new \ConfigTransformer202112191\PhpParser\Node\Stmt\Nop()], $nodes);
    }
    /**
     * @param Node[] $nodes
     */
    ...
    private function createUses(array $imports) : array
    {
        ...
        foreach ($imports as $import) {
            $name = new \ConfigTransformer202112191\PhpParser\Node\Name($import->getFullyQualified());
            switch ($import->getType()) {
                case \ConfigTransformer202112191\Symplify\PhpConfigPrinter\ValueObject\ImportType::FUNCTION_TYPE:
                    $useBuilder = $this->builderFactory->useFunction($name);
                    break;
                case \ConfigTransformer202112191\Symplify\PhpConfigPrinter\ValueObject\ImportType::CONSTANT_TYPE:
