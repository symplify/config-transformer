...
use ConfigTransformer202202134\locale;
...
use ConfigTransformer202202134\Symplify\PackageBuilder\Strings\StringFormatConverter;
...
final class ImportRoutingCaseConverter implements \ConfigTransformer202202134\Symplify\PhpConfigPrinter\Contract\RoutingCaseConverterInterface
{
    /**
    ...
    public function __construct(\ConfigTransformer202202134\Symplify\PhpConfigPrinter\NodeFactory\ArgsNodeFactory $argsNodeFactory)
    {
        ...
        $this->stringFormatConverter = new \ConfigTransformer202202134\Symplify\PackageBuilder\Strings\StringFormatConverter();
    }
    /**
     * @param mixed[] $values
     */
    ...
    public function convertToMethodCall(string $key, $values) : \ConfigTransformer202202134\PhpParser\Node\Stmt\Expression
    {
        ...
        $methodCall = new \ConfigTransformer202202134\PhpParser\Node\Expr\MethodCall($variable, 'import', $args);
        ...
        if (isset($values[self::PREFIX])) {
            ...
            $methodCall = new \ConfigTransformer202202134\PhpParser\Node\Expr\MethodCall($methodCall, self::PREFIX, $args);
        }
        foreach (self::NESTED_KEYS as $nestedKey) {
            ...
            $methodCall = new \ConfigTransformer202202134\PhpParser\Node\Expr\MethodCall($methodCall, $name, $args);
        }
