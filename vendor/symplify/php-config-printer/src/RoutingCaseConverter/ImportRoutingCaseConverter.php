...
use ConfigTransformer202111205\locale;
...
use ConfigTransformer202111205\Symplify\PackageBuilder\Strings\StringFormatConverter;
...
final class ImportRoutingCaseConverter implements \ConfigTransformer202111205\Symplify\PhpConfigPrinter\Contract\RoutingCaseConverterInterface
{
    /**
    ...
    public function __construct(\ConfigTransformer202111205\Symplify\PhpConfigPrinter\NodeFactory\ArgsNodeFactory $argsNodeFactory)
    {
        ...
        $this->stringFormatConverter = new \ConfigTransformer202111205\Symplify\PackageBuilder\Strings\StringFormatConverter();
    }
    /**
     * @param mixed[] $values
     * @param string $key
    ...
    public function convertToMethodCall($key, $values) : \ConfigTransformer202111205\PhpParser\Node\Stmt\Expression
    {
        ...
        $methodCall = new \ConfigTransformer202111205\PhpParser\Node\Expr\MethodCall($variable, 'import', $args);
        ...
        if (isset($values[self::PREFIX])) {
            ...
            $methodCall = new \ConfigTransformer202111205\PhpParser\Node\Expr\MethodCall($methodCall, self::PREFIX, $args);
        }
        foreach (self::NESTED_KEYS as $nestedKey) {
            ...
            $methodCall = new \ConfigTransformer202111205\PhpParser\Node\Expr\MethodCall($methodCall, $name, $args);
        }
