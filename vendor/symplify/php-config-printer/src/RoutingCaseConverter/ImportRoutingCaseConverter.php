...
use ConfigTransformer202204142\Symplify\PackageBuilder\Strings\StringFormatConverter;
...
final class ImportRoutingCaseConverter implements \ConfigTransformer202204142\Symplify\PhpConfigPrinter\Contract\RoutingCaseConverterInterface
{
    /**
     * @var string[]
     */
    ...
    public function __construct(\ConfigTransformer202204142\Symplify\PhpConfigPrinter\NodeFactory\ArgsNodeFactory $argsNodeFactory)
    {
        ...
        $this->stringFormatConverter = new \ConfigTransformer202204142\Symplify\PackageBuilder\Strings\StringFormatConverter();
    }
    /**
     * @param mixed $values
     */
    ...
    public function convertToMethodCall(string $key, $values) : \ConfigTransformer202204142\PhpParser\Node\Stmt\Expression
    {
        ...
        $methodCall = new \ConfigTransformer202204142\PhpParser\Node\Expr\MethodCall($variable, 'import', $args);
        ...
        if (isset($values[self::PREFIX])) {
            ...
            $methodCall = new \ConfigTransformer202204142\PhpParser\Node\Expr\MethodCall($methodCall, self::PREFIX, $args);
        }
        foreach (self::NESTED_KEYS as $nestedKey) {
            ...
            $methodCall = new \ConfigTransformer202204142\PhpParser\Node\Expr\MethodCall($methodCall, $name, $args);
        }
