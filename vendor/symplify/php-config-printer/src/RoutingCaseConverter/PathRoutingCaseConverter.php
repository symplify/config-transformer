...
final class PathRoutingCaseConverter implements \ConfigTransformer2021092210\Symplify\PhpConfigPrinter\Contract\RoutingCaseConverterInterface
{
    /**
     * @var string[]
     */
    ...
    public function convertToMethodCall($key, $values) : \ConfigTransformer2021092210\PhpParser\Node\Stmt\Expression
    {
        ...
        $methodCall = new \ConfigTransformer2021092210\PhpParser\Node\Expr\MethodCall($variable, 'add', $args);
        foreach (self::NESTED_KEYS as $nestedKey) {
            ...
            $methodCall = new \ConfigTransformer2021092210\PhpParser\Node\Expr\MethodCall($methodCall, $nestedKey, $args);
        }
