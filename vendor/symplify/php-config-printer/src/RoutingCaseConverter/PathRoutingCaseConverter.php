...
namespace ConfigTransformer202204179\Symplify\PhpConfigPrinter\RoutingCaseConverter;
...
use ConfigTransformer202204179\Symplify\PhpConfigPrinter\Contract\RoutingCaseConverterInterface;
...
final class PathRoutingCaseConverter implements \ConfigTransformer202204179\Symplify\PhpConfigPrinter\Contract\RoutingCaseConverterInterface
{
    /**
     * @var string[]
     */
     ...
     * @param mixed $values
     */
    public function convertToMethodCall(string $key, $values) : \ConfigTransformer202204179\PhpParser\Node\Stmt\Expression
    {
        $variable = new \ConfigTransformer202204179\PhpParser\Node\Expr\Variable(\ConfigTransformer202204179\Symplify\PhpConfigPrinter\ValueObject\VariableName::ROUTING_CONFIGURATOR);
        // @todo args
        $args = $this->createAddArgs($key, $values);
        $methodCall = new \ConfigTransformer202204179\PhpParser\Node\Expr\MethodCall($variable, 'add', $args);
        foreach (self::NESTED_KEYS as $nestedKey) {
            ...
            $methodCall = new \ConfigTransformer202204179\PhpParser\Node\Expr\MethodCall($methodCall, $nestedKey, $args);
        }
