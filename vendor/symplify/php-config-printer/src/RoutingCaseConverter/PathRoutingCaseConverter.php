...
namespace ConfigTransformer2022020510\Symplify\PhpConfigPrinter\RoutingCaseConverter;
...
use ConfigTransformer2022020510\Symplify\PhpConfigPrinter\Contract\RoutingCaseConverterInterface;
...
final class PathRoutingCaseConverter implements \ConfigTransformer2022020510\Symplify\PhpConfigPrinter\Contract\RoutingCaseConverterInterface
{
    /**
     * @var string[]
     */
     ...
     * @param mixed $values
     */
    public function convertToMethodCall(string $key, $values) : \ConfigTransformer2022020510\PhpParser\Node\Stmt\Expression
    {
        $variable = new \ConfigTransformer2022020510\PhpParser\Node\Expr\Variable(\ConfigTransformer2022020510\Symplify\PhpConfigPrinter\ValueObject\VariableName::ROUTING_CONFIGURATOR);
        // @todo args
        $args = $this->createAddArgs($key, $values);
        $methodCall = new \ConfigTransformer2022020510\PhpParser\Node\Expr\MethodCall($variable, 'add', $args);
        foreach (self::NESTED_KEYS as $nestedKey) {
            ...
            $methodCall = new \ConfigTransformer2022020510\PhpParser\Node\Expr\MethodCall($methodCall, $nestedKey, $args);
        }
