...
final class SharedPublicServiceOptionKeyYamlToPhpFactory implements \ConfigTransformer202107055\Symplify\PhpConfigPrinter\Contract\Converter\ServiceOptionsKeyYamlToPhpFactoryInterface
{
    /**
    ...
    public function decorateServiceMethodCall($key, $yaml, $values, \ConfigTransformer202107055\PhpParser\Node\Expr\MethodCall $methodCall) : \ConfigTransformer202107055\PhpParser\Node\Expr\MethodCall
    {
        ...
        if ($key === 'shared') {
            if ($yaml === \false) {
                return new \ConfigTransformer202107055\PhpParser\Node\Expr\MethodCall($methodCall, 'share', [new \ConfigTransformer202107055\PhpParser\Node\Arg($this->commonNodeFactory->createFalse())]);
            }
            return new \ConfigTransformer202107055\PhpParser\Node\Expr\MethodCall($methodCall, 'share');
        }
