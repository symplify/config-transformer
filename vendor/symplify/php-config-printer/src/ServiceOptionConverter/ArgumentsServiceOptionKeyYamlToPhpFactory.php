...
final class ArgumentsServiceOptionKeyYamlToPhpFactory implements \ConfigTransformer202201158\Symplify\PhpConfigPrinter\Contract\Converter\ServiceOptionsKeyYamlToPhpFactoryInterface
{
    /**
    ...
    public function __construct(\ConfigTransformer202201158\Symplify\PhpConfigPrinter\NodeFactory\ArgsNodeFactory $argsNodeFactory, \ConfigTransformer202201158\Symplify\PhpConfigPrinter\ServiceOptionAnalyzer\ServiceOptionAnalyzer $serviceOptionAnalyzer)
    {
    ...
    public function decorateServiceMethodCall($key, $yaml, $values, \ConfigTransformer202201158\PhpParser\Node\Expr\MethodCall $methodCall) : \ConfigTransformer202201158\PhpParser\Node\Expr\MethodCall
    {
        if (!$this->serviceOptionAnalyzer->hasNamedArguments($yaml)) {
            ...
            return new \ConfigTransformer202201158\PhpParser\Node\Expr\MethodCall($methodCall, 'args', $args);
        }
        foreach ($yaml as $key => $value) {
            $args = $this->argsNodeFactory->createFromValues([$key, $value], \false, \true);
            $methodCall = new \ConfigTransformer202201158\PhpParser\Node\Expr\MethodCall($methodCall, 'arg', $args);
        }
        return $methodCall;
    }
    /**
    ...
    {
        return $key === \ConfigTransformer202201158\Symplify\PhpConfigPrinter\ValueObject\YamlServiceKey::ARGUMENTS;
    }
}
