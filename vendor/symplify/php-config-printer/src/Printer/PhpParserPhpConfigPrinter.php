...
final class PhpParserPhpConfigPrinter extends \ConfigTransformer202206075\PhpParser\PrettyPrinter\Standard
{
    /**
    ...
    public function __construct(\ConfigTransformer202206075\Symplify\PhpConfigPrinter\NodeTraverser\ImportFullyQualifiedNamesNodeTraverser $importFullyQualifiedNamesNodeTraverser, \ConfigTransformer202206075\Symplify\PhpConfigPrinter\Printer\NodeDecorator\EmptyLineNodeDecorator $emptyLineNodeDecorator)
    {
    ...
    public function prettyPrintFile(array $stmts) : string
    {
        ...
        $printedContent = \ConfigTransformer202206075\Nette\Utils\Strings::replace($printedContent, self::START_WITH_SPACE_REGEX, "\n");
        // remove space before " :" in main closure
        $printedContent = \ConfigTransformer202206075\Nette\Utils\Strings::replace($printedContent, self::VOID_AFTER_FUNC_REGEX, '): void');
        // remove space between declare strict types
        $printedContent = \ConfigTransformer202206075\Nette\Utils\Strings::replace($printedContent, self::DECLARE_SPACE_STRICT_REGEX, 'declare(strict');
    ...
    {
        return "'" . \ConfigTransformer202206075\Nette\Utils\Strings::replace($string, self::QUOTE_SLASH_REGEX, '\\\\$0') . "'";
    }
    protected function pExpr_Array(\ConfigTransformer202206075\PhpParser\Node\Expr\Array_ $array) : string
    {
        $array->setAttribute(self::KIND, \ConfigTransformer202206075\PhpParser\Node\Expr\Array_::KIND_SHORT);
        return parent::pExpr_Array($array);
    }
    protected function pExpr_MethodCall(\ConfigTransformer202206075\PhpParser\Node\Expr\MethodCall $methodCall) : string
    {
    ...
    {
        $nextCallIndentReplacement = ')' . \PHP_EOL . \ConfigTransformer202206075\Nette\Utils\Strings::indent('->', 8, ' ');
        return \ConfigTransformer202206075\Nette\Utils\Strings::replace($content, '#\\)->#', $nextCallIndentReplacement);
    }
    /**
     * @param Node[] $stmts
     * @return Node[]
    ...
    private function prependStrictTypesDeclare(array $stmts) : array
    {
        ...
        return \array_merge([$strictTypesDeclare, new \ConfigTransformer202206075\PhpParser\Node\Stmt\Nop()], $stmts);
    }
    private function createStrictTypesDeclare() : \ConfigTransformer202206075\PhpParser\Node\Stmt\Declare_
    {
        $declareDeclare = new \ConfigTransformer202206075\PhpParser\Node\Stmt\DeclareDeclare('strict_types', new \ConfigTransformer202206075\PhpParser\Node\Scalar\LNumber(1));
        return new \ConfigTransformer202206075\PhpParser\Node\Stmt\Declare_([$declareDeclare]);
    }
}
