...
namespace ConfigTransformer202201248\Symplify\PhpConfigPrinter\Printer\ArrayDecorator;
...
use ConfigTransformer202201248\PhpParser\Node\Expr\New_;
use ConfigTransformer202201248\PhpParser\Node\Expr\StaticCall;
use ConfigTransformer202201248\PhpParser\Node\Name\FullyQualified;
use ConfigTransformer202201248\Symplify\PhpConfigPrinter\NodeFactory\NewValueObjectFactory;
use ConfigTransformer202201248\Symplify\PhpConfigPrinter\Reflection\ConstantNameFromValueResolver;
use ConfigTransformer202201248\Symplify\SymfonyPhpConfig\ValueObjectInliner;
final class ServiceConfigurationDecorator
{
    ...
    public function __construct(\ConfigTransformer202201248\Symplify\PhpConfigPrinter\Reflection\ConstantNameFromValueResolver $constantNameFromValueResolver, \ConfigTransformer202201248\Symplify\PhpConfigPrinter\NodeFactory\NewValueObjectFactory $newValueObjectFactory)
    {
    ...
    private function decorateValueObject($value, bool $shouldUseConfigureMethod)
    {
        ...
        $args = [new \ConfigTransformer202201248\PhpParser\Node\Arg($new)];
    ...
    private function decorateValueObjects(array $values, bool $shouldUseConfigureMethod)
    {
        ...
        foreach ($values as $value) {
            $new = $this->newValueObjectFactory->create($value);
            $arrayItems[] = new \ConfigTransformer202201248\PhpParser\Node\Expr\ArrayItem($new);
        }
        $array = new \ConfigTransformer202201248\PhpParser\Node\Expr\Array_($arrayItems);
        if ($shouldUseConfigureMethod) {
            return $array;
        }
        $args = [new \ConfigTransformer202201248\PhpParser\Node\Arg($array)];
        return $this->createInlineStaticCall($args);
    }
    /**
     * @param mixed $values
     ...
     * @param Arg[] $args
     */
    private function createInlineStaticCall(array $args) : \ConfigTransformer202201248\PhpParser\Node\Expr\StaticCall
    {
        $fullyQualified = new \ConfigTransformer202201248\PhpParser\Node\Name\FullyQualified(\ConfigTransformer202201248\Symplify\SymfonyPhpConfig\ValueObjectInliner::class);
        return new \ConfigTransformer202201248\PhpParser\Node\Expr\StaticCall($fullyQualified, 'inline', $args);
    }
}
