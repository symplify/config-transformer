...
namespace ConfigTransformer2021110410\Symplify\PhpConfigPrinter\Printer\ArrayDecorator;
...
use ConfigTransformer2021110410\PhpParser\Node\Expr\StaticCall;
use ConfigTransformer2021110410\PhpParser\Node\Name\FullyQualified;
use ConfigTransformer2021110410\Symplify\PhpConfigPrinter\NodeFactory\NewValueObjectFactory;
use ConfigTransformer2021110410\Symplify\PhpConfigPrinter\Reflection\ConstantNameFromValueResolver;
use ConfigTransformer2021110410\Symplify\SymfonyPhpConfig\ValueObjectInliner;
final class ServiceConfigurationDecorator
{
    ...
    public function __construct(\ConfigTransformer2021110410\Symplify\PhpConfigPrinter\Reflection\ConstantNameFromValueResolver $constantNameFromValueResolver, \ConfigTransformer2021110410\Symplify\PhpConfigPrinter\NodeFactory\NewValueObjectFactory $newValueObjectFactory)
    {
     ...
     * @param object $value
     */
    private function decorateValueObject($value) : \ConfigTransformer2021110410\PhpParser\Node\Expr\StaticCall
    {
        $new = $this->newValueObjectFactory->create($value);
        $args = [new \ConfigTransformer2021110410\PhpParser\Node\Arg($new)];
        return $this->createInlineStaticCall($args);
    }
    /**
     * @param mixed[] $values
     */
    private function decorateValueObjects(array $values) : \ConfigTransformer2021110410\PhpParser\Node\Expr\StaticCall
    {
        $arrayItems = [];
        foreach ($values as $value) {
            $new = $this->newValueObjectFactory->create($value);
            $arrayItems[] = new \ConfigTransformer2021110410\PhpParser\Node\Expr\ArrayItem($new);
        }
        $array = new \ConfigTransformer2021110410\PhpParser\Node\Expr\Array_($arrayItems);
        $args = [new \ConfigTransformer2021110410\PhpParser\Node\Arg($array)];
        return $this->createInlineStaticCall($args);
    }
    /**
     * @param mixed $values
     ...
     * @param Arg[] $args
     */
    private function createInlineStaticCall(array $args) : \ConfigTransformer2021110410\PhpParser\Node\Expr\StaticCall
    {
        $fullyQualified = new \ConfigTransformer2021110410\PhpParser\Node\Name\FullyQualified(\ConfigTransformer2021110410\Symplify\SymfonyPhpConfig\ValueObjectInliner::class);
        return new \ConfigTransformer2021110410\PhpParser\Node\Expr\StaticCall($fullyQualified, 'inline', $args);
    }
}
