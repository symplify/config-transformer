...
namespace ConfigTransformer202111241\Symplify\PhpConfigPrinter\Printer\ArrayDecorator;
...
use ConfigTransformer202111241\PhpParser\Node\Expr\StaticCall;
use ConfigTransformer202111241\PhpParser\Node\Name\FullyQualified;
use ConfigTransformer202111241\Symplify\PhpConfigPrinter\NodeFactory\NewValueObjectFactory;
use ConfigTransformer202111241\Symplify\PhpConfigPrinter\Reflection\ConstantNameFromValueResolver;
use ConfigTransformer202111241\Symplify\SymfonyPhpConfig\ValueObjectInliner;
final class ServiceConfigurationDecorator
{
    ...
    public function __construct(\ConfigTransformer202111241\Symplify\PhpConfigPrinter\Reflection\ConstantNameFromValueResolver $constantNameFromValueResolver, \ConfigTransformer202111241\Symplify\PhpConfigPrinter\NodeFactory\NewValueObjectFactory $newValueObjectFactory)
    {
     ...
     * @param object $value
     */
    private function decorateValueObject($value) : \ConfigTransformer202111241\PhpParser\Node\Expr\StaticCall
    {
        $new = $this->newValueObjectFactory->create($value);
        $args = [new \ConfigTransformer202111241\PhpParser\Node\Arg($new)];
        return $this->createInlineStaticCall($args);
    }
    /**
     * @param mixed[] $values
     */
    private function decorateValueObjects(array $values) : \ConfigTransformer202111241\PhpParser\Node\Expr\StaticCall
    {
        $arrayItems = [];
        foreach ($values as $value) {
            $new = $this->newValueObjectFactory->create($value);
            $arrayItems[] = new \ConfigTransformer202111241\PhpParser\Node\Expr\ArrayItem($new);
        }
        $array = new \ConfigTransformer202111241\PhpParser\Node\Expr\Array_($arrayItems);
        $args = [new \ConfigTransformer202111241\PhpParser\Node\Arg($array)];
        return $this->createInlineStaticCall($args);
    }
    /**
     * @param mixed $values
     ...
     * @param Arg[] $args
     */
    private function createInlineStaticCall(array $args) : \ConfigTransformer202111241\PhpParser\Node\Expr\StaticCall
    {
        $fullyQualified = new \ConfigTransformer202111241\PhpParser\Node\Name\FullyQualified(\ConfigTransformer202111241\Symplify\SymfonyPhpConfig\ValueObjectInliner::class);
        return new \ConfigTransformer202111241\PhpParser\Node\Expr\StaticCall($fullyQualified, 'inline', $args);
    }
}
