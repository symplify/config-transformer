 ...
 */
final class AutowireArrayParameterCompilerPass implements \ConfigTransformer202206075\Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface
{
    /**
    ...
    {
        $this->definitionFinder = new \ConfigTransformer202206075\Symplify\AutowireArrayParameter\DependencyInjection\DefinitionFinder();
        $paramTypeDocBlockResolver = new \ConfigTransformer202206075\Symplify\AutowireArrayParameter\DocBlock\ParamTypeDocBlockResolver();
        $this->parameterTypeResolver = new \ConfigTransformer202206075\Symplify\AutowireArrayParameter\TypeResolver\ParameterTypeResolver($paramTypeDocBlockResolver);
        $this->parameterSkipper = new \ConfigTransformer202206075\Symplify\AutowireArrayParameter\Skipper\ParameterSkipper($this->parameterTypeResolver, $excludedFatalClasses);
    }
    ...
    private function shouldSkipDefinition(\ConfigTransformer202206075\Symfony\Component\DependencyInjection\ContainerBuilder $containerBuilder, \ConfigTransformer202206075\Symfony\Component\DependencyInjection\Definition $definition) : bool
    {
        ...
        if (\ConfigTransformer202206075\Nette\Utils\Strings::match($resolvedClassName, $excludedNamespacePattern)) {
            return \true;
        }
        ...
        if (!$reflectionClass->hasMethod(\ConfigTransformer202206075\Symplify\PackageBuilder\ValueObject\MethodName::CONSTRUCTOR)) {
            return \true;
        }
        /** @var ReflectionMethod $constructorReflectionMethod */
        $constructorReflectionMethod = $reflectionClass->getConstructor();
    ...
    private function processParameters(\ConfigTransformer202206075\Symfony\Component\DependencyInjection\ContainerBuilder $containerBuilder, \ReflectionMethod $reflectionMethod, \ConfigTransformer202206075\Symfony\Component\DependencyInjection\Definition $definition) : void
    {
    ...
    private function createReferencesFromDefinitions(array $definitions) : array
    {
        ...
        foreach ($definitionOfTypeNames as $definitionOfTypeName) {
            $references[] = new \ConfigTransformer202206075\Symfony\Component\DependencyInjection\Reference($definitionOfTypeName);
        }
        return $references;
    }
}
