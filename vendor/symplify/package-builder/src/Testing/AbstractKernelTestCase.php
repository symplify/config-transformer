...
namespace ConfigTransformer202206079\Symplify\PackageBuilder\Testing;
...
use ConfigTransformer202206079\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface;
use ConfigTransformer202206079\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException;
 ...
 */
abstract class AbstractKernelTestCase extends \ConfigTransformer202206079\PHPUnit\Framework\TestCase
{
    /**
    ...
    protected function getService(string $type) : object
    {
        if (self::$container === null) {
            throw new \ConfigTransformer202206079\Symplify\SymplifyKernel\Exception\ShouldNotHappenException('First, create container with booKernel(KernelClass::class)');
        }
        $service = self::$container->get($type);
        if ($service === null) {
            $errorMessage = \sprintf('Services "%s" was not found', $type);
            throw new \ConfigTransformer202206079\Symplify\Astral\Exception\ShouldNotHappenException($errorMessage);
        }
        return $service;
    }
    /**
    ...
    protected function bootKernel(string $kernelClass) : void
    {
        ...
        if (!$kernel instanceof \ConfigTransformer202206079\Symfony\Component\HttpKernel\KernelInterface) {
    ...
    {
        if (static::$kernel !== null && static::$kernel instanceof \ConfigTransformer202206079\Symfony\Component\HttpKernel\KernelInterface) {
            ...
            if ($kernel !== null) {
                ...
                if ($container instanceof \ConfigTransformer202206079\Symfony\Contracts\Service\ResetInterface) {
                    $container->reset();
                }
            }
        }
    ...
    protected function resolveConfigFilePaths(array $configs) : array
    {
        $configFilePaths = [];
        foreach ($configs as $config) {
            $configFilePaths[] = $config instanceof \ConfigTransformer202206079\Symplify\SmartFileSystem\SmartFileInfo ? $config->getRealPath() : $config;
        }
    ...
    private function ensureIsConfigAwareKernel($kernel) : void
    {
        ...
        if ($kernel instanceof \ConfigTransformer202206079\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface) {
            return;
        }
        throw new \ConfigTransformer202206079\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException(\sprintf('"%s" is missing an "%s" interface', \get_class($kernel), \ConfigTransformer202206079\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface::class));
    }
    private function bootAndReturnKernel(\ConfigTransformer202206079\Symfony\Component\HttpKernel\KernelInterface $kernel) : \ConfigTransformer202206079\Symfony\Component\HttpKernel\KernelInterface
    {
        $kernel->boot();
        ...
        if (!$container instanceof \ConfigTransformer202206079\Symfony\Component\DependencyInjection\ContainerInterface) {
            throw new \ConfigTransformer202206079\Symplify\SymplifyKernel\Exception\ShouldNotHappenException();
        }
        // has output? keep it silent out of tests
        if ($container->has(\ConfigTransformer202206079\Symfony\Component\Console\Style\SymfonyStyle::class)) {
            $symfonyStyle = $container->get(\ConfigTransformer202206079\Symfony\Component\Console\Style\SymfonyStyle::class);
            $symfonyStyle->setVerbosity(\ConfigTransformer202206079\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
        }
