...
namespace ConfigTransformer2022051110\Symplify\PackageBuilder\Testing;
...
use ConfigTransformer2022051110\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface;
use ConfigTransformer2022051110\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException;
 ...
 */
abstract class AbstractKernelTestCase extends \ConfigTransformer2022051110\PHPUnit\Framework\TestCase
{
    /**
    ...
    protected function getService(string $type)
    {
        if (self::$container === null) {
            throw new \ConfigTransformer2022051110\Symplify\SymplifyKernel\Exception\ShouldNotHappenException('First, create container with booKernel(KernelClass::class)');
        }
        $service = self::$container->get($type);
        if ($service === null) {
            $errorMessage = \sprintf('Services "%s" was not found', $type);
            throw new \ConfigTransformer2022051110\Symplify\Astral\Exception\ShouldNotHappenException($errorMessage);
        }
        return $service;
    }
    /**
    ...
    protected function bootKernel(string $kernelClass) : void
    {
        ...
        if (!$kernel instanceof \ConfigTransformer2022051110\Symfony\Component\HttpKernel\KernelInterface) {
    ...
    {
        if (static::$kernel !== null && static::$kernel instanceof \ConfigTransformer2022051110\Symfony\Component\HttpKernel\KernelInterface) {
            ...
            if ($kernel !== null) {
                ...
                if ($container instanceof \ConfigTransformer2022051110\Symfony\Contracts\Service\ResetInterface) {
                    $container->reset();
                }
            }
        }
    ...
    protected function resolveConfigFilePaths(array $configs) : array
    {
        $configFilePaths = [];
        foreach ($configs as $config) {
            $configFilePaths[] = $config instanceof \ConfigTransformer2022051110\Symplify\SmartFileSystem\SmartFileInfo ? $config->getRealPath() : $config;
        }
    ...
    private function ensureIsConfigAwareKernel($kernel) : void
    {
        ...
        if ($kernel instanceof \ConfigTransformer2022051110\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface) {
            return;
        }
        throw new \ConfigTransformer2022051110\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException(\sprintf('"%s" is missing an "%s" interface', \get_class($kernel), \ConfigTransformer2022051110\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface::class));
    }
    private function bootAndReturnKernel(\ConfigTransformer2022051110\Symfony\Component\HttpKernel\KernelInterface $kernel) : \ConfigTransformer2022051110\Symfony\Component\HttpKernel\KernelInterface
    {
        $kernel->boot();
        ...
        if (!$container instanceof \ConfigTransformer2022051110\Symfony\Component\DependencyInjection\ContainerInterface) {
            throw new \ConfigTransformer2022051110\Symplify\SymplifyKernel\Exception\ShouldNotHappenException();
        }
        // has output? keep it silent out of tests
        if ($container->has(\ConfigTransformer2022051110\Symfony\Component\Console\Style\SymfonyStyle::class)) {
            $symfonyStyle = $container->get(\ConfigTransformer2022051110\Symfony\Component\Console\Style\SymfonyStyle::class);
            $symfonyStyle->setVerbosity(\ConfigTransformer2022051110\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
        }
