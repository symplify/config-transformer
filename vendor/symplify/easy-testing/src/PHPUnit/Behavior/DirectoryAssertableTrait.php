...
namespace ConfigTransformer2021092210\Symplify\EasyTesting\PHPUnit\Behavior;

use ConfigTransformer2021092210\Symfony\Component\Finder\Finder;
use ConfigTransformer2021092210\Symplify\EasyTesting\ValueObject\ExpectedAndOutputFileInfoPair;
use ConfigTransformer2021092210\Symplify\SmartFileSystem\Finder\FinderSanitizer;
...
trait DirectoryAssertableTrait
{
    ...
    {
        $firstDirectoryFinder = new \ConfigTransformer2021092210\Symfony\Component\Finder\Finder();
        $firstDirectoryFinder->files()->in($directory);
        $finderSanitizer = new \ConfigTransformer2021092210\Symplify\SmartFileSystem\Finder\FinderSanitizer();
    ...
    private function groupFileInfosByRelativeFilePath(array $expectedFileInfos, string $expectedDirectory, array $outputFileInfos, string $outputDirectory) : array
    {
        ...
        foreach ($expectedFileInfos as $expectedFileInfo) {
            ...
            $fileInfosByRelativeFilePath[$relativeFilePath] = new \ConfigTransformer2021092210\Symplify\EasyTesting\ValueObject\ExpectedAndOutputFileInfoPair($expectedFileInfo, $outputFileInfo);
        }
     ...
     */
    private function resolveFileInfoByRelativeFilePath(array $fileInfos, string $directory, string $desiredRelativeFilePath) : ?\ConfigTransformer2021092210\Symplify\SmartFileSystem\SmartFileInfo
    {
