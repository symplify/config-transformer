...
namespace ConfigTransformer202205317\Symplify\EasyTesting\PHPUnit\Behavior;

use ConfigTransformer202205317\Symfony\Component\Finder\Finder;
use ConfigTransformer202205317\Symplify\EasyTesting\ValueObject\ExpectedAndOutputFileInfoPair;
use ConfigTransformer202205317\Symplify\SmartFileSystem\Finder\FinderSanitizer;
...
trait DirectoryAssertableTrait
{
    ...
    {
        $firstDirectoryFinder = new \ConfigTransformer202205317\Symfony\Component\Finder\Finder();
        $firstDirectoryFinder->files()->in($directory);
        $finderSanitizer = new \ConfigTransformer202205317\Symplify\SmartFileSystem\Finder\FinderSanitizer();
    ...
    private function groupFileInfosByRelativeFilePath(array $expectedFileInfos, string $expectedDirectory, array $outputFileInfos, string $outputDirectory) : array
    {
        ...
        foreach ($expectedFileInfos as $expectedFileInfo) {
            ...
            $fileInfosByRelativeFilePath[$relativeFilePath] = new \ConfigTransformer202205317\Symplify\EasyTesting\ValueObject\ExpectedAndOutputFileInfoPair($expectedFileInfo, $outputFileInfo);
        }
     ...
     */
    private function resolveFileInfoByRelativeFilePath(array $fileInfos, string $directory, string $desiredRelativeFilePath) : ?\ConfigTransformer202205317\Symplify\SmartFileSystem\SmartFileInfo
    {
