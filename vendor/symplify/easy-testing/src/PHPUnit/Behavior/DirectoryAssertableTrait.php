...
namespace ConfigTransformer202205010\Symplify\EasyTesting\PHPUnit\Behavior;

use ConfigTransformer202205010\Symfony\Component\Finder\Finder;
use ConfigTransformer202205010\Symplify\EasyTesting\ValueObject\ExpectedAndOutputFileInfoPair;
use ConfigTransformer202205010\Symplify\SmartFileSystem\Finder\FinderSanitizer;
...
trait DirectoryAssertableTrait
{
    ...
    {
        $firstDirectoryFinder = new \ConfigTransformer202205010\Symfony\Component\Finder\Finder();
        $firstDirectoryFinder->files()->in($directory);
        $finderSanitizer = new \ConfigTransformer202205010\Symplify\SmartFileSystem\Finder\FinderSanitizer();
    ...
    private function groupFileInfosByRelativeFilePath(array $expectedFileInfos, string $expectedDirectory, array $outputFileInfos, string $outputDirectory) : array
    {
        ...
        foreach ($expectedFileInfos as $expectedFileInfo) {
            ...
            $fileInfosByRelativeFilePath[$relativeFilePath] = new \ConfigTransformer202205010\Symplify\EasyTesting\ValueObject\ExpectedAndOutputFileInfoPair($expectedFileInfo, $outputFileInfo);
        }
     ...
     */
    private function resolveFileInfoByRelativeFilePath(array $fileInfos, string $directory, string $desiredRelativeFilePath) : ?\ConfigTransformer202205010\Symplify\SmartFileSystem\SmartFileInfo
    {
