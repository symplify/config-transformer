...
namespace ConfigTransformer2021093010\Symplify\EasyTesting\DataProvider;

use Iterator;
use ConfigTransformer2021093010\Nette\Utils\Strings;
use ConfigTransformer2021093010\Symfony\Component\Finder\Finder;
use ConfigTransformer2021093010\Symfony\Component\Finder\SplFileInfo;
use ConfigTransformer2021093010\Symplify\SmartFileSystem\Exception\FileNotFoundException;
...
final class StaticFixtureFinder
{
    ...
    private static function yieldFileInfos(array $fileInfos) : \Iterator
    {
        foreach ($fileInfos as $fileInfo) {
            try {
                $smartFileInfo = new \ConfigTransformer2021093010\Symplify\SmartFileSystem\SmartFileInfo($fileInfo->getRealPath());
                (yield [$smartFileInfo]);
            } catch (\ConfigTransformer2021093010\Symplify\SmartFileSystem\Exception\FileNotFoundException $exception) {
            }
        }
    }
    /**
    ...
    {
        $finder = \ConfigTransformer2021093010\Symfony\Component\Finder\Finder::create()->in($directory)->files()->name($suffix);
    ...
    {
        $iterator = \ConfigTransformer2021093010\Symfony\Component\Finder\Finder::create()->in($directory)->files()->notName($suffix)->getIterator();
        $relativeFilePaths = [];
        foreach ($iterator as $fileInfo) {
            $relativeFilePaths[] = \ConfigTransformer2021093010\Nette\Utils\Strings::substring($fileInfo->getRealPath(), \strlen(\getcwd()) + 1);
        }
        if ($relativeFilePaths === []) {
            return;
        }
        throw new \ConfigTransformer2021093010\Symplify\SymplifyKernel\Exception\ShouldNotHappenException(\sprintf('Files "%s" have invalid suffix, use "%s" suffix instead', \implode('", ', $relativeFilePaths), $suffix));
    }
}
