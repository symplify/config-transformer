...
namespace ConfigTransformer202108306\Symplify\EasyTesting\DataProvider;

use Iterator;
use ConfigTransformer202108306\Nette\Utils\Strings;
use ConfigTransformer202108306\Symfony\Component\Finder\Finder;
use ConfigTransformer202108306\Symfony\Component\Finder\SplFileInfo;
use ConfigTransformer202108306\Symplify\SmartFileSystem\Exception\FileNotFoundException;
...
final class StaticFixtureFinder
{
    ...
    private static function yieldFileInfos(array $fileInfos) : \Iterator
    {
        foreach ($fileInfos as $fileInfo) {
            try {
                $smartFileInfo = new \ConfigTransformer202108306\Symplify\SmartFileSystem\SmartFileInfo($fileInfo->getRealPath());
                (yield [$smartFileInfo]);
            } catch (\ConfigTransformer202108306\Symplify\SmartFileSystem\Exception\FileNotFoundException $exception) {
            }
        }
    }
    /**
    ...
    {
        $finder = \ConfigTransformer202108306\Symfony\Component\Finder\Finder::create()->in($directory)->files()->name($suffix);
    ...
    {
        $iterator = \ConfigTransformer202108306\Symfony\Component\Finder\Finder::create()->in($directory)->files()->notName($suffix)->getIterator();
        $relativeFilePaths = [];
        foreach ($iterator as $fileInfo) {
            $relativeFilePaths[] = \ConfigTransformer202108306\Nette\Utils\Strings::substring($fileInfo->getRealPath(), \strlen(\getcwd()) + 1);
        }
        if ($relativeFilePaths === []) {
            return;
        }
        throw new \ConfigTransformer202108306\Symplify\SymplifyKernel\Exception\ShouldNotHappenException(\sprintf('Files "%s" have invalid suffix, use "%s" suffix instead', \implode('", ', $relativeFilePaths), $suffix));
    }
}
