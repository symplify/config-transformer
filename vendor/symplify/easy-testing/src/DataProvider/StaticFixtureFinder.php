...
namespace ConfigTransformer202205215\Symplify\EasyTesting\DataProvider;

use Iterator;
use ConfigTransformer202205215\Nette\Utils\Strings;
use ConfigTransformer202205215\Symfony\Component\Finder\Finder;
use ConfigTransformer202205215\Symfony\Component\Finder\SplFileInfo;
use ConfigTransformer202205215\Symplify\SmartFileSystem\Exception\FileNotFoundException;
...
final class StaticFixtureFinder
{
    ...
    private static function yieldFileInfos(array $fileInfos) : \Iterator
    {
        foreach ($fileInfos as $fileInfo) {
            try {
                $smartFileInfo = new \ConfigTransformer202205215\Symplify\SmartFileSystem\SmartFileInfo($fileInfo->getRealPath());
                (yield [$smartFileInfo]);
            } catch (\ConfigTransformer202205215\Symplify\SmartFileSystem\Exception\FileNotFoundException $exception) {
            }
        }
    }
    /**
    ...
    {
        $finder = \ConfigTransformer202205215\Symfony\Component\Finder\Finder::create()->in($directory)->files()->name($suffix);
    ...
    {
        $finder = \ConfigTransformer202205215\Symfony\Component\Finder\Finder::create()->in($directory)->files()->notName($suffix);
        /** @var SplFileInfo[] $fileInfos */
        $fileInfos = \iterator_to_array($finder->getIterator());
        $relativeFilePaths = [];
        foreach ($fileInfos as $fileInfo) {
            $relativeFilePaths[] = \ConfigTransformer202205215\Nette\Utils\Strings::substring($fileInfo->getRealPath(), \strlen(\getcwd()) + 1);
        }
        if ($relativeFilePaths === []) {
            return;
        }
        throw new \ConfigTransformer202205215\Symplify\SymplifyKernel\Exception\ShouldNotHappenException(\sprintf('Files "%s" have invalid suffix, use "%s" suffix instead', \implode('", ', $relativeFilePaths), $suffix));
    }
}
