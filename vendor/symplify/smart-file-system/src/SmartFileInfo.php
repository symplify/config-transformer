...
use ConfigTransformer202107229\Symplify\EasyTesting\PHPUnit\StaticPHPUnitEnvironment;
use ConfigTransformer202107229\Symplify\EasyTesting\StaticFixtureSplitter;
 ...
 */
final class SmartFileInfo extends \ConfigTransformer202107229\Symfony\Component\Finder\SplFileInfo
{
    /**
     * @var string
    ...
    {
        $this->smartFileSystem = new \ConfigTransformer202107229\Symplify\SmartFileSystem\SmartFileSystem();
        // accepts also dirs
        if (!\file_exists($filePath)) {
            throw new \ConfigTransformer202107229\Symplify\SmartFileSystem\Exception\FileNotFoundException(\sprintf('File path "%s" was not found while creating "%s" object.', $filePath, self::class));
        }
    ...
    {
        return \ConfigTransformer202107229\Nette\Utils\Strings::replace($this->getRealPath(), self::LAST_SUFFIX_REGEX, '');
    }
    ...
    public function getRelativeFilePathFromDirectory($directory) : string
    {
        if (!\file_exists($directory)) {
            throw new \ConfigTransformer202107229\Symplify\SmartFileSystem\Exception\DirectoryNotFoundException(\sprintf('Directory "%s" was not found in %s.', $directory, self::class));
        }
    ...
    public function getRelativeFilePathFromCwdInTests() : string
    {
        ...
        if (\ConfigTransformer202107229\Symplify\EasyTesting\PHPUnit\StaticPHPUnitEnvironment::isPHPUnitRun()) {
            return $this->getRelativeFilePathFromDirectory(\ConfigTransformer202107229\Symplify\EasyTesting\StaticFixtureSplitter::getTemporaryPath());
        }
