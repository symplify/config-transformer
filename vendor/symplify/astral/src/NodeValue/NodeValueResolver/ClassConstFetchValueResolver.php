 ...
 */
final class ClassConstFetchValueResolver implements \ConfigTransformer2022052410\Symplify\Astral\Contract\NodeValueResolver\NodeValueResolverInterface
{
    /**
    ...
    public function __construct(\ConfigTransformer2022052410\Symplify\Astral\Naming\SimpleNameResolver $simpleNameResolver, \ConfigTransformer2022052410\Symplify\Astral\NodeFinder\SimpleNodeFinder $simpleNodeFinder)
    {
    ...
    {
        return \ConfigTransformer2022052410\PhpParser\Node\Expr\ClassConstFetch::class;
    }
    /**
     * @param ClassConstFetch $expr
     * @return mixed
     */
    public function resolve(\ConfigTransformer2022052410\PhpParser\Node\Expr $expr, string $currentFilePath)
    {
        $className = $this->simpleNameResolver->getName($expr->class);
        if ($className === 'self') {
            $classLike = $this->simpleNodeFinder->findFirstParentByType($expr, \ConfigTransformer2022052410\PhpParser\Node\Stmt\ClassLike::class);
            if (!$classLike instanceof \ConfigTransformer2022052410\PhpParser\Node\Stmt\ClassLike) {
                return null;
            }
