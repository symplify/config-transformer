 ...
 */
final class ClassConstFetchValueResolver implements \ConfigTransformer2021123110\Symplify\Astral\Contract\NodeValueResolver\NodeValueResolverInterface
{
    /**
    ...
    public function __construct(\ConfigTransformer2021123110\Symplify\Astral\Naming\SimpleNameResolver $simpleNameResolver, \ConfigTransformer2021123110\Symplify\Astral\NodeFinder\SimpleNodeFinder $simpleNodeFinder)
    {
    ...
    {
        return \ConfigTransformer2021123110\PhpParser\Node\Expr\ClassConstFetch::class;
    }
    /**
     * @param ClassConstFetch $expr
     * @return null|string|mixed
     */
    public function resolve(\ConfigTransformer2021123110\PhpParser\Node\Expr $expr, string $currentFilePath)
    {
        $className = $this->simpleNameResolver->getName($expr->class);
        if ($className === 'self') {
            $classLike = $this->simpleNodeFinder->findFirstParentByType($expr, \ConfigTransformer2021123110\PhpParser\Node\Stmt\ClassLike::class);
            if (!$classLike instanceof \ConfigTransformer2021123110\PhpParser\Node\Stmt\ClassLike) {
                return null;
            }
