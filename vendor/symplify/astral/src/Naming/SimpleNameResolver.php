...
namespace ConfigTransformer202110101\Symplify\Astral\Naming;
...
final class SimpleNameResolver
{
    ...
    public function getName($node) : ?string
    {
        ...
        if ($node instanceof \ConfigTransformer202110101\PhpParser\Node\Expr\ClassConstFetch && $this->isName($node->name, 'class')) {
            return $this->getName($node->class);
        }
        if ($node instanceof \ConfigTransformer202110101\PhpParser\Node\Stmt\Property) {
            $propertyProperty = $node->props[0];
            return $this->getName($propertyProperty->name);
        }
        if ($node instanceof \ConfigTransformer202110101\PhpParser\Node\Expr\Variable) {
     ...
     */
    public function isNames(\ConfigTransformer202110101\PhpParser\Node $node, array $desiredNames) : bool
    {
    ...
    public function areNamesEqual(\ConfigTransformer202110101\PhpParser\Node $firstNode, \ConfigTransformer202110101\PhpParser\Node $secondNode) : bool
    {
    ...
    public function resolveShortNameFromNode(\ConfigTransformer202110101\PhpParser\Node\Stmt\ClassLike $classLike) : ?string
    {
        ...
        if (\ConfigTransformer202110101\Nette\Utils\Strings::match($className, self::ANONYMOUS_CLASS_REGEX)) {
            return null;
        }
        return $this->resolveShortName($className);
    }
    public function resolveShortNameFromScope(\ConfigTransformer202110101\PHPStan\Analyser\Scope $scope) : ?string
    {
    ...
    public function getClassNameFromScope(\ConfigTransformer202110101\PHPStan\Analyser\Scope $scope) : ?string
    {
        if ($scope->isInTrait()) {
            $traitReflection = $scope->getTraitReflection();
            if (!$traitReflection instanceof \ConfigTransformer202110101\PHPStan\Reflection\ClassReflection) {
                return null;
            }
    ...
    public function isNameMatch(\ConfigTransformer202110101\PhpParser\Node $node, string $desiredNameRegex) : bool
    {
        $name = $this->getName($node);
        if ($name === null) {
            return \false;
        }
        return (bool) \ConfigTransformer202110101\Nette\Utils\Strings::match($name, $desiredNameRegex);
    }
    public function resolveShortName(string $className) : string
    {
        ...
        return (string) \ConfigTransformer202110101\Nette\Utils\Strings::after($className, '\\', -1);
    }
}
