...
namespace ConfigTransformer202202249\Symplify\Astral\Naming;
...
final class SimpleNameResolver
{
    ...
    public function getName($node) : ?string
    {
        ...
        if ($node instanceof \ConfigTransformer202202249\PhpParser\Node\Expr\ClassConstFetch && $this->isName($node->name, 'class')) {
            return $this->getName($node->class);
        }
        if ($node instanceof \ConfigTransformer202202249\PhpParser\Node\Stmt\Property) {
            $propertyProperty = $node->props[0];
            return $this->getName($propertyProperty->name);
        }
        if ($node instanceof \ConfigTransformer202202249\PhpParser\Node\Expr\Variable) {
     ...
     */
    public function isNames(\ConfigTransformer202202249\PhpParser\Node $node, array $desiredNames) : bool
    {
    ...
    public function areNamesEqual(\ConfigTransformer202202249\PhpParser\Node $firstNode, \ConfigTransformer202202249\PhpParser\Node $secondNode) : bool
    {
    ...
    public function resolveShortNameFromNode(\ConfigTransformer202202249\PhpParser\Node\Stmt\ClassLike $classLike) : ?string
    {
        ...
        if (\ConfigTransformer202202249\Nette\Utils\Strings::match($className, self::ANONYMOUS_CLASS_REGEX)) {
            return null;
        }
        return $this->resolveShortName($className);
    }
    public function resolveShortNameFromScope(\ConfigTransformer202202249\PHPStan\Analyser\Scope $scope) : ?string
    {
    ...
    public function getClassNameFromScope(\ConfigTransformer202202249\PHPStan\Analyser\Scope $scope) : ?string
    {
        if ($scope->isInTrait()) {
            $traitReflection = $scope->getTraitReflection();
            if (!$traitReflection instanceof \ConfigTransformer202202249\PHPStan\Reflection\ClassReflection) {
                return null;
            }
    ...
    public function isNameMatch(\ConfigTransformer202202249\PhpParser\Node $node, string $desiredNameRegex) : bool
    {
        $name = $this->getName($node);
        if ($name === null) {
            return \false;
        }
        return (bool) \ConfigTransformer202202249\Nette\Utils\Strings::match($name, $desiredNameRegex);
    }
    public function resolveShortName(string $className) : string
    {
        ...
        return (string) \ConfigTransformer202202249\Nette\Utils\Strings::after($className, '\\', -1);
    }
}
