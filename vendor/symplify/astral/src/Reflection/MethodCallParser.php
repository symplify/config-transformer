...
final class MethodCallParser
{
    ...
    public function __construct(\ConfigTransformer202206075\Symplify\Astral\Naming\SimpleNameResolver $simpleNameResolver, \ConfigTransformer202206075\Symplify\Astral\Reflection\ReflectionParser $reflectionParser)
    {
     ...
     */
    public function parseMethodCall(\ConfigTransformer202206075\PhpParser\Node\Expr\MethodCall $methodCall, \ConfigTransformer202206075\PHPStan\Analyser\Scope $scope)
    {
        $callerType = $scope->getType($methodCall->var);
        if ($callerType instanceof \ConfigTransformer202206075\PHPStan\Type\ThisType) {
            $callerType = $callerType->getStaticObjectType();
        }
        if (!$callerType instanceof \ConfigTransformer202206075\PHPStan\Type\ObjectType) {
            return null;
        }
