...
use ConfigTransformer202206063\Symplify\Astral\PhpDocParser\Exception\InvalidTraverseException;
use ConfigTransformer202206063\Symplify\Astral\PhpDocParser\PhpDocNodeVisitor\CallablePhpDocNodeVisitor;
/**
 * @api
 *
 * Mimics
...
final class PhpDocNodeTraverser
{
    ...
    public function addPhpDocNodeVisitor(\ConfigTransformer202206063\Symplify\Astral\PhpDocParser\Contract\PhpDocNodeVisitorInterface $phpDocNodeVisitor) : void
    {
        $this->phpDocNodeVisitors[] = $phpDocNodeVisitor;
    }
    public function traverse(\ConfigTransformer202206063\PHPStan\PhpDocParser\Ast\Node $node) : void
    {
     ...
     */
    public function traverseWithCallable(\ConfigTransformer202206063\PHPStan\PhpDocParser\Ast\Node $node, string $docContent, callable $callable) : \ConfigTransformer202206063\PHPStan\PhpDocParser\Ast\Node
    {
        $callablePhpDocNodeVisitor = new \ConfigTransformer202206063\Symplify\Astral\PhpDocParser\PhpDocNodeVisitor\CallablePhpDocNodeVisitor($callable, $docContent);
     ...
     * @return TNode
     */
    private function traverseNode(\ConfigTransformer202206063\PHPStan\PhpDocParser\Ast\Node $node) : \ConfigTransformer202206063\PHPStan\PhpDocParser\Ast\Node
    {
        ...
        foreach ($subNodeNames as $subNodeName) {
            ...
            if (\is_array($subNode)) {
            ...
            } elseif ($subNode instanceof \ConfigTransformer202206063\PHPStan\PhpDocParser\Ast\Node) {
                ...
                foreach ($this->phpDocNodeVisitors as $visitorIndex => $phpDocNodeVisitor) {
                    ...
                    if ($return !== null) {
                        if ($return instanceof \ConfigTransformer202206063\PHPStan\PhpDocParser\Ast\Node) {
                            $subNode = $return;
                            ...
                            throw new \ConfigTransformer202206063\Symplify\Astral\PhpDocParser\Exception\InvalidTraverseException('enterNode() returned invalid value of type ' . \gettype($return));
                        }
                    }
                }
                if ($traverseChildren) {
    ...
    private function traverseArray(array $nodes) : array
    {
        foreach ($nodes as $key => &$node) {
            ...
            if (!$node instanceof \ConfigTransformer202206063\PHPStan\PhpDocParser\Ast\Node) {
                continue;
            }
            $traverseChildren = \true;
            $breakVisitorIndex = null;
            ...
            foreach ($this->phpDocNodeVisitors as $visitorIndex => $phpDocNodeVisitor) {
                ...
                if ($return !== null) {
                    if ($return instanceof \ConfigTransformer202206063\PHPStan\PhpDocParser\Ast\Node) {
                        ...
                        throw new \ConfigTransformer202206063\Symplify\Astral\PhpDocParser\Exception\InvalidTraverseException('leaveNode() returned invalid value of type ' . \gettype($return));
                    }
                }
                if ($breakVisitorIndex === $visitorIndex) {
                    break;
