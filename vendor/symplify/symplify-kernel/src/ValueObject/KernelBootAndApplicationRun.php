...
namespace ConfigTransformer2022012610\Symplify\SymplifyKernel\ValueObject;
...
use ConfigTransformer2022012610\Symplify\PackageBuilder\Console\Input\StaticInputDetector;
...
use ConfigTransformer2022012610\Symplify\SymplifyKernel\Exception\BootException;
use Throwable;
/**
 * @api
 */
final class KernelBootAndApplicationRun
{
    ...
    public function run() : void
    {
        try {
            ...
            $symfonyStyleFactory = new \ConfigTransformer2022012610\Symplify\PackageBuilder\Console\Style\SymfonyStyleFactory();
            $symfonyStyle = $symfonyStyleFactory->create();
            $symfonyStyle->error($throwable->getMessage());
            exit(\ConfigTransformer2022012610\Symfony\Component\Console\Command\Command::FAILURE);
        }
    }
    /**
    ...
    private function createKernel()
    {
        ...
        return new $kernelClass($environment, \ConfigTransformer2022012610\Symplify\PackageBuilder\Console\Input\StaticInputDetector::isDebug());
    }
    private function booKernelAndRunApplication() : void
    {
        ...
        $application = $container->get(\ConfigTransformer2022012610\Symfony\Component\Console\Application::class);
    ...
    {
        if (\is_a($kernelClass, \ConfigTransformer2022012610\Symfony\Component\HttpKernel\KernelInterface::class, \true)) {
            return;
        }
        if (\is_a($kernelClass, \ConfigTransformer2022012610\Symplify\SymplifyKernel\Contract\LightKernelInterface::class, \true)) {
            return;
        ...
        $errorMessage = \sprintf('Class "%s" must by type of "%s" or "%s". "%s" given', $kernelClass, \ConfigTransformer2022012610\Symfony\Component\HttpKernel\KernelInterface::class, \ConfigTransformer2022012610\Symplify\SymplifyKernel\Contract\LightKernelInterface::class, $currentValueType);
        throw new \ConfigTransformer2022012610\Symplify\SymplifyKernel\Exception\BootException($errorMessage);
    }
}
