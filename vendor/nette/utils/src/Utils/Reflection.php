...
final class Reflection
{
    ...
    private static function getType($reflection, ?\ReflectionType $type, bool $asArray = \false)
    {
        if ($type === null) {
            ...
            throw new \ConfigTransformer202108309\Nette\InvalidStateException('The ' . self::toString($reflection) . ' is not expected to have a union type.');
        } else {
            throw new \ConfigTransformer202108309\Nette\InvalidStateException('Unexpected type of ' . self::toString($reflection));
        }
    }
    /**
     * @param  \ReflectionFunction|\ReflectionMethod|\ReflectionParameter|\ReflectionProperty  $reflection
    ...
    public static function toString($ref) : string
    {
        if ($ref instanceof \ReflectionClass) {
            ...
            throw new \ConfigTransformer202108309\Nette\InvalidArgumentException();
        }
    }
    /**
    ...
    public static function expandClassName($name, $context) : string
    {
        ...
        if (empty($name)) {
            throw new \ConfigTransformer202108309\Nette\InvalidArgumentException('Class name must not be empty.');
    ...
    public static function getUseStatements($class) : array
    {
        if ($class->isAnonymous()) {
            throw new \ConfigTransformer202108309\Nette\NotImplementedException('Anonymous classes are not supported.');
        }
        static $cache = [];
