...
use ConfigTransformer202112016\Nette\MemberAccessException;
/**
...
final class ObjectHelpers
{
    ...
    public static function strictGet($class, $name) : void
    {
        ...
        throw new \ConfigTransformer202112016\Nette\MemberAccessException("Cannot read an undeclared property {$class}::\${$name}" . ($hint ? ", did you mean \${$hint}?" : '.'));
    }
    /**
     * @return never
     * @throws MemberAccessException
    ...
    public static function strictSet($class, $name) : void
    {
        ...
        throw new \ConfigTransformer202112016\Nette\MemberAccessException("Cannot write to an undeclared property {$class}::\${$name}" . ($hint ? ", did you mean \${$hint}?" : '.'));
    }
    /**
     * @return never
     * @throws MemberAccessException
    ...
    public static function strictCall($class, $method, $additionalMethods = []) : void
    {
        ...
        if (\method_exists($class, $method)) {
            ...
            throw new \ConfigTransformer202112016\Nette\MemberAccessException("Call to {$visibility}method {$class}::{$method}() from " . ($context ? "scope {$context}." : 'global scope.'));
        } else {
            ...
            throw new \ConfigTransformer202112016\Nette\MemberAccessException("Call to undefined method {$class}::{$method}()" . ($hint ? ", did you mean {$hint}()?" : '.'));
        }
    }
    /**
     * @return never
    ...
    public static function strictStaticCall($class, $method) : void
    {
        ...
        if (\method_exists($class, $method)) {
            ...
            throw new \ConfigTransformer202112016\Nette\MemberAccessException("Call to undefined static method {$class}::{$method}()" . ($hint ? ", did you mean {$hint}()?" : '.'));
        }
    }
    /**
