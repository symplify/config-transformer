...
final class Type
{
    ...
    public static function fromReflection($reflection) : ?self
    {
        ...
        if ($type === null) {
        ...
        } elseif ($type instanceof \ReflectionUnionType || $type instanceof \ConfigTransformer202110314\ReflectionIntersectionType) {
            ...
            throw new \ConfigTransformer202110314\Nette\InvalidStateException('Unexpected type of ' . \ConfigTransformer202110314\Nette\Utils\Reflection::toString($reflection));
        }
    }
    /**
    ...
    public static function fromString($type) : self
    {
        if (!\preg_match('#(?:
            ...
            throw new \ConfigTransformer202110314\Nette\InvalidArgumentException("Invalid type '{$type}'.");
        }
        [, $nType, $iType] = $m;
        if ($nType) {
    ...
    {
        return $this->single && \ConfigTransformer202110314\Nette\Utils\Reflection::isBuiltinType($this->types[0]);
    }
    /**
    ...
    public function allows($type) : bool
    {
        ...
        if ($this->isIntersection()) {
            ...
            return \ConfigTransformer202110314\Nette\Utils\Arrays::every($this->types, function ($currentType) use($type) {
                $builtin = \ConfigTransformer202110314\Nette\Utils\Reflection::isBuiltinType($currentType);
                return \ConfigTransformer202110314\Nette\Utils\Arrays::some($type->types, function ($testedType) use($currentType, $builtin) {
                    return $builtin ? \strcasecmp($currentType, $testedType) === 0 : \is_a($testedType, $currentType, \true);
                });
            });
        }
        $method = $type->isIntersection() ? 'some' : 'every';
        return \ConfigTransformer202110314\Nette\Utils\Arrays::$method($type->types, function ($testedType) {
            $builtin = \ConfigTransformer202110314\Nette\Utils\Reflection::isBuiltinType($testedType);
            return \ConfigTransformer202110314\Nette\Utils\Arrays::some($this->types, function ($currentType) use($testedType, $builtin) {
                return $builtin ? \strcasecmp($currentType, $testedType) === 0 : \is_a($testedType, $currentType, \true);
            });
        });
    }
