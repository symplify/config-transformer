 ...
 */
class UnicodeString extends \ConfigTransformer202201117\Symfony\Component\String\AbstractUnicodeString
{
    public function __construct(string $string = '')
    {
        $this->string = \normalizer_is_normalized($string) ? $string : \normalizer_normalize($string);
        if (\false === $this->string) {
            throw new \ConfigTransformer202201117\Symfony\Component\String\Exception\InvalidArgumentException('Invalid UTF-8 string.');
        }
    }
    /**
     * @return $this
    ...
    public function normalize(int $form = self::NFC)
    {
        ...
        if (\in_array($form, [self::NFC, self::NFKC], \true)) {
            ...
            throw new \ConfigTransformer202201117\Symfony\Component\String\Exception\InvalidArgumentException('Unsupported normalization form.');
    ...
    public function split(string $delimiter, int $limit = null, int $flags = null) : array
    {
        ...
        if (\false === $delimiter) {
            throw new \ConfigTransformer202201117\Symfony\Component\String\Exception\InvalidArgumentException('Split delimiter is not a valid UTF-8 string.');
        }
        $str = clone $this;
        $tail = $this->string;
        $chunks = [];
