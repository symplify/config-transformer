 ...
 * @internal
 */
class ArrayNode extends \ConfigTransformer202111140\Symfony\Component\ExpressionLanguage\Node\Node
{
    protected $index;
    ...
    {
        if (null === $key) {
            $key = new \ConfigTransformer202111140\Symfony\Component\ExpressionLanguage\Node\ConstantNode(++$this->index);
        }
    ...
    public function toArray()
    {
        ...
        if ($this->isHash($value)) {
            foreach ($value as $k => $v) {
                $array[] = ', ';
                $array[] = new \ConfigTransformer202111140\Symfony\Component\ExpressionLanguage\Node\ConstantNode($k);
                $array[] = ': ';
                $array[] = $v;
            }
            $array[0] = '{';
