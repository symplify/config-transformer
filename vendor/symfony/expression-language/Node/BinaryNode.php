...
use ConfigTransformer202206079\Symfony\Component\ExpressionLanguage\SyntaxError;
/**
 * @author Fabien Potencier <fabien@symfony.com>
 *
 * @internal
 */
class BinaryNode extends \ConfigTransformer202206079\Symfony\Component\ExpressionLanguage\Node\Node
{
    private const OPERATORS = ['~' => '.', 'and' => '&&', 'or' => '||'];
    private const FUNCTIONS = ['**' => 'pow', '..' => 'range', 'in' => 'in_array', 'not in' => '!in_array'];
    public function __construct(string $operator, \ConfigTransformer202206079\Symfony\Component\ExpressionLanguage\Node\Node $left, \ConfigTransformer202206079\Symfony\Component\ExpressionLanguage\Node\Node $right)
    {
    ...
    public function compile(\ConfigTransformer202206079\Symfony\Component\ExpressionLanguage\Compiler $compiler)
    {
        ...
        if ('matches' == $operator) {
            if ($this->nodes['right'] instanceof \ConfigTransformer202206079\Symfony\Component\ExpressionLanguage\Node\ConstantNode) {
    ...
    private function evaluateMatches(string $regexp, ?string $str) : int
    {
        \set_error_handler(function ($t, $m) use($regexp) {
            throw new \ConfigTransformer202206079\Symfony\Component\ExpressionLanguage\SyntaxError(\sprintf('Regexp "%s" passed to "matches" is not valid', $regexp) . \substr($m, 12));
        });
        try {
