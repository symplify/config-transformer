 ...
 */
class RegisterLocaleAwareServicesPass implements \ConfigTransformer202106282\Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface
{
    ...
    public function process(\ConfigTransformer202106282\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        ...
        foreach ($container->findTaggedServiceIds($this->localeAwareTag) as $id => $tags) {
            $services[] = new \ConfigTransformer202106282\Symfony\Component\DependencyInjection\Reference($id);
        }
        if (!$services) {
            $container->removeDefinition($this->listenerServiceId);
            return;
        }
        $container->getDefinition($this->listenerServiceId)->setArgument(0, new \ConfigTransformer202106282\Symfony\Component\DependencyInjection\Argument\IteratorArgument($services));
    }
}
