 ...
 */
class RegisterLocaleAwareServicesPass implements \ConfigTransformer202107121\Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface
{
    ...
    public function process($container)
    {
        ...
        foreach ($container->findTaggedServiceIds($this->localeAwareTag) as $id => $tags) {
            $services[] = new \ConfigTransformer202107121\Symfony\Component\DependencyInjection\Reference($id);
        }
        if (!$services) {
            $container->removeDefinition($this->listenerServiceId);
            return;
        }
        $container->getDefinition($this->listenerServiceId)->setArgument(0, new \ConfigTransformer202107121\Symfony\Component\DependencyInjection\Argument\IteratorArgument($services));
    }
}
