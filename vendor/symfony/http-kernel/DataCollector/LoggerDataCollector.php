 ...
 * @final
 */
class LoggerDataCollector extends \ConfigTransformer202109091\Symfony\Component\HttpKernel\DataCollector\DataCollector implements \ConfigTransformer202109091\Symfony\Component\HttpKernel\DataCollector\LateDataCollectorInterface
{
    private $logger;
     ...
     * @param object $logger
     */
    public function __construct($logger = null, string $containerPathPrefix = null, \ConfigTransformer202109091\Symfony\Component\HttpFoundation\RequestStack $requestStack = null)
    {
        if (null !== $logger && $logger instanceof \ConfigTransformer202109091\Symfony\Component\HttpKernel\Log\DebugLoggerInterface) {
            $this->logger = $logger;
        }
    ...
    {
        if ($this->logger instanceof \ConfigTransformer202109091\Symfony\Component\HttpKernel\Log\DebugLoggerInterface) {
            $this->logger->clear();
        }
        $this->data = [];
    }
    ...
    private function getContainerDeprecationLogs() : array
    {
        ...
        foreach (\unserialize($logContent) as $log) {
            $log['context'] = ['exception' => new \ConfigTransformer202109091\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext($log['type'], $log['file'], $log['line'], $log['trace'], $log['count'])];
    ...
    private function sanitizeLogs(array $logs)
    {
        ...
        foreach ($logs as $log) {
            ...
            if ($exception instanceof \ConfigTransformer202109091\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext) {
