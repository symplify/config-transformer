...
use ConfigTransformerPrefix202606\Symfony\Component\Yaml\Parser as YamlParser;
use ConfigTransformerPrefix202606\Symfony\Component\Yaml\Tag\TaggedValue;
use ConfigTransformerPrefix202606\Symfony\Component\Yaml\Yaml;
/**
...
class YamlFileLoader extends FileLoader
{
    ...
    private function parseDefinition(string $id, $service, string $file, array $defaults, bool $return = \false, bool $trackBindings = \true)
    {
        ...
        if (\is_string($service) && \strncmp($service, '@', \strlen('@')) === 0) {
            $alias = new Alias((string) \substr($service, 1));
        ...
        if (null !== $definition) {
            ...
            if ('' !== $service['parent'] && '@' === $service['parent'][0]) {
                throw new InvalidArgumentException(\sprintf('The value of the "parent" option for the "%s" service must be the id of the service without the "@" prefix (replace "%s" with "%s").', $id, $service['parent'], (string) \substr($service['parent'], 1)));
            }
        ...
        if (null !== ($decorates = $service['decorates'] ?? null)) {
            if ('' !== $decorates && '@' === $decorates[0]) {
                throw new InvalidArgumentException(\sprintf('The value of the "decorates" option for the "%s" service must be the id of the service without the "@" prefix (replace "%s" with "%s").', $id, $service['decorates'], (string) \substr($decorates, 1)));
            }
    ...
    private function parseCallable($callable, string $parameter, string $id, string $file)
    {
        if (\is_string($callable)) {
            ...
            if ('' !== $callable && '@' === $callable[0]) {
                ...
                throw new InvalidArgumentException(\sprintf('The value of the "%s" option for the "%s" service must be the id of the service without the "@" prefix (replace "%s" with "%s" in "%s").', $parameter, $id, $callable, (string) \substr($callable, 1), $file));
            }
            return $callable;
        }
    ...
    {
        if (!\class_exists(\ConfigTransformerPrefix202606\Symfony\Component\Yaml\Parser::class)) {
