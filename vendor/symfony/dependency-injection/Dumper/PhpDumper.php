...
use ConfigTransformerPrefix202605\Symfony\Component\Debug\DebugClassLoader as LegacyDebugClassLoader;
...
use ConfigTransformerPrefix202605\Symfony\Component\DependencyInjection\Argument\ServiceLocator;
use ConfigTransformerPrefix202605\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument;
use ConfigTransformerPrefix202605\Symfony\Component\DependencyInjection\Compiler\AnalyzeServiceReferencesPass;
use ConfigTransformerPrefix202605\Symfony\Component\DependencyInjection\Compiler\CheckCircularReferencesPass;
use ConfigTransformerPrefix202605\Symfony\Component\DependencyInjection\Compiler\ServiceReferenceGraphNode;
...
use ConfigTransformerPrefix202605\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\DumperInterface;
use ConfigTransformerPrefix202605\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\NullDumper;
use ConfigTransformerPrefix202605\Symfony\Component\DependencyInjection\Loader\FileLoader;
...
use ConfigTransformerPrefix202605\Symfony\Component\DependencyInjection\ServiceLocator as BaseServiceLocator;
use ConfigTransformerPrefix202605\Symfony\Component\DependencyInjection\TypedReference;
use ConfigTransformerPrefix202605\Symfony\Component\DependencyInjection\Variable;
use ConfigTransformerPrefix202605\Symfony\Component\ErrorHandler\DebugClassLoader;
use ConfigTransformerPrefix202605\Symfony\Component\ExpressionLanguage\Expression;
use ConfigTransformerPrefix202605\Symfony\Component\HttpKernel\Kernel;
/**
...
use ConfigTransformerPrefix202605\\Symfony\\Component\\DependencyInjection\\Dumper\\Preloader;
...
EOF;
    ...
    private function collectLineage(string $class, array &$lineage)
    {
        ...
        if (\substr_compare($file, ') : eval()\'d code', -\strlen(') : eval()\'d code')) === 0) {
            $file = (string) \substr($file, 0, \strrpos($file, '(', -17));
        }
        if (!$file || $this->doExport($file) === ($exportedFile = $this->export($file))) {
            return;
        }
        $lineage[$class] = (string) \substr($exportedFile, 1, -1);
    ...
    private function generateProxyClasses() : array
    {
        ...
        foreach ($definitions as $definition) {
            ...
            if ($strip) {
                $proxyCode = "<?php\n" . $proxyCode;
                $proxyCode = (string) \substr(Kernel::stripComments($proxyCode), 5);
            }
            $proxyClass = \explode(' ', $this->inlineRequires ? (string) \substr($proxyCode, \strlen($code)) : $proxyCode, 3)[1];
    ...
    private function addServiceInclude(string $cId, Definition $definition) : string
    {
        ...
        foreach ($this->inlinedDefinitions as $def) {
            if ($file = $def->getFile()) {
                ...
                $file = '(' === $file[0] ? (string) \substr($file, 1, -1) : $file;
                $code .= \sprintf("        include_once %s;\n", $file);
            }
        }
        if ('' !== $code) {
    ...
    private function addNewInstance(Definition $definition, string $return = '', string $id = null) : string
    {
        ...
        if (null !== $definition->getFactory()) {
            ...
            if (\is_string($callable) && \strncmp($callable, '@=', \strlen('@=')) === 0) {
                return $return . \sprintf('(($args = %s) ? (%s) : null)', $this->dumpValue(new ServiceLocatorArgument($definition->getArguments())), $this->getExpressionLanguage()->compile((string) \substr($callable, 2), ['this' => 'container', 'args' => 'args'])) . $tail;
            }
...
EOF;
    ...
    private function addDefaultParametersMethod() : string
    {
        ...
        foreach ($this->container->getParameterBag()->all() as $key => $value) {
            ...
            $export = \explode('0 => ', (string) \substr(\rtrim($export, " ]\n"), 2, -1), 2);
...
EOF;
    ...
    private function dumpLiteralClass(string $class) : string
    {
        ...
        $class = (string) \substr(\str_replace('\\\\', '\\', $class), 1, -1);
    ...
    private function generateMethodName(string $id) : string
    {
        ...
        $i = \strrpos($id, '\\');
        $name = Container::camelize(\false !== $i && isset($id[1 + $i]) ? (string) \substr($id, 1 + $i) : $id);
    ...
    private function getExpressionLanguage() : ExpressionLanguage
    {
        if (!isset($this->expressionLanguage)) {
            if (!\class_exists(\ConfigTransformerPrefix202605\Symfony\Component\ExpressionLanguage\ExpressionLanguage::class)) {
                throw new LogicException('Unable to use expressions as the Symfony ExpressionLanguage component is not installed.');
            }
            $providers = $this->container->getExpressionLanguageProviders();
            $this->expressionLanguage = new ExpressionLanguage(null, $providers, function ($arg) {
                $id = '""' === \substr_replace($arg, '', 1, -1) ? \stripcslashes((string) \substr($arg, 1, -1)) : null;
    ...
    private function export($value)
    {
        if (null !== $this->targetDirRegex && \is_string($value) && \preg_match($this->targetDirRegex, $value, $matches, \PREG_OFFSET_CAPTURE)) {
            ...
            if ('\\' === \DIRECTORY_SEPARATOR && isset($value[$suffix])) {
                ...
                $suffix = '.' . $this->doExport(\str_replace('\\', $cookie, (string) \substr($value, $suffix)), \true);
    ...
    private function doExport($value, bool $resolveEnv = \false)
    {
        ...
        if ($resolveEnv && "'" === $export[0] && $export !== ($resolvedExport = $this->container->resolveEnvPlaceholders($export, "'.\$this->getEnv('string:%s').'"))) {
            ...
            if (\substr_compare($export, ".''", -\strlen(".''")) === 0) {
                $export = (string) \substr($export, 0, -3);
                if ("'" === $export[1]) {
                    $export = \substr_replace($export, '', 18, 7);
                }
            }
            if ("'" === $export[1]) {
                $export = (string) \substr($export, 3);
            }
        }
        if ($shouldCacheValue) {
        ...
        while ($definition instanceof Definition) {
            ...
            if (\is_string($factory[0])) {
                if (\false !== ($i = \strrpos($factory[0], '::'))) {
                    $factory[0] = (string) \substr($factory[0], 0, $i);
                }
