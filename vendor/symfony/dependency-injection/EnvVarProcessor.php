...
class EnvVarProcessor implements EnvVarProcessorInterface
{
    ...
    public function getEnv(string $prefix, string $name, \Closure $getEnv)
    {
        ...
        if ('key' === $prefix) {
            ...
            $next = (string) \substr($name, $i + 1);
            $key = (string) \substr($name, 0, $i);
            $array = $getEnv($next);
            if (!\is_array($array)) {
        ...
        if ('default' === $prefix) {
            ...
            $next = (string) \substr($name, $i + 1);
            $default = (string) \substr($name, 0, $i);
        ...
        if ('url' === $prefix) {
            ...
            $parsedEnv['path'] = '/' === ($parsedEnv['path'] ?? '/') ? '' : (string) \substr($parsedEnv['path'], 1);
            return $parsedEnv;
        }
        ...
        if ('resolve' === $prefix) {
            return \preg_replace_callback('/%%|%([^%\\s]+)%/', function ($match) use($name, $getEnv) {
                ...
                if (\strncmp($match[1], 'env(', \strlen('env(')) === 0 && \substr_compare($match[1], ')', -\strlen(')')) === 0 && 'env()' !== $match[1]) {
                    $value = $getEnv((string) \substr($match[1], 4, -1));
                } else {
