...
use ConfigTransformer202201309\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException;
 ...
 */
final class CheckTypeDeclarationsPass extends \ConfigTransformer202201309\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    protected function processValue($value, bool $isRoot = \false)
    {
        ...
        if (!$value instanceof \ConfigTransformer202201309\Symfony\Component\DependencyInjection\Definition || $value->hasErrors() || $value->isDeprecated()) {
        ...
        if (\ConfigTransformer202201309\Symfony\Component\DependencyInjection\ServiceLocator::class === $value->getClass()) {
            return parent::processValue($value, $isRoot);
        }
     ...
     */
    private function checkTypeDeclarations(\ConfigTransformer202201309\Symfony\Component\DependencyInjection\Definition $checkedDefinition, \ReflectionFunctionAbstract $reflectionFunction, array $values) : void
    {
        $numberOfRequiredParameters = $reflectionFunction->getNumberOfRequiredParameters();
        if (\count($values) < $numberOfRequiredParameters) {
            throw new \ConfigTransformer202201309\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid definition for service "%s": "%s::%s()" requires %d arguments, %d passed.', $this->currentId, $reflectionFunction->class, $reflectionFunction->name, $numberOfRequiredParameters, \count($values)));
        }
        $reflectionParameters = $reflectionFunction->getParameters();
        $checksCount = \min($reflectionFunction->getNumberOfParameters(), \count($values));
        $envPlaceholderUniquePrefix = $this->container->getParameterBag() instanceof \ConfigTransformer202201309\Symfony\Component\DependencyInjection\ParameterBag\EnvPlaceholderParameterBag ? $this->container->getParameterBag()->getEnvPlaceholderUniquePrefix() : null;
     ...
     * @param mixed $value
     */
    private function checkType(\ConfigTransformer202201309\Symfony\Component\DependencyInjection\Definition $checkedDefinition, $value, \ReflectionParameter $parameter, ?string $envPlaceholderUniquePrefix, \ReflectionType $reflectionType = null) : void
    {
        ...
        if ($reflectionType instanceof \ReflectionUnionType) {
            foreach ($reflectionType->getTypes() as $t) {
                try {
                    ...
                    return;
                } catch (\ConfigTransformer202201309\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException $e) {
                }
            }
            throw new \ConfigTransformer202201309\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException($this->currentId, $e->getCode(), $parameter);
        }
        if ($reflectionType instanceof \ConfigTransformer202201309\ReflectionIntersectionType) {
            foreach ($reflectionType->getTypes() as $t) {
                $this->checkType($checkedDefinition, $value, $parameter, $envPlaceholderUniquePrefix, $t);
            }
            return;
        ...
        if ($value instanceof \ConfigTransformer202201309\Symfony\Component\DependencyInjection\Reference) {
            ...
            if ('service_container' === $value && \is_a($type, \ConfigTransformer202201309\Symfony\Component\DependencyInjection\Container::class, \true)) {
                return;
            }
        ...
        if (null === $class) {
            if ($value instanceof \ConfigTransformer202201309\Symfony\Component\DependencyInjection\Argument\IteratorArgument) {
                $class = \ConfigTransformer202201309\Symfony\Component\DependencyInjection\Argument\RewindableGenerator::class;
                ...
                $class = \ConfigTransformer202201309\Symfony\Component\DependencyInjection\ServiceLocator::class;
        ...
        if ('callable' === $type && \is_array($value) && isset($value[0]) && ($value[0] instanceof \ConfigTransformer202201309\Symfony\Component\DependencyInjection\Reference || $value[0] instanceof \ConfigTransformer202201309\Symfony\Component\DependencyInjection\Definition || \is_string($value[0]))) {
            return;
        }
        ...
        throw new \ConfigTransformer202201309\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException($this->currentId, \is_object($value) ? $class : \get_debug_type($value), $parameter);
    }
    ...
    {
        return $this->expressionLanguage = $this->expressionLanguage ?? new \ConfigTransformer202201309\Symfony\Component\DependencyInjection\ExpressionLanguage(null, $this->container->getExpressionLanguageProviders());
    }
}
