...
use ConfigTransformer2021113010\Symfony\Component\DependencyInjection\Attribute\Autoconfigure;
 ...
 */
final class RegisterAutoconfigureAttributesPass implements \ConfigTransformer2021113010\Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface
{
    ...
    {
        foreach ($class->getAttributes(\ConfigTransformer2021113010\Symfony\Component\DependencyInjection\Attribute\Autoconfigure::class, \ReflectionAttribute::IS_INSTANCEOF) as $attribute) {
            self::registerForAutoconfiguration($container, $class, $attribute);
        }
    }
    private static function registerForAutoconfiguration(\ConfigTransformer2021113010\Symfony\Component\DependencyInjection\ContainerBuilder $container, \ReflectionClass $class, \ReflectionAttribute $attribute)
    {
        if (self::$registerForAutoconfiguration) {
            return (self::$registerForAutoconfiguration)($container, $class, $attribute);
        }
        $parseDefinitions = new \ReflectionMethod(\ConfigTransformer2021113010\Symfony\Component\DependencyInjection\Loader\YamlFileLoader::class, 'parseDefinitions');
        $parseDefinitions->setAccessible(\true);
        $yamlLoader = $parseDefinitions->getDeclaringClass()->newInstanceWithoutConstructor();
        self::$registerForAutoconfiguration = static function (\ConfigTransformer2021113010\Symfony\Component\DependencyInjection\ContainerBuilder $container, \ReflectionClass $class, \ReflectionAttribute $attribute) use($parseDefinitions, $yamlLoader) {
