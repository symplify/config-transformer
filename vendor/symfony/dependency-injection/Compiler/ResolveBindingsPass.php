...
use ConfigTransformerPrefix202605\Symfony\Component\DependencyInjection\Attribute\Target;
...
use ConfigTransformerPrefix202605\Symfony\Component\DependencyInjection\LazyProxy\ProxyHelper;
    ...
    protected function processValue($value, bool $isRoot = \false)
    {
        ...
        foreach ($calls as $i => $call) {
            ...
            foreach ($reflectionMethod->getParameters() as $key => $parameter) {
                ...
                if ($typeHint && '\\' === $typeHint[0] && isset($bindings[$typeHint = (string) \substr($typeHint, 1)])) {
                    $arguments[$key] = $this->getBindingValue($bindings[$typeHint]);
                    continue;
                }
                if (isset($bindingNames[$name]) || isset($bindingNames[$parameter->name])) {
                    $bindingKey = \array_search($binding, $bindings, \true);
                    $argumentType = (string) \substr($bindingKey, 0, \strpos($bindingKey, ' '));
