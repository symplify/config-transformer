...
use ConfigTransformer2021062210\Doctrine\Common\Persistence\Proxy as LegacyProxy;
use ConfigTransformer2021062210\Doctrine\Persistence\Proxy;
use ConfigTransformer2021062210\Mockery\MockInterface;
use ConfigTransformer2021062210\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation;
use ConfigTransformer2021062210\PHPUnit\Framework\MockObject\MockObject;
use ConfigTransformer2021062210\Prophecy\Prophecy\ProphecySubjectInterface;
...
class DebugClassLoader
{
    ...
    public static function enable() : void
    {
        ...
        \class_exists(\ConfigTransformer2021062210\Symfony\Component\ErrorHandler\ErrorHandler::class);
        \class_exists(\ConfigTransformer2021062210\Psr\Log\LogLevel::class);
    ...
    public static function checkClasses() : bool
    {
        ...
        foreach ($offsets as $getSymbols => $i) {
            ...
            for (; $i < \count($symbols); ++$i) {
                if (!\is_subclass_of($symbols[$i], \ConfigTransformer2021062210\PHPUnit\Framework\MockObject\MockObject::class) && !\is_subclass_of($symbols[$i], \ConfigTransformer2021062210\Prophecy\Prophecy\ProphecySubjectInterface::class) && !\is_subclass_of($symbols[$i], \ConfigTransformer2021062210\Doctrine\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \ConfigTransformer2021062210\ProxyManager\Proxy\ProxyInterface::class) && !\is_subclass_of($symbols[$i], \ConfigTransformer2021062210\Doctrine\Common\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \ConfigTransformer2021062210\Mockery\MockInterface::class)) {
    ...
    public function checkAnnotations(\ReflectionClass $refl, string $class) : array
    {
        ...
        foreach ($parentAndOwnInterfaces + \class_uses($class, \false) as $use) {
            ...
            if (isset(self::$method[$use])) {
                if ($refl->isAbstract()) {
                    ...
                    if (!\strncmp($vendor, \str_replace('_', '\\', $use), $vendorLen) && 0 === \strpos($className, 'Symfony\\') && (!\class_exists(\ConfigTransformer2021062210\Composer\InstalledVersions::class) || 'symfony/symfony' !== \ConfigTransformer2021062210\Composer\InstalledVersions::getRootPackage()['name'])) {
        ...
        foreach ($refl->getMethods() as $method) {
            ...
            if ($finalOrInternal || $method->isConstructor() || \false === \strpos($doc, '@param') || \ConfigTransformer2021062210\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation::class === $class) {
                continue;
            }
