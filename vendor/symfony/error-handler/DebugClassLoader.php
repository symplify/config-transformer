...
use ConfigTransformer202106263\Doctrine\Common\Persistence\Proxy as LegacyProxy;
use ConfigTransformer202106263\Doctrine\Persistence\Proxy;
use ConfigTransformer202106263\Mockery\MockInterface;
use ConfigTransformer202106263\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation;
use ConfigTransformer202106263\PHPUnit\Framework\MockObject\MockObject;
use ConfigTransformer202106263\Prophecy\Prophecy\ProphecySubjectInterface;
...
class DebugClassLoader
{
    ...
    public static function enable() : void
    {
        ...
        \class_exists(\ConfigTransformer202106263\Symfony\Component\ErrorHandler\ErrorHandler::class);
        \class_exists(\ConfigTransformer202106263\Psr\Log\LogLevel::class);
    ...
    public static function checkClasses() : bool
    {
        ...
        foreach ($offsets as $getSymbols => $i) {
            ...
            for (; $i < \count($symbols); ++$i) {
                if (!\is_subclass_of($symbols[$i], \ConfigTransformer202106263\PHPUnit\Framework\MockObject\MockObject::class) && !\is_subclass_of($symbols[$i], \ConfigTransformer202106263\Prophecy\Prophecy\ProphecySubjectInterface::class) && !\is_subclass_of($symbols[$i], \ConfigTransformer202106263\Doctrine\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \ConfigTransformer202106263\ProxyManager\Proxy\ProxyInterface::class) && !\is_subclass_of($symbols[$i], \ConfigTransformer202106263\Doctrine\Common\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \ConfigTransformer202106263\Mockery\MockInterface::class)) {
    ...
    public function checkAnnotations(\ReflectionClass $refl, string $class) : array
    {
        ...
        foreach ($parentAndOwnInterfaces + \class_uses($class, \false) as $use) {
            ...
            if (isset(self::$method[$use])) {
                if ($refl->isAbstract()) {
                    ...
                    if (!\strncmp($vendor, \str_replace('_', '\\', $use), $vendorLen) && 0 === \strpos($className, 'Symfony\\') && (!\class_exists(\ConfigTransformer202106263\Composer\InstalledVersions::class) || 'symfony/symfony' !== \ConfigTransformer202106263\Composer\InstalledVersions::getRootPackage()['name'])) {
        ...
        foreach ($refl->getMethods() as $method) {
            ...
            if ($finalOrInternal || $method->isConstructor() || \false === \strpos($doc, '@param') || \ConfigTransformer202106263\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation::class === $class) {
                continue;
            }
