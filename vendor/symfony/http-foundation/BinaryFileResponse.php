...
use ConfigTransformer2021090610\Symfony\Component\HttpFoundation\File\File;
/**
 ...
 */
class BinaryFileResponse extends \ConfigTransformer2021090610\Symfony\Component\HttpFoundation\Response
{
    ...
    {
        if (!$file instanceof \ConfigTransformer2021090610\Symfony\Component\HttpFoundation\File\File) {
            if ($file instanceof \SplFileInfo) {
                $file = new \ConfigTransformer2021090610\Symfony\Component\HttpFoundation\File\File($file->getPathname());
            } else {
                $file = new \ConfigTransformer2021090610\Symfony\Component\HttpFoundation\File\File((string) $file);
            }
        }
        if (!$file->isReadable()) {
            throw new \ConfigTransformer2021090610\Symfony\Component\HttpFoundation\File\Exception\FileException('File must be readable.');
        }
        $this->file = $file;
        if ($autoEtag) {
            $this->setAutoEtag();
    ...
    public function prepare($request)
    {
        ...
        if (self::$trustXSendfileTypeHeader && $request->headers->has('X-Sendfile-Type')) {
            ...
            if ('x-accel-redirect' === \strtolower($type)) {
                ...
                $parts = \ConfigTransformer2021090610\Symfony\Component\HttpFoundation\HeaderUtils::split($request->headers->get('X-Accel-Mapping', ''), ',=');
