...
use ConfigTransformerPrefix202606\Symfony\Component\Yaml\Exception\DumpException;
                ...
                switch (\true) {
                    ...
                    case \strncmp($scalar, '!php/enum', \strlen('!php/enum')) === 0:
                        if (self::$constantSupport) {
                            ...
                            $i = 0;
                            $enum = self::parseScalar((string) \substr($scalar, 10), 0, null, $i, \false);
                            if ($useValue = \substr_compare($enum, '->value', -\strlen('->value')) === 0) {
                                $enum = (string) \substr($enum, 0, -7);
                            }
                            if (!\defined($enum)) {
            ...
            case \in_array($scalar[0], ['+', '-', '.'], \true) || \is_numeric($scalar[0]):
                ...
                switch (\true) {
                    ...
                    case Parser::preg_match(self::getTimestampRegex(), $scalar):
                        ...
                        } catch (\ConfigTransformerPrefix202606\DateRangeError|\ValueError $exception) {
                            // no-op
                        }
