...
class InputArgument
{
    ...
    public function __construct(string $name, int $mode = null, string $description = '', $default = null)
    {
        if (null === $mode) {
            ...
            throw new \ConfigTransformer202206079\Symfony\Component\Console\Exception\InvalidArgumentException(\sprintf('Argument mode "%s" is not valid.', $mode));
        }
        $this->name = $name;
        $this->mode = $mode;
    ...
    public function setDefault($default = null)
    {
        if ($this->isRequired() && null !== $default) {
            throw new \ConfigTransformer202206079\Symfony\Component\Console\Exception\LogicException('Cannot set a default value except for InputArgument::OPTIONAL mode.');
        }
        if ($this->isArray()) {
            if (null === $default) {
                $default = [];
            } elseif (!\is_array($default)) {
                throw new \ConfigTransformer202206079\Symfony\Component\Console\Exception\LogicException('A default value for an array argument must be an array.');
            }
        }
