...
use ConfigTransformer202112105\Symfony\Component\Console\Completion\Output\BashCompletionOutput;
use ConfigTransformer202112105\Symfony\Component\Console\Completion\Output\CompletionOutputInterface;
 ...
 */
final class CompleteCommand extends \ConfigTransformer202112105\Symfony\Component\Console\Command\Command
{
    ...
    public function __construct(array $completionOutputs = [])
    {
        ...
        $this->completionOutputs = $completionOutputs + ['bash' => \ConfigTransformer202112105\Symfony\Component\Console\Completion\Output\BashCompletionOutput::class];
        parent::__construct();
    }
    protected function configure() : void
    {
        $this->addOption('shell', 's', \ConfigTransformer202112105\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'The shell type ("' . \implode('", "', \array_keys($this->completionOutputs)) . '")')->addOption('input', 'i', \ConfigTransformer202112105\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED | \ConfigTransformer202112105\Symfony\Component\Console\Input\InputOption::VALUE_IS_ARRAY, 'An array of input tokens (e.g. COMP_WORDS or argv)')->addOption('current', 'c', \ConfigTransformer202112105\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'The index of the "input" array that the cursor is in (e.g. COMP_CWORD)')->addOption('symfony', 'S', \ConfigTransformer202112105\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'The version of the completion script');
    }
    /**
    ...
    protected function execute($input, $output) : int
    {
        try {
            ...
            $suggestions = new \ConfigTransformer202112105\Symfony\Component\Console\Completion\CompletionSuggestions();
            ...
            if (null === $command) {
                ...
                if (\ConfigTransformer202112105\Symfony\Component\Console\Completion\CompletionInput::TYPE_OPTION_NAME === $completionInput->getCompletionType()) {
                    $this->log('  Completing option names for the <comment>' . \get_class($command instanceof \ConfigTransformer202112105\Symfony\Component\Console\Command\LazyCommand ? $command->getCommand() : $command) . '</> command.');
                    $suggestions->suggestOptions($command->getDefinition()->getOptions());
                } else {
                    $this->log(['  Completing using the <comment>' . \get_class($command instanceof \ConfigTransformer202112105\Symfony\Component\Console\Command\LazyCommand ? $command->getCommand() : $command) . '</> class.', '  Completing <comment>' . $completionInput->getCompletionType() . '</> for <comment>' . $completionInput->getCompletionName() . '</>']);
    ...
    private function createCompletionInput(\ConfigTransformer202112105\Symfony\Component\Console\Input\InputInterface $input) : \ConfigTransformer202112105\Symfony\Component\Console\Completion\CompletionInput
    {
        $currentIndex = $input->getOption('current');
        if (!$currentIndex || !\ctype_digit($currentIndex)) {
            throw new \RuntimeException('The "--current" option must be set and it must be an integer.');
        }
        $completionInput = \ConfigTransformer202112105\Symfony\Component\Console\Completion\CompletionInput::fromTokens($input->getOption('input'), (int) $currentIndex);
        try {
    ...
    private function findCommand(\ConfigTransformer202112105\Symfony\Component\Console\Completion\CompletionInput $completionInput, \ConfigTransformer202112105\Symfony\Component\Console\Output\OutputInterface $output) : ?\ConfigTransformer202112105\Symfony\Component\Console\Command\Command
    {
        try {
        ...
        } catch (\ConfigTransformer202112105\Symfony\Component\Console\Exception\CommandNotFoundException $e) {
        }
        return null;
    }
