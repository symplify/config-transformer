 ...
 */
namespace ConfigTransformer202201208\Symfony\Component\VarExporter\Internal;

use ConfigTransformer202201208\Symfony\Component\VarExporter\Exception\NotInstantiableTypeException;
/**
...
class Exporter
{
    ...
    public static function prepare($values, $objectsPool, &$refsPool, &$objectsCount, &$valuesAreStatic) : array
    {
        ...
        foreach ($values as $k => $value) {
            if (\is_resource($value)) {
                throw new \ConfigTransformer202201208\Symfony\Component\VarExporter\Exception\NotInstantiableTypeException(\get_resource_type($value) . ' resource');
            }
            $refs[$k] = $objectsPool;
            if ($isRef = !($valueIsStatic = $values[$k] !== $objectsPool)) {
                ...
                if ($value instanceof \ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Reference && 0 > $value->id) {
                    $valuesAreStatic = \false;
                    ++$value->count;
                    continue;
                }
                $refsPool[] = [&$refs[$k], $value, &$value];
                $refs[$k] = $values[$k] = new \ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Reference(-\count($refsPool), $value);
            }
            if (\is_array($value)) {
                if ($value) {
                ...
                goto handle_value;
            } elseif (!\is_object($value) || $value instanceof \ConfigTransformer202201208\UnitEnum) {
                goto handle_value;
            }
            $valueIsStatic = \false;
            if (isset($objectsPool[$value])) {
                ++$objectsCount;
                $value = new \ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Reference($objectsPool[$value][0]);
                goto handle_value;
            }
            $class = \get_class($value);
            $reflector = \ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Registry::$reflectors[$class] ?? \ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Registry::getClassReflector($class);
            ...
            $properties = [];
            $sleep = null;
            $proto = \ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Registry::$prototypes[$class];
            if (($value instanceof \ArrayIterator || $value instanceof \ArrayObject) && null !== $proto) {
                ...
                \ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Registry::getClassReflector($class, \ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Registry::$instantiableWithoutConstructor[$class], \ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Registry::$cloneable[$class]);
            } elseif ($value instanceof \SplObjectStorage && \ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Registry::$cloneable[$class] && null !== $proto) {
                ...
                $value = new \ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Reference($id);
                goto handle_value;
            } else {
    ...
    {
        switch (\true) {
            case \is_int($value) || \is_float($value) || $value instanceof \ConfigTransformer202201208\UnitEnum:
        ...
        if ($value instanceof \ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Reference) {
            if (0 <= $value->id) {
                return '$o[' . $value->id . ']';
            }
            if (!$value->count) {
        ...
        if ($value instanceof \ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Values) {
            $code = $subIndent . "\$r = [],\n";
        ...
        if ($value instanceof \ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Registry) {
            return self::exportRegistry($value, $indent, $subIndent);
        }
        if ($value instanceof \ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Hydrator) {
    ...
    private static function exportRegistry(\ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Registry $value, string $indent, string $subIndent) : string
    {
        $code = '';
        $serializables = [];
        $seen = [];
        $prototypesAccess = 0;
        $factoriesAccess = 0;
        $r = '\\' . \ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Registry::class;
        $j = -1;
        foreach ($value->classes as $k => $class) {
            ...
            if (!\ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Registry::$instantiableWithoutConstructor[$class]) {
            ...
            $c = '[' . self::export($class) . ']';
            if ($seen[$class] ?? \false) {
                if (\ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Registry::$cloneable[$class]) {
                    ++$prototypesAccess;
                    $code .= 'clone $p' . $c;
                } else {
                    ++$factoriesAccess;
    ...
    private static function exportHydrator(\ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Hydrator $value, string $indent, string $subIndent) : string
    {
        $code = '';
    ...
    private static function getArrayObjectProperties($value, $proto) : array
    {
        ...
        $reflector = \ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Registry::$reflectors[$reflector] ?? \ConfigTransformer202201208\Symfony\Component\VarExporter\Internal\Registry::getClassReflector($reflector);
