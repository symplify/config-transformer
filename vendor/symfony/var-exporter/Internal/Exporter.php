 ...
 */
namespace ConfigTransformerPrefix202606\Symfony\Component\VarExporter\Internal;

use ConfigTransformerPrefix202606\Symfony\Component\VarExporter\Exception\NotInstantiableTypeException;
/**
...
class Exporter
{
    ...
    public static function prepare($values, $objectsPool, &$refsPool, &$objectsCount, &$valuesAreStatic)
    {
        ...
        foreach ($values as $k => $value) {
            ...
            if ($value instanceof \Closure && !($r = new \ReflectionFunction($value))->isAnonymous()) {
                $callable = [$r->getClosureThis() ?? (($nullsafeVariable1 = $r->getClosureCalledClass()) ? $nullsafeVariable1->name : null), $r->name];
            ...
            foreach ($arrayValue as $name => $v) {
                ...
                if ('' === $n || "\x00" !== $n[0]) {
                    ...
                    $i = \strpos($n, "\x00", 2);
                    $c = (string) \substr($n, 1, $i - 1);
                    $n = (string) \substr($n, 1 + $i);
                }
                if (null !== $sleep) {
    ...
    public static function export($value, $indent = '')
    {
        ...
        if ($value instanceof NamedClosure) {
            if ((($nullsafeVariable2 = $value->method) ? $nullsafeVariable2->isPublic() : null) ?? \true) {
                switch (\true) {
                    case null === $value->callable[0]:
                        return '\\' . $value->callable[1] . '(...)';
                    case \is_string($value->callable[0]):
                        return '\\' . $value->callable[0] . '::' . $value->callable[1] . '(...)';
                    case \is_object($value->callable[0]):
                        return self::export($value->callable[0], $subIndent) . '->' . $value->callable[1] . '(...)';
                }
            }
