...
use ConfigTransformerPrefix202605\Symfony\Component\VarExporter\Exception\LogicException;
use ConfigTransformerPrefix202605\Symfony\Component\VarExporter\Internal\Hydrator;
use ConfigTransformerPrefix202605\Symfony\Component\VarExporter\Internal\LazyDecoratorTrait;
use ConfigTransformerPrefix202605\Symfony\Component\VarExporter\Internal\LazyObjectRegistry;
/**
...
EOPHP;
    ...
    public static function exportParameters(\ReflectionFunctionAbstract $function, bool $withParameterTypes = \true, ?string &$args = null) : string
    {
        ...
        $namespace = (string) \substr($namespace, 0, \strrpos($namespace, '\\') ?: 0);
        foreach ($function->getParameters() as $param) {
            $parameters[] = ((\method_exists($param, 'getAttributes') ? $param->getAttributes(\ConfigTransformerPrefix202605\SensitiveParameter::class) : []) ? '#[\\SensitiveParameter] ' : '') . ($withParameterTypes && $param->hasType() ? self::exportType($param) . ' ' : '') . ($param->isPassedByReference() ? '&' : '') . ($param->isVariadic() ? '...' : '') . '$' . $param->name . ($param->isOptional() && !$param->isVariadic() ? ' = ' . self::exportDefault($param, $namespace) : '');
    ...
    public static function exportType($owner, bool $noBuiltin = \false, ?\ReflectionType $type = null) : ?string
    {
        ...
        if (null === $glue) {
            $defaultNull = $owner instanceof \ReflectionParameter && 'NULL' === \rtrim((string) \substr(\explode('$' . $owner->name . ' = ', (string) $owner, 2)[1] ?? '', 0, -2));
    ...
    {
        $default = \rtrim((string) \substr(\explode('$' . $param->name . ' = ', (string) $param, 2)[1] ?? '', 0, -2));
        ...
        return \implode('', \array_map(static function ($part) use($regexp, $callback) {
            switch ($part[0]) {
                case '"':
                    return $part;
                case "'":
    ...
    {
        if (!$mightBeRootConst || \false === ($ns = \strrpos($symbol, '\\')) || \substr($symbol, 0, $ns) !== $namespace || \defined($symbol) || !\defined((string) \substr($symbol, $ns + 1))) {
            return '\\' . $symbol;
        }
