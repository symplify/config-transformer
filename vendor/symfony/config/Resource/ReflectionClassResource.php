 ...
 */
namespace ConfigTransformerPrefix202605\Symfony\Component\Config\Resource;

use ConfigTransformerPrefix202605\Symfony\Component\EventDispatcher\EventSubscriberInterface;
use ConfigTransformerPrefix202605\Symfony\Component\Form\FormTypeExtensionInterface;
use ConfigTransformerPrefix202605\Symfony\Component\Messenger\Handler\MessageSubscriberInterface;
...
class ReflectionClassResource implements SelfCheckingResourceInterface
{
        ...
        do {
            ...
            if (\false !== $file && \is_file($file)) {
                foreach ($this->excludedVendors as $vendor) {
                    if (\strncmp($file, $vendor, \strlen($vendor)) === 0 && \false !== \strpbrk((string) \substr($file, \strlen($vendor), 1), '/' . \DIRECTORY_SEPARATOR)) {
                        $file = \false;
                        break;
                    }
                }
    ...
    private function generateSignature(\ReflectionClass $class) : iterable
    {
        ...
        foreach ($class->getReflectionConstants() as $constant) {
            foreach (\method_exists($constant, 'getAttributes') ? $constant->getAttributes() : [] as $a) {
                $attributes[] = [$a->getName(), (string) $a];
            }
