...
use ConfigTransformerPrefix202605\Symfony\Component\Finder\Finder;
use ConfigTransformerPrefix202605\Symfony\Component\Finder\Glob;
/**
...
class GlobResource implements \IteratorAggregate, SelfCheckingResourceInterface
{
    ...
    public function getIterator() : \Traversable
    {
        ...
        if (\class_exists(Finder::class)) {
            ...
            if ($this->recursive) {
                $regex = \substr_replace($regex, \substr_compare($pattern, '/', -\strlen('/')) === 0 ? '' : '(/|$)', -2, 1);
            }
        } else {
            $regex = null;
        }
        ...
        if (null !== $paths) {
            ...
            foreach ($paths as $path => $regex) {
                ...
                if ((null === $regex || \preg_match($regex, (string) \substr(\str_replace('\\', '/', $path), $prefixLen))) && \is_file($path)) {
                    (yield $path => new \SplFileInfo($path));
                }
                if (!\is_dir($path)) {
                    continue;
                }
                if ($this->forExclusion && (null === $regex || \preg_match($regex, (string) \substr(\str_replace('\\', '/', $path), $prefixLen)))) {
                    (yield $path => new \SplFileInfo($path));
                    continue;
                }
                ...
                $files = \iterator_to_array(new \RecursiveIteratorIterator(new \RecursiveCallbackFilterIterator(new \RecursiveDirectoryIterator($path, \FilesystemIterator::SKIP_DOTS | \FilesystemIterator::FOLLOW_SYMLINKS), function (\SplFileInfo $file, $path) use($regex, $prefixLen) {
                    return !isset($this->excludedPrefixes[$path = \str_replace('\\', '/', $path)]) && (null === $regex || \preg_match($regex, (string) \substr($path, $prefixLen)) || $file->isDir()) && '.' !== $file->getBasename()[0];
        ...
        yield from (new Finder())->followLinks()->filter(function (\SplFileInfo $info) use($regex, $prefixLen, $prefix) {
            ...
            if (!\preg_match($regex, (string) \substr($normalizedPath, $prefixLen)) || !$info->isFile()) {
                return \false;
            }
            if ($this->excludedPrefixes) {
                do {
