...
use ConfigTransformerPrefix202605\Symfony\Component\Config\Definition\Builder\ExprBuilder;
...
class ConfigBuilderGenerator implements ConfigBuilderGeneratorInterface
{
    ...
    public function build(ConfigurationInterface $configuration) : \Closure
    {
        ...
        $rootClass = new ClassBuilder('ConfigTransformerPrefix202605\\Symfony\\Config', $rootNode->getName());
    ...
    private function getComment(BaseNode $node) : string
    {
        ...
        if (!$node instanceof ArrayNode) {
            ...
            if ($node instanceof EnumNode) {
                $comment .= \sprintf('@param ParamConfigurator|%s $value', \implode('|', \array_unique(\array_map(static function ($a) {
...
public function NAME(string $key, mixed $value): static
{
    ...
    {
        return \sprintf('%s\\%s', $rootClass->getNamespace(), (string) \substr($rootClass->getName(), 0, -6));
    }
