...
', ['NAMESPACE' => $this->namespace, 'REQUIRE' => $require, 'USE' => $use, 'CLASS' => $this->getName(), 'IMPLEMENTS' => $implements, 'BODY' => $body]);
    ...
    {
        $this->methods[] = new \ConfigTransformer202107233\Symfony\Component\Config\Builder\Method(\strtr($body, ['NAME' => $this->camelCase($name)] + $params));
    }
    /**
     * @param string $name
     * @param string|null $classType
     */
    public function addProperty($name, $classType = null) : \ConfigTransformer202107233\Symfony\Component\Config\Builder\Property
    {
        $property = new \ConfigTransformer202107233\Symfony\Component\Config\Builder\Property($name, $this->camelCase($name));
        if (null !== $classType) {
