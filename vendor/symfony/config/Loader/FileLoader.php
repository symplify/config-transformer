 ...
 */
namespace ConfigTransformerPrefix202605\Symfony\Component\Config\Loader;

use ConfigTransformerPrefix202605\Symfony\Component\Config\Exception\FileLoaderImportCircularReferenceException;
...
use ConfigTransformerPrefix202605\Symfony\Component\Config\Resource\FileExistenceResource;
...
abstract class FileLoader extends Loader
{
    ...
    public function import($resource, ?string $type = null, bool $ignoreErrors = \false, ?string $sourceResource = null, $exclude = null)
    {
        ...
        if (\is_string($resource) && !\class_exists($resource)) {
            ...
            if ($isGlobPattern && \strpos($resource, "\n") === \false) {
                $ret = [];
                $i = \strcspn($resource, '*?{[');
                $isSubpath = 0 !== $i && \strpos((string) \substr($resource, 0, $i), '/') !== \false;
    ...
    protected function glob(string $pattern, bool $recursive, &$resource = null, bool $ignoreErrors = \false, bool $forExclusion = \false, array $excluded = []) : iterable
    {
        if (\strlen($pattern) === ($i = \strcspn($pattern, '*?{['))) {
            $prefix = $pattern;
            $pattern = '';
        } elseif (0 === $i || \strpos((string) \substr($pattern, 0, $i), '/') === \false) {
            $prefix = '.';
            $pattern = '/' . $pattern;
        } else {
            $prefix = \dirname((string) \substr($pattern, 0, 1 + $i));
            $pattern = (string) \substr($pattern, \strlen($prefix));
        }
        try {
