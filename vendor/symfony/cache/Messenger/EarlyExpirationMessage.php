 ...
 */
namespace ConfigTransformerPrefix202605\Symfony\Component\Cache\Messenger;

use ConfigTransformerPrefix202605\Symfony\Component\Cache\Adapter\AdapterInterface;
use ConfigTransformerPrefix202605\Symfony\Component\Cache\CacheItem;
use ConfigTransformerPrefix202605\Symfony\Component\DependencyInjection\ReverseContainer;
/**
...
final class EarlyExpirationMessage
{
    ...
    public static function create(ReverseContainer $reverseContainer, callable $callback, CacheItem $item, AdapterInterface $pool) : ?self
    {
        ...
        if ($callback instanceof \Closure && \strpos(($r = new \ReflectionFunction($callback))->name, '{closure') === \false) {
            $callback = [$r->getClosureThis() ?? (($nullsafeVariable1 = \PHP_VERSION_ID >= 80111 ? $r->getClosureCalledClass() : $r->getClosureScopeClass()) ? $nullsafeVariable1->name : null), $r->name];
    ...
    public function findCallback(ReverseContainer $reverseContainer) : callable
    {
        if (\is_string($callback = $this->callback)) {
            return '@' === $callback[0] ? $reverseContainer->getService((string) \substr($callback, 1)) : $callback;
        }
        if ('@' === $callback[0][0]) {
            $callback[0] = $reverseContainer->getService((string) \substr($callback[0], 1));
        }
        return $callback;
    }
    /**
