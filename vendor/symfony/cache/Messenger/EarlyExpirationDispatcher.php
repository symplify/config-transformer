...
class EarlyExpirationDispatcher
{
    ...
    public function __construct(\ConfigTransformer202206079\Symfony\Component\Messenger\MessageBusInterface $bus, \ConfigTransformer202206079\Symfony\Component\DependencyInjection\ReverseContainer $reverseContainer, callable $callbackWrapper = null)
    {
        $this->bus = $bus;
    ...
    public function __invoke(callable $callback, \ConfigTransformer202206079\Symfony\Component\Cache\CacheItem $item, bool &$save, \ConfigTransformer202206079\Symfony\Component\Cache\Adapter\AdapterInterface $pool, \Closure $setMetadata, \ConfigTransformer202206079\Psr\Log\LoggerInterface $logger = null)
    {
        if (!$item->isHit() || null === ($message = \ConfigTransformer202206079\Symfony\Component\Cache\Messenger\EarlyExpirationMessage::create($this->reverseContainer, $callback, $item, $pool))) {
        ...
        if ($logger) {
            if ($envelope->last(\ConfigTransformer202206079\Symfony\Component\Messenger\Stamp\HandledStamp::class)) {
