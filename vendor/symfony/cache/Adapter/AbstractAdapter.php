...
use ConfigTransformer202111231\Psr\Log\LoggerInterface;
...
use ConfigTransformer202111231\Symfony\Contracts\Cache\CacheInterface;
/**
 * @author Nicolas Grekas <p@tchwork.com>
 */
abstract class AbstractAdapter implements \ConfigTransformer202111231\Symfony\Component\Cache\Adapter\AdapterInterface, \ConfigTransformer202111231\Symfony\Contracts\Cache\CacheInterface, \ConfigTransformer202111231\Psr\Log\LoggerAwareInterface, \ConfigTransformer202111231\Symfony\Component\Cache\ResettableInterface
{
    ...
    {
        $this->namespace = '' === $namespace ? '' : \ConfigTransformer202111231\Symfony\Component\Cache\CacheItem::validateKey($namespace) . static::NS_SEPARATOR;
     ...
     */
    public static function createSystemCache(string $namespace, int $defaultLifetime, string $version, string $directory, \ConfigTransformer202111231\Psr\Log\LoggerInterface $logger = null)
    {
        $opcache = new \ConfigTransformer202111231\Symfony\Component\Cache\Adapter\PhpFilesAdapter($namespace, $defaultLifetime, $directory, \true);
        if (null !== $logger) {
            $opcache->setLogger($logger);
        }
        if (!(self::$apcuSupported = self::$apcuSupported ?? \ConfigTransformer202111231\Symfony\Component\Cache\Adapter\ApcuAdapter::isSupported())) {
            return $opcache;
        }
        if (\in_array(\PHP_SAPI, ['cli', 'phpdbg'], \true) && !\filter_var(\ini_get('apc.enable_cli'), \FILTER_VALIDATE_BOOLEAN)) {
            return $opcache;
        }
        $apcu = new \ConfigTransformer202111231\Symfony\Component\Cache\Adapter\ApcuAdapter($namespace, \intdiv($defaultLifetime, 5), $version);
        if (null !== $logger) {
            $apcu->setLogger($logger);
        }
        return new \ConfigTransformer202111231\Symfony\Component\Cache\Adapter\ChainAdapter([$apcu, $opcache]);
    }
    public static function createConnection(string $dsn, array $options = [])
    {
        if (\str_starts_with($dsn, 'redis:') || \str_starts_with($dsn, 'rediss:')) {
            return \ConfigTransformer202111231\Symfony\Component\Cache\Adapter\RedisAdapter::createConnection($dsn, $options);
        }
        if (\str_starts_with($dsn, 'memcached:')) {
            return \ConfigTransformer202111231\Symfony\Component\Cache\Adapter\MemcachedAdapter::createConnection($dsn, $options);
        }
        if (0 === \strpos($dsn, 'couchbase:')) {
            return \ConfigTransformer202111231\Symfony\Component\Cache\Adapter\CouchbaseBucketAdapter::createConnection($dsn, $options);
        }
        throw new \ConfigTransformer202111231\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Unsupported DSN: "%s".', $dsn));
    }
    /**
     * {@inheritdoc}
     *
