...
class TraceableAdapter implements \ConfigTransformer2021120210\Symfony\Component\Cache\Adapter\AdapterInterface, \ConfigTransformer2021120210\Symfony\Contracts\Cache\CacheInterface, \ConfigTransformer2021120210\Symfony\Component\Cache\PruneableInterface, \ConfigTransformer2021120210\Symfony\Component\Cache\ResettableInterface
{
    ...
    public function get(string $key, callable $callback, float $beta = null, array &$metadata = null) : mixed
    {
        if (!$this->pool instanceof \ConfigTransformer2021120210\Symfony\Contracts\Cache\CacheInterface) {
            throw new \BadMethodCallException(\sprintf('Cannot call "%s::get()": this class doesn\'t implement "%s".', \get_debug_type($this->pool), \ConfigTransformer2021120210\Symfony\Contracts\Cache\CacheInterface::class));
        }
        $isHit = \true;
        $callback = function (\ConfigTransformer2021120210\Symfony\Component\Cache\CacheItem $item, bool &$save) use($callback, &$isHit) {
            $isHit = $item->isHit();
    ...
    {
        if (!$this->pool instanceof \ConfigTransformer2021120210\Symfony\Component\Cache\PruneableInterface) {
            return \false;
        }
        $event = $this->start(__FUNCTION__);
        try {
    ...
    {
        if ($this->pool instanceof \ConfigTransformer2021120210\Symfony\Contracts\Service\ResetInterface) {
            $this->pool->reset();
        }
        $this->clearCalls();
    }
    ...
    {
        $this->calls[] = $event = new \ConfigTransformer2021120210\Symfony\Component\Cache\Adapter\TraceableAdapterEvent();
        $event->name = $name;
