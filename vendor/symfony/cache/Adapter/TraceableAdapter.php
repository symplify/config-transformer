...
class TraceableAdapter implements \ConfigTransformer202203065\Symfony\Component\Cache\Adapter\AdapterInterface, \ConfigTransformer202203065\Symfony\Contracts\Cache\CacheInterface, \ConfigTransformer202203065\Symfony\Component\Cache\PruneableInterface, \ConfigTransformer202203065\Symfony\Component\Cache\ResettableInterface
{
    ...
    {
        if (!$this->pool instanceof \ConfigTransformer202203065\Symfony\Contracts\Cache\CacheInterface) {
            throw new \BadMethodCallException(\sprintf('Cannot call "%s::get()": this class doesn\'t implement "%s".', \get_debug_type($this->pool), \ConfigTransformer202203065\Symfony\Contracts\Cache\CacheInterface::class));
        }
        $isHit = \true;
        $callback = function (\ConfigTransformer202203065\Symfony\Component\Cache\CacheItem $item, bool &$save) use($callback, &$isHit) {
            $isHit = $item->isHit();
    ...
    public function clear(string $prefix = '') : bool
    {
        $event = $this->start(__FUNCTION__);
        try {
            if ($this->pool instanceof \ConfigTransformer202203065\Symfony\Component\Cache\Adapter\AdapterInterface) {
    ...
    {
        if (!$this->pool instanceof \ConfigTransformer202203065\Symfony\Component\Cache\PruneableInterface) {
            return \false;
        }
        $event = $this->start(__FUNCTION__);
        try {
    ...
    {
        if ($this->pool instanceof \ConfigTransformer202203065\Symfony\Contracts\Service\ResetInterface) {
            $this->pool->reset();
        }
        $this->clearCalls();
    }
    ...
    {
        $this->calls[] = $event = new \ConfigTransformer202203065\Symfony\Component\Cache\Adapter\TraceableAdapterEvent();
        $event->name = $name;
