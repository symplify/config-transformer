...
class ChainAdapter implements \ConfigTransformer2021113010\Symfony\Component\Cache\Adapter\AdapterInterface, \ConfigTransformer2021113010\Symfony\Contracts\Cache\CacheInterface, \ConfigTransformer2021113010\Symfony\Component\Cache\PruneableInterface, \ConfigTransformer2021113010\Symfony\Component\Cache\ResettableInterface
{
    ...
    {
        if (!$adapters) {
            throw new \ConfigTransformer2021113010\Symfony\Component\Cache\Exception\InvalidArgumentException('At least one adapter must be specified.');
        }
        foreach ($adapters as $adapter) {
            if (!$adapter instanceof \ConfigTransformer2021113010\Psr\Cache\CacheItemPoolInterface) {
                throw new \ConfigTransformer2021113010\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('The class "%s" does not implement the "%s" interface.', \get_debug_type($adapter), \ConfigTransformer2021113010\Psr\Cache\CacheItemPoolInterface::class));
            }
            if (\in_array(\PHP_SAPI, ['cli', 'phpdbg'], \true) && $adapter instanceof \ConfigTransformer2021113010\Symfony\Component\Cache\Adapter\ApcuAdapter && !\filter_var(\ini_get('apc.enable_cli'), \FILTER_VALIDATE_BOOLEAN)) {
                continue;
                // skip putting APCu in the chain when the backend is disabled
            }
            if ($adapter instanceof \ConfigTransformer2021113010\Symfony\Component\Cache\Adapter\AdapterInterface) {
                $this->adapters[] = $adapter;
            } else {
                $this->adapters[] = new \ConfigTransformer2021113010\Symfony\Component\Cache\Adapter\ProxyAdapter($adapter);
            }
        }
        ...
        self::$syncItem ?? (self::$syncItem = \Closure::bind(static function ($sourceItem, $item, $defaultLifetime, $sourceMetadata = null) {
            ...
            unset($sourceMetadata[\ConfigTransformer2021113010\Symfony\Component\Cache\CacheItem::METADATA_TAGS]);
            $item->value = $sourceItem->value;
            $item->isHit = $sourceItem->isHit;
            $item->metadata = $item->newMetadata = $sourceItem->metadata = $sourceMetadata;
            if (isset($item->metadata[\ConfigTransformer2021113010\Symfony\Component\Cache\CacheItem::METADATA_EXPIRY])) {
                $item->expiresAt(\DateTime::createFromFormat('U.u', \sprintf('%.6F', $item->metadata[\ConfigTransformer2021113010\Symfony\Component\Cache\CacheItem::METADATA_EXPIRY])));
    ...
    public function get(string $key, callable $callback, float $beta = null, array &$metadata = null) : mixed
    {
        $lastItem = null;
        $i = 0;
        $wrap = function (\ConfigTransformer2021113010\Symfony\Component\Cache\CacheItem $item = null) use($key, $callback, $beta, &$wrap, &$i, &$lastItem, &$metadata) {
            $adapter = $this->adapters[$i];
            if (isset($this->adapters[++$i])) {
                $callback = $wrap;
                $beta = \INF === $beta ? \INF : 0;
            }
            if ($adapter instanceof \ConfigTransformer2021113010\Symfony\Contracts\Cache\CacheInterface) {
    ...
    public function clear(string $prefix = '') : bool
    {
        ...
        $i = $this->adapterCount;
        while ($i--) {
            if ($this->adapters[$i] instanceof \ConfigTransformer2021113010\Symfony\Component\Cache\Adapter\AdapterInterface) {
    ...
    public function prune() : bool
    {
        $pruned = \true;
        foreach ($this->adapters as $adapter) {
            if ($adapter instanceof \ConfigTransformer2021113010\Symfony\Component\Cache\PruneableInterface) {
    ...
    {
        foreach ($this->adapters as $adapter) {
            if ($adapter instanceof \ConfigTransformer2021113010\Symfony\Contracts\Service\ResetInterface) {
                $adapter->reset();
            }
        }
    }
