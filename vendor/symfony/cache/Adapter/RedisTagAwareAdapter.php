...
use ConfigTransformerPrefix202301\Predis\Connection\Aggregate\ClusterInterface;
use ConfigTransformerPrefix202301\Predis\Connection\Aggregate\PredisCluster;
use ConfigTransformerPrefix202301\Predis\Connection\Aggregate\ReplicationInterface;
use ConfigTransformerPrefix202301\Predis\Response\ErrorInterface;
use ConfigTransformerPrefix202301\Predis\Response\Status;
...
use ConfigTransformerPrefix202301\Symfony\Component\Cache\Exception\LogicException;
use ConfigTransformerPrefix202301\Symfony\Component\Cache\Marshaller\DeflateMarshaller;
use ConfigTransformerPrefix202301\Symfony\Component\Cache\Marshaller\MarshallerInterface;
use ConfigTransformerPrefix202301\Symfony\Component\Cache\Marshaller\TagAwareMarshaller;
use ConfigTransformerPrefix202301\Symfony\Component\Cache\Traits\RedisTrait;
/**
...
class RedisTagAwareAdapter extends AbstractTagAwareAdapter
{
    ...
    {
        if ($redis instanceof \ConfigTransformerPrefix202301\Predis\ClientInterface && $redis->getConnection() instanceof ClusterInterface && !$redis->getConnection() instanceof PredisCluster) {
...
EOLUA;
        $results = $this->pipeline(function () use($ids, $lua) {
            foreach ($ids as $id) {
                (yield 'eval' => $this->redis instanceof \ConfigTransformerPrefix202301\Predis\ClientInterface ? [$lua, 1, $id] : [$lua, [$id], 1]);
            }
        });
        foreach ($results as $id => $result) {
...
EOLUA;
        $results = $this->pipeline(function () use($tagIds, $lua) {
            if ($this->redis instanceof \ConfigTransformerPrefix202301\Predis\ClientInterface) {
                $prefix = $this->redis->getOptions()->prefix ? $this->redis->getOptions()->prefix->getPrefix() : '';
            } elseif (\is_array($prefix = $this->redis->getOption(\Redis::OPT_PREFIX) ?? '')) {
                $prefix = \current($prefix);
            }
            foreach ($tagIds as $id) {
                (yield 'eval' => $this->redis instanceof \ConfigTransformerPrefix202301\Predis\ClientInterface ? [$lua, 1, $id, $prefix] : [$lua, [$id, $prefix], 1]);
            }
        });
        $lua = <<<'EOLUA'
            redis.replicate_commands()
...
EOLUA;
    ...
    private function getRedisEvictionPolicy() : string
    {
        ...
        $hosts = $this->getHosts();
        $host = \reset($hosts);
        if ($host instanceof \ConfigTransformerPrefix202301\Predis\Client && $host->getConnection() instanceof ReplicationInterface) {
