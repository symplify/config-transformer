...
use ConfigTransformerPrefix202312\Predis\Connection\Aggregate\ClusterInterface;
use ConfigTransformerPrefix202312\Predis\Connection\Aggregate\PredisCluster;
use ConfigTransformerPrefix202312\Predis\Connection\Aggregate\ReplicationInterface;
use ConfigTransformerPrefix202312\Predis\Response\ErrorInterface;
use ConfigTransformerPrefix202312\Predis\Response\Status;
use ConfigTransformerPrefix202312\Relay\Relay;
use ConfigTransformerPrefix202312\Symfony\Component\Cache\CacheItem;
use ConfigTransformerPrefix202312\Symfony\Component\Cache\Exception\InvalidArgumentException;
use ConfigTransformerPrefix202312\Symfony\Component\Cache\Exception\LogicException;
use ConfigTransformerPrefix202312\Symfony\Component\Cache\Marshaller\DeflateMarshaller;
use ConfigTransformerPrefix202312\Symfony\Component\Cache\Marshaller\MarshallerInterface;
use ConfigTransformerPrefix202312\Symfony\Component\Cache\Marshaller\TagAwareMarshaller;
...
class RedisTagAwareAdapter extends AbstractTagAwareAdapter
{
    ...
    {
        if ($redis instanceof \ConfigTransformerPrefix202312\Predis\ClientInterface && $redis->getConnection() instanceof ClusterInterface && !$redis->getConnection() instanceof PredisCluster) {
...
EOLUA;
        $results = $this->pipeline(function () use($ids, $lua) {
            foreach ($ids as $id) {
                (yield 'eval' => $this->redis instanceof \ConfigTransformerPrefix202312\Predis\ClientInterface ? [$lua, 1, $id] : [$lua, [$id], 1]);
            }
        });
        foreach ($results as $id => $result) {
            if ($result instanceof \RedisException || $result instanceof \ConfigTransformerPrefix202312\Relay\Exception || $result instanceof ErrorInterface) {
...
EOLUA;
        $results = $this->pipeline(function () use($tagIds, $lua) {
            if ($this->redis instanceof \ConfigTransformerPrefix202312\Predis\ClientInterface) {
            ...
            foreach ($tagIds as $id) {
                (yield 'eval' => $this->redis instanceof \ConfigTransformerPrefix202312\Predis\ClientInterface ? [$lua, 1, $id, $prefix] : [$lua, [$id, $prefix], 1]);
            }
        });
        $lua = <<<'EOLUA'
            redis.replicate_commands()
...
EOLUA;
        ...
        foreach ($results as $id => $values) {
            if ($values instanceof \RedisException || $values instanceof \ConfigTransformerPrefix202312\Relay\Exception || $values instanceof ErrorInterface) {
    ...
    private function getRedisEvictionPolicy() : string
    {
        ...
        $hosts = $this->getHosts();
        $host = \reset($hosts);
        if ($host instanceof \ConfigTransformerPrefix202312\Predis\Client && $host->getConnection() instanceof ReplicationInterface) {
