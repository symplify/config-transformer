...
use ConfigTransformer202107276\Predis\Connection\Aggregate\ClusterInterface;
use ConfigTransformer202107276\Predis\Connection\Aggregate\PredisCluster;
use ConfigTransformer202107276\Predis\Connection\Aggregate\ReplicationInterface;
use ConfigTransformer202107276\Predis\Response\Status;
use ConfigTransformer202107276\Symfony\Component\Cache\Exception\InvalidArgumentException;
use ConfigTransformer202107276\Symfony\Component\Cache\Exception\LogicException;
use ConfigTransformer202107276\Symfony\Component\Cache\Marshaller\DeflateMarshaller;
use ConfigTransformer202107276\Symfony\Component\Cache\Marshaller\MarshallerInterface;
use ConfigTransformer202107276\Symfony\Component\Cache\Marshaller\TagAwareMarshaller;
 ...
 */
class RedisTagAwareAdapter extends \ConfigTransformer202107276\Symfony\Component\Cache\Adapter\AbstractTagAwareAdapter
{
    use RedisTrait;
    /**
    ...
    {
        if ($redis instanceof \ConfigTransformer202107276\Predis\ClientInterface && $redis->getConnection() instanceof \ConfigTransformer202107276\Predis\Connection\Aggregate\ClusterInterface && !$redis->getConnection() instanceof \ConfigTransformer202107276\Predis\Connection\Aggregate\PredisCluster) {
            throw new \ConfigTransformer202107276\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Unsupported Predis cluster connection: only "%s" is, "%s" given.', \ConfigTransformer202107276\Predis\Connection\Aggregate\PredisCluster::class, \get_debug_type($redis->getConnection())));
        }
        if (\defined('Redis::OPT_COMPRESSION') && ($redis instanceof \Redis || $redis instanceof \RedisArray || $redis instanceof \RedisCluster)) {
            ...
            foreach (\is_array($compression) ? $compression : [$compression] as $c) {
                if (\Redis::COMPRESSION_NONE !== $c) {
                    throw new \ConfigTransformer202107276\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('phpredis compression must be disabled when using "%s", use "%s" instead.', static::class, \ConfigTransformer202107276\Symfony\Component\Cache\Marshaller\DeflateMarshaller::class));
                }
            }
        }
        $this->init($redis, $namespace, $defaultLifetime, new \ConfigTransformer202107276\Symfony\Component\Cache\Marshaller\TagAwareMarshaller($marshaller));
    }
    /**
     * {@inheritdoc}
     * @param mixed[] $values
    ...
    protected function doSave($values, $lifetime, $addTagData = [], $delTagData = []) : array
    {
        ...
        if ('noeviction' !== $eviction && \strncmp($eviction, 'volatile-', \strlen('volatile-')) !== 0) {
            throw new \ConfigTransformer202107276\Symfony\Component\Cache\Exception\LogicException(\sprintf('Redis maxmemory-policy setting "%s" is *not* supported by RedisTagAwareAdapter, use "noeviction" or "volatile-*" eviction policies.', $eviction));
        }
        // serialize values
        ...
        foreach ($results as $id => $result) {
            ...
            // setEx results
            if (\true !== $result && (!$result instanceof \ConfigTransformer202107276\Predis\Response\Status || \ConfigTransformer202107276\Predis\Response\Status::get('OK') !== $result)) {
                $failed[] = $id;
            }
        }
        return $failed;
...
EOLUA;
        $results = $this->pipeline(function () use($ids, $lua) {
            foreach ($ids as $id) {
                (yield 'eval' => $this->redis instanceof \ConfigTransformer202107276\Predis\ClientInterface ? [$lua, 1, $id] : [$lua, [$id], 1]);
            }
        });
        foreach ($results as $id => $result) {
            try {
...
EOLUA;
        $results = $this->pipeline(function () use($tagIds, $lua) {
            if ($this->redis instanceof \ConfigTransformer202107276\Predis\ClientInterface) {
                $prefix = $this->redis->getOptions()->prefix ? $this->redis->getOptions()->prefix->getPrefix() : '';
            } elseif (\is_array($prefix = $this->redis->getOption(\Redis::OPT_PREFIX) ?? '')) {
                $prefix = \current($prefix);
            }
            foreach ($tagIds as $id) {
                (yield 'eval' => $this->redis instanceof \ConfigTransformer202107276\Predis\ClientInterface ? [$lua, 1, $id, $prefix] : [$lua, [$id, $prefix], 1]);
            }
        });
        $lua = <<<'EOLUA'
            local id = KEYS[1]
...
EOLUA;
    ...
    private function getRedisEvictionPolicy() : string
    {
        ...
        $hosts = $this->getHosts();
        $host = \reset($hosts);
        if ($host instanceof \ConfigTransformer202107276\Predis\Client && $host->getConnection() instanceof \ConfigTransformer202107276\Predis\Connection\Aggregate\ReplicationInterface) {
