...
use ConfigTransformer202106280\Predis\Connection\Aggregate\ClusterInterface;
use ConfigTransformer202106280\Predis\Connection\Aggregate\PredisCluster;
use ConfigTransformer202106280\Predis\Connection\Aggregate\ReplicationInterface;
use ConfigTransformer202106280\Predis\Response\Status;
use ConfigTransformer202106280\Symfony\Component\Cache\Exception\InvalidArgumentException;
use ConfigTransformer202106280\Symfony\Component\Cache\Exception\LogicException;
use ConfigTransformer202106280\Symfony\Component\Cache\Marshaller\DeflateMarshaller;
use ConfigTransformer202106280\Symfony\Component\Cache\Marshaller\MarshallerInterface;
use ConfigTransformer202106280\Symfony\Component\Cache\Marshaller\TagAwareMarshaller;
use ConfigTransformer202106280\Symfony\Component\Cache\Traits\RedisTrait;
/**
 ...
 */
class RedisTagAwareAdapter extends \ConfigTransformer202106280\Symfony\Component\Cache\Adapter\AbstractTagAwareAdapter
{
    use RedisTrait;
    /**
     ...
     */
    public function __construct($redisClient, string $namespace = '', int $defaultLifetime = 0, \ConfigTransformer202106280\Symfony\Component\Cache\Marshaller\MarshallerInterface $marshaller = null)
    {
        if ($redisClient instanceof \ConfigTransformer202106280\Predis\ClientInterface && $redisClient->getConnection() instanceof \ConfigTransformer202106280\Predis\Connection\Aggregate\ClusterInterface && !$redisClient->getConnection() instanceof \ConfigTransformer202106280\Predis\Connection\Aggregate\PredisCluster) {
            throw new \ConfigTransformer202106280\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Unsupported Predis cluster connection: only "%s" is, "%s" given.', \ConfigTransformer202106280\Predis\Connection\Aggregate\PredisCluster::class, \get_debug_type($redisClient->getConnection())));
        }
        if (\defined('Redis::OPT_COMPRESSION') && ($redisClient instanceof \Redis || $redisClient instanceof \RedisArray || $redisClient instanceof \RedisCluster)) {
            ...
            foreach (\is_array($compression) ? $compression : [$compression] as $c) {
                if (\Redis::COMPRESSION_NONE !== $c) {
                    throw new \ConfigTransformer202106280\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('phpredis compression must be disabled when using "%s", use "%s" instead.', static::class, \ConfigTransformer202106280\Symfony\Component\Cache\Marshaller\DeflateMarshaller::class));
                }
            }
        }
        $this->init($redisClient, $namespace, $defaultLifetime, new \ConfigTransformer202106280\Symfony\Component\Cache\Marshaller\TagAwareMarshaller($marshaller));
    }
    /**
     * {@inheritdoc}
    ...
    protected function doSave(array $values, int $lifetime, $addTagData = [], $delTagData = []) : array
    {
        ...
        if ('noeviction' !== $eviction && 0 !== \strpos($eviction, 'volatile-')) {
            throw new \ConfigTransformer202106280\Symfony\Component\Cache\Exception\LogicException(\sprintf('Redis maxmemory-policy setting "%s" is *not* supported by RedisTagAwareAdapter, use "noeviction" or  "volatile-*" eviction policies.', $eviction));
        }
        // serialize values
        ...
        foreach ($results as $id => $result) {
            ...
            // setEx results
            if (\true !== $result && (!$result instanceof \ConfigTransformer202106280\Predis\Response\Status || \ConfigTransformer202106280\Predis\Response\Status::get('OK') !== $result)) {
                $failed[] = $id;
            }
        }
        return $failed;
...
EOLUA;
        if ($this->redis instanceof \ConfigTransformer202106280\Predis\ClientInterface) {
            $evalArgs = [$lua, 1, &$id];
        } else {
            $evalArgs = [$lua, [&$id], 1];
        }
    ...
    {
        if (!$this->redis instanceof \ConfigTransformer202106280\Predis\ClientInterface || !$this->redis->getConnection() instanceof \ConfigTransformer202106280\Predis\Connection\Aggregate\PredisCluster) {
    ...
    private function getRedisEvictionPolicy() : string
    {
        ...
        $hosts = $this->getHosts();
        $host = \reset($hosts);
        if ($host instanceof \ConfigTransformer202106280\Predis\Client && $host->getConnection() instanceof \ConfigTransformer202106280\Predis\Connection\Aggregate\ReplicationInterface) {
