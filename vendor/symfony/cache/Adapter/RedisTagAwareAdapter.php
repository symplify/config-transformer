...
use ConfigTransformer20220607\Predis\Connection\Aggregate\ClusterInterface;
use ConfigTransformer20220607\Predis\Connection\Aggregate\PredisCluster;
use ConfigTransformer20220607\Predis\Connection\Aggregate\ReplicationInterface;
use ConfigTransformer20220607\Predis\Response\ErrorInterface;
use ConfigTransformer20220607\Predis\Response\Status;
use ConfigTransformer20220607\Symfony\Component\Cache\CacheItem;
use ConfigTransformer20220607\Symfony\Component\Cache\Exception\InvalidArgumentException;
use ConfigTransformer20220607\Symfony\Component\Cache\Exception\LogicException;
use ConfigTransformer20220607\Symfony\Component\Cache\Marshaller\DeflateMarshaller;
use ConfigTransformer20220607\Symfony\Component\Cache\Marshaller\MarshallerInterface;
use ConfigTransformer20220607\Symfony\Component\Cache\Marshaller\TagAwareMarshaller;
use ConfigTransformer20220607\Symfony\Component\Cache\Traits\RedisClusterProxy;
use ConfigTransformer20220607\Symfony\Component\Cache\Traits\RedisProxy;
use ConfigTransformer20220607\Symfony\Component\Cache\Traits\RedisTrait;
/**
...
class RedisTagAwareAdapter extends AbstractTagAwareAdapter
{
    ...
    public function __construct(\Redis|\RedisArray|\RedisCluster|\ConfigTransformer20220607\Predis\ClientInterface|RedisProxy|RedisClusterProxy $redis, string $namespace = '', int $defaultLifetime = 0, MarshallerInterface $marshaller = null)
    {
        if ($redis instanceof \ConfigTransformer20220607\Predis\ClientInterface && $redis->getConnection() instanceof ClusterInterface && !$redis->getConnection() instanceof PredisCluster) {
...
EOLUA;
        $results = $this->pipeline(function () use($ids, $lua) {
            foreach ($ids as $id) {
                (yield 'eval' => $this->redis instanceof \ConfigTransformer20220607\Predis\ClientInterface ? [$lua, 1, $id] : [$lua, [$id], 1]);
            }
        });
        foreach ($results as $id => $result) {
...
EOLUA;
        $results = $this->pipeline(function () use($tagIds, $lua) {
            if ($this->redis instanceof \ConfigTransformer20220607\Predis\ClientInterface) {
                $prefix = $this->redis->getOptions()->prefix ? $this->redis->getOptions()->prefix->getPrefix() : '';
            } elseif (\is_array($prefix = $this->redis->getOption(\Redis::OPT_PREFIX) ?? '')) {
                $prefix = \current($prefix);
            }
            foreach ($tagIds as $id) {
                (yield 'eval' => $this->redis instanceof \ConfigTransformer20220607\Predis\ClientInterface ? [$lua, 1, $id, $prefix] : [$lua, [$id, $prefix], 1]);
            }
        });
        $lua = <<<'EOLUA'
            redis.replicate_commands()
...
EOLUA;
    ...
    private function getRedisEvictionPolicy() : string
    {
        ...
        $hosts = $this->getHosts();
        $host = \reset($hosts);
        if ($host instanceof \ConfigTransformer20220607\Predis\Client && $host->getConnection() instanceof ReplicationInterface) {
