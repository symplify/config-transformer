...
use ConfigTransformerPrefix202501\Predis\Connection\Aggregate\ClusterInterface;
use ConfigTransformerPrefix202501\Predis\Connection\Aggregate\PredisCluster;
use ConfigTransformerPrefix202501\Predis\Connection\Aggregate\ReplicationInterface;
use ConfigTransformerPrefix202501\Predis\Response\ErrorInterface;
use ConfigTransformerPrefix202501\Predis\Response\Status;
use ConfigTransformerPrefix202501\Relay\Relay;
...
use ConfigTransformerPrefix202501\Symfony\Component\Cache\Marshaller\DeflateMarshaller;
use ConfigTransformerPrefix202501\Symfony\Component\Cache\Marshaller\MarshallerInterface;
use ConfigTransformerPrefix202501\Symfony\Component\Cache\Marshaller\TagAwareMarshaller;
use ConfigTransformerPrefix202501\Symfony\Component\Cache\Traits\RedisTrait;
/**
...
class RedisTagAwareAdapter extends AbstractTagAwareAdapter
{
    ...
    {
        if ($redis instanceof \ConfigTransformerPrefix202501\Predis\ClientInterface && $redis->getConnection() instanceof ClusterInterface && !$redis->getConnection() instanceof PredisCluster) {
...
EOLUA;
        $results = $this->pipeline(function () use($ids, $lua) {
            foreach ($ids as $id) {
                (yield 'eval' => $this->redis instanceof \ConfigTransformerPrefix202501\Predis\ClientInterface ? [$lua, 1, $id] : [$lua, [$id], 1]);
            }
        });
        foreach ($results as $id => $result) {
            if ($result instanceof \RedisException || $result instanceof \ConfigTransformerPrefix202501\Relay\Exception || $result instanceof ErrorInterface) {
...
EOLUA;
        $results = $this->pipeline(function () use($tagIds, $lua) {
            if ($this->redis instanceof \ConfigTransformerPrefix202501\Predis\ClientInterface) {
            ...
            foreach ($tagIds as $id) {
                (yield 'eval' => $this->redis instanceof \ConfigTransformerPrefix202501\Predis\ClientInterface ? [$lua, 1, $id, $prefix] : [$lua, [$id, $prefix], 1]);
            }
        });
        $lua = <<<'EOLUA'
            redis.replicate_commands()
...
EOLUA;
        ...
        foreach ($results as $id => $values) {
            if ($values instanceof \RedisException || $values instanceof \ConfigTransformerPrefix202501\Relay\Exception || $values instanceof ErrorInterface) {
    ...
    private function getRedisEvictionPolicy() : string
    {
        ...
        $hosts = $this->getHosts();
        $host = \reset($hosts);
        if ($host instanceof \ConfigTransformerPrefix202501\Predis\Client && $host->getConnection() instanceof ReplicationInterface) {
