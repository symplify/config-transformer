...
class PdoAdapter extends \ConfigTransformer202112169\Symfony\Component\Cache\Adapter\AbstractAdapter implements \ConfigTransformer202112169\Symfony\Component\Cache\PruneableInterface
{
     ...
     */
    public function __construct(\PDO|string $connOrDsn, string $namespace = '', int $defaultLifetime = 0, array $options = [], \ConfigTransformer202112169\Symfony\Component\Cache\Marshaller\MarshallerInterface $marshaller = null)
    {
        if (\is_string($connOrDsn) && \str_contains($connOrDsn, '://')) {
            throw new \ConfigTransformer202112169\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Usage of Doctrine DBAL URL with "%s" is not supported. Use a PDO DSN or "%s" instead. Got "%s".', __CLASS__, \ConfigTransformer202112169\Symfony\Component\Cache\Adapter\DoctrineDbalAdapter::class, $connOrDsn));
        }
        ...
        if ($connOrDsn instanceof \PDO) {
            if (\PDO::ERRMODE_EXCEPTION !== $connOrDsn->getAttribute(\PDO::ATTR_ERRMODE)) {
                throw new \ConfigTransformer202112169\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('"%s" requires PDO error mode attribute be set to throw Exceptions (i.e. $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION)).', __CLASS__));
            }
            $this->conn = $connOrDsn;
        } else {
            $this->dsn = $connOrDsn;
