 ...
 */
class TagAwareAdapter implements \ConfigTransformer202111303\Symfony\Component\Cache\Adapter\TagAwareAdapterInterface, \ConfigTransformer202111303\Symfony\Contracts\Cache\TagAwareCacheInterface, \ConfigTransformer202111303\Symfony\Component\Cache\PruneableInterface, \ConfigTransformer202111303\Symfony\Component\Cache\ResettableInterface, \ConfigTransformer202111303\Psr\Log\LoggerAwareInterface
{
    use ContractsTrait;
    ...
    private \ConfigTransformer202111303\Symfony\Component\Cache\Adapter\AdapterInterface $tags;
    ...
    public function __construct(\ConfigTransformer202111303\Symfony\Component\Cache\Adapter\AdapterInterface $itemsPool, \ConfigTransformer202111303\Symfony\Component\Cache\Adapter\AdapterInterface $tagsPool = null, float $knownTagVersionsTtl = 0.15)
    {
        $this->pool = $itemsPool;
        $this->tags = $tagsPool ?? $itemsPool;
        $this->knownTagVersionsTtl = $knownTagVersionsTtl;
        self::$createCacheItem ?? (self::$createCacheItem = \Closure::bind(static function ($key, $value, \ConfigTransformer202111303\Symfony\Component\Cache\CacheItem $protoItem) {
        ...
        self::$setCacheItemTags ?? (self::$setCacheItemTags = \Closure::bind(static function (\ConfigTransformer202111303\Symfony\Component\Cache\CacheItem $item, $key, array &$itemTags) {
            $item->isTaggable = \true;
            if (!$item->isHit) {
                return $item;
            }
            if (isset($itemTags[$key])) {
                foreach ($itemTags[$key] as $tag => $version) {
                    $item->metadata[\ConfigTransformer202111303\Symfony\Component\Cache\CacheItem::METADATA_TAGS][$tag] = $tag;
                }
                unset($itemTags[$key]);
            } else {
                $item->value = null;
        ...
        self::$getTagsByKey ?? (self::$getTagsByKey = \Closure::bind(static function ($deferred) {
            ...
            foreach ($deferred as $key => $item) {
                $tagsByKey[$key] = $item->newMetadata[\ConfigTransformer202111303\Symfony\Component\Cache\CacheItem::METADATA_TAGS] ?? [];
                $item->metadata = $item->newMetadata;
            }
            return $tagsByKey;
        }, null, \ConfigTransformer202111303\Symfony\Component\Cache\CacheItem::class));
        self::$saveTags ?? (self::$saveTags = \Closure::bind(static function (\ConfigTransformer202111303\Symfony\Component\Cache\Adapter\AdapterInterface $tagsAdapter, array $tags) {
            \ksort($tags);
            foreach ($tags as $v) {
                $v->expiry = 0;
    ...
    public function invalidateTags(array $tags) : bool
    {
        $ids = [];
        foreach ($tags as $tag) {
            \assert('' !== \ConfigTransformer202111303\Symfony\Component\Cache\CacheItem::validateKey($tag));
    ...
    public function getItems(array $keys = []) : iterable
    {
        ...
        } catch (\ConfigTransformer202111303\Psr\Cache\InvalidArgumentException $e) {
