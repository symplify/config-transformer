 ...
 */
class TagAwareAdapter implements \ConfigTransformer202109186\Symfony\Component\Cache\Adapter\TagAwareAdapterInterface, \ConfigTransformer202109186\Symfony\Contracts\Cache\TagAwareCacheInterface, \ConfigTransformer202109186\Symfony\Component\Cache\PruneableInterface, \ConfigTransformer202109186\Symfony\Component\Cache\ResettableInterface, \ConfigTransformer202109186\Psr\Log\LoggerAwareInterface
{
    use ContractsTrait;
    ...
    public function __construct(\ConfigTransformer202109186\Symfony\Component\Cache\Adapter\AdapterInterface $itemsPool, \ConfigTransformer202109186\Symfony\Component\Cache\Adapter\AdapterInterface $tagsPool = null, float $knownTagVersionsTtl = 0.15)
    {
        $this->pool = $itemsPool;
        $this->tags = $tagsPool ?: $itemsPool;
        $this->knownTagVersionsTtl = $knownTagVersionsTtl;
        self::$createCacheItem ?? (self::$createCacheItem = \Closure::bind(static function ($key, $value, \ConfigTransformer202109186\Symfony\Component\Cache\CacheItem $protoItem) {
        ...
        self::$setCacheItemTags ?? (self::$setCacheItemTags = \Closure::bind(static function (\ConfigTransformer202109186\Symfony\Component\Cache\CacheItem $item, $key, array &$itemTags) {
            $item->isTaggable = \true;
            if (!$item->isHit) {
                return $item;
            }
            if (isset($itemTags[$key])) {
                foreach ($itemTags[$key] as $tag => $version) {
                    $item->metadata[\ConfigTransformer202109186\Symfony\Component\Cache\CacheItem::METADATA_TAGS][$tag] = $tag;
                }
                unset($itemTags[$key]);
            } else {
                $item->value = null;
        ...
        self::$getTagsByKey ?? (self::$getTagsByKey = \Closure::bind(static function ($deferred) {
            ...
            foreach ($deferred as $key => $item) {
                $tagsByKey[$key] = $item->newMetadata[\ConfigTransformer202109186\Symfony\Component\Cache\CacheItem::METADATA_TAGS] ?? [];
                $item->metadata = $item->newMetadata;
            }
            return $tagsByKey;
        }, null, \ConfigTransformer202109186\Symfony\Component\Cache\CacheItem::class));
        self::$invalidateTags ?? (self::$invalidateTags = \Closure::bind(static function (\ConfigTransformer202109186\Symfony\Component\Cache\Adapter\AdapterInterface $tagsAdapter, array $tags) {
            foreach ($tags as $v) {
                $v->expiry = 0;
    ...
    public function invalidateTags($tags)
    {
        ...
        $invalidatedTags = [];
        foreach ($tags as $tag) {
            \assert('' !== \ConfigTransformer202109186\Symfony\Component\Cache\CacheItem::validateKey($tag));
    ...
    public function getItems($keys = [])
    {
        ...
        } catch (\ConfigTransformer202109186\Psr\Cache\InvalidArgumentException $e) {
