 ...
 */
class TagAwareAdapter implements \ConfigTransformer2021120210\Symfony\Component\Cache\Adapter\TagAwareAdapterInterface, \ConfigTransformer2021120210\Symfony\Contracts\Cache\TagAwareCacheInterface, \ConfigTransformer2021120210\Symfony\Component\Cache\PruneableInterface, \ConfigTransformer2021120210\Symfony\Component\Cache\ResettableInterface, \ConfigTransformer2021120210\Psr\Log\LoggerAwareInterface
{
    use ContractsTrait;
    ...
    private \ConfigTransformer2021120210\Symfony\Component\Cache\Adapter\AdapterInterface $tags;
    ...
    public function __construct(\ConfigTransformer2021120210\Symfony\Component\Cache\Adapter\AdapterInterface $itemsPool, \ConfigTransformer2021120210\Symfony\Component\Cache\Adapter\AdapterInterface $tagsPool = null, float $knownTagVersionsTtl = 0.15)
    {
        $this->pool = $itemsPool;
        $this->tags = $tagsPool ?? $itemsPool;
        $this->knownTagVersionsTtl = $knownTagVersionsTtl;
        self::$createCacheItem ?? (self::$createCacheItem = \Closure::bind(static function ($key, $value, \ConfigTransformer2021120210\Symfony\Component\Cache\CacheItem $protoItem) {
        ...
        self::$setCacheItemTags ?? (self::$setCacheItemTags = \Closure::bind(static function (\ConfigTransformer2021120210\Symfony\Component\Cache\CacheItem $item, $key, array &$itemTags) {
            $item->isTaggable = \true;
            if (!$item->isHit) {
                return $item;
            }
            if (isset($itemTags[$key])) {
                foreach ($itemTags[$key] as $tag => $version) {
                    $item->metadata[\ConfigTransformer2021120210\Symfony\Component\Cache\CacheItem::METADATA_TAGS][$tag] = $tag;
                }
                unset($itemTags[$key]);
            } else {
                $item->value = null;
        ...
        self::$getTagsByKey ?? (self::$getTagsByKey = \Closure::bind(static function ($deferred) {
            ...
            foreach ($deferred as $key => $item) {
                $tagsByKey[$key] = $item->newMetadata[\ConfigTransformer2021120210\Symfony\Component\Cache\CacheItem::METADATA_TAGS] ?? [];
                $item->metadata = $item->newMetadata;
            }
            return $tagsByKey;
        }, null, \ConfigTransformer2021120210\Symfony\Component\Cache\CacheItem::class));
        self::$saveTags ?? (self::$saveTags = \Closure::bind(static function (\ConfigTransformer2021120210\Symfony\Component\Cache\Adapter\AdapterInterface $tagsAdapter, array $tags) {
            \ksort($tags);
            foreach ($tags as $v) {
                $v->expiry = 0;
    ...
    public function invalidateTags(array $tags) : bool
    {
        $ids = [];
        foreach ($tags as $tag) {
            \assert('' !== \ConfigTransformer2021120210\Symfony\Component\Cache\CacheItem::validateKey($tag));
    ...
    public function getItems(array $keys = []) : iterable
    {
        ...
        } catch (\ConfigTransformer2021120210\Psr\Cache\InvalidArgumentException $e) {
