...
use ConfigTransformer202205143\Couchbase\ClusterOptions;
use ConfigTransformer202205143\Couchbase\Collection;
use ConfigTransformer202205143\Couchbase\DocumentNotFoundException;
use ConfigTransformer202205143\Couchbase\UpsertOptions;
 ...
 */
class CouchbaseCollectionAdapter extends \ConfigTransformer202205143\Symfony\Component\Cache\Adapter\AbstractAdapter
{
    private const MAX_KEY_LENGTH = 250;
    private $connection;
    private $marshaller;
    public function __construct(\ConfigTransformer202205143\Couchbase\Collection $connection, string $namespace = '', int $defaultLifetime = 0, \ConfigTransformer202205143\Symfony\Component\Cache\Marshaller\MarshallerInterface $marshaller = null)
    {
        if (!static::isSupported()) {
            throw new \ConfigTransformer202205143\Symfony\Component\Cache\Exception\CacheException('Couchbase >= 3.0.0 < 4.0.0 is required.');
        }
        ...
        try {
            ...
            foreach ($dsn as $server) {
                if (0 !== \strpos($server, 'couchbase:')) {
                    throw new \ConfigTransformer202205143\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Invalid Couchbase DSN: "%s" does not start with "couchbase:".', $server));
                }
            ...
            $clusterOptions = new \ConfigTransformer202205143\Couchbase\ClusterOptions();
    ...
    protected function doFetch(array $ids) : array
    {
        ...
        foreach ($ids as $id) {
            ...
            } catch (\ConfigTransformer202205143\Couchbase\DocumentNotFoundException $exception) {
                continue;
            }
    ...
    protected function doSave(array $values, $lifetime) : array|bool
    {
        ...
        $upsertOptions = new \ConfigTransformer202205143\Couchbase\UpsertOptions();
