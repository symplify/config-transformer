...
use ConfigTransformer202206011\Couchbase\ClusterOptions;
use ConfigTransformer202206011\Couchbase\Collection;
use ConfigTransformer202206011\Couchbase\DocumentNotFoundException;
use ConfigTransformer202206011\Couchbase\UpsertOptions;
 ...
 */
class CouchbaseCollectionAdapter extends \ConfigTransformer202206011\Symfony\Component\Cache\Adapter\AbstractAdapter
{
    private const MAX_KEY_LENGTH = 250;
    private $connection;
    private $marshaller;
    public function __construct(\ConfigTransformer202206011\Couchbase\Collection $connection, string $namespace = '', int $defaultLifetime = 0, \ConfigTransformer202206011\Symfony\Component\Cache\Marshaller\MarshallerInterface $marshaller = null)
    {
        if (!static::isSupported()) {
            throw new \ConfigTransformer202206011\Symfony\Component\Cache\Exception\CacheException('Couchbase >= 3.0.0 < 4.0.0 is required.');
        }
        ...
        try {
            ...
            foreach ($dsn as $server) {
                if (0 !== \strpos($server, 'couchbase:')) {
                    throw new \ConfigTransformer202206011\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Invalid Couchbase DSN: "%s" does not start with "couchbase:".', $server));
                }
            ...
            $clusterOptions = new \ConfigTransformer202206011\Couchbase\ClusterOptions();
    ...
    protected function doFetch(array $ids) : array
    {
        ...
        foreach ($ids as $id) {
            ...
            } catch (\ConfigTransformer202206011\Couchbase\DocumentNotFoundException $exception) {
                continue;
            }
    ...
    protected function doSave(array $values, $lifetime) : array|bool
    {
        ...
        $upsertOptions = new \ConfigTransformer202206011\Couchbase\UpsertOptions();
