 ...
 */
namespace ConfigTransformer202206010\Symfony\Component\Cache\Traits;

use ConfigTransformer202206010\Psr\Cache\CacheItemInterface;
use ConfigTransformer202206010\Psr\Log\LoggerAwareTrait;
...
trait AbstractAdapterTrait
{
    ...
    public function hasItem(mixed $key) : bool
    {
        ...
        try {
            ...
            \ConfigTransformer202206010\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to check if key "{key}" is cached: ' . $e->getMessage(), ['key' => $key, 'exception' => $e, 'cache-adapter' => \get_debug_type($this)]);
            return \false;
        }
    }
    /**
    ...
    public function clear(string $prefix = '') : bool
    {
        ...
        if ($cleared = $this->versioningIsEnabled) {
            ...
            if (\true !== $e && [] !== $e) {
                ...
                \ConfigTransformer202206010\Symfony\Component\Cache\CacheItem::log($this->logger, $message, ['exception' => $e instanceof \Exception ? $e : null, 'cache-adapter' => \get_debug_type($this)]);
            } else {
        ...
        try {
            ...
            \ConfigTransformer202206010\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to clear the cache: ' . $e->getMessage(), ['exception' => $e, 'cache-adapter' => \get_debug_type($this)]);
            return \false;
        }
    }
    /**
    ...
    public function deleteItems(array $keys) : bool
    {
        ...
        foreach ($ids as $key => $id) {
            ...
            \ConfigTransformer202206010\Symfony\Component\Cache\CacheItem::log($this->logger, $message, ['key' => $key, 'exception' => $e, 'cache-adapter' => \get_debug_type($this)]);
            $ok = \false;
        }
        return $ok;
    }
    ...
    public function getItem(mixed $key) : \ConfigTransformer202206010\Symfony\Component\Cache\CacheItem
    {
        ...
        try {
            ...
            \ConfigTransformer202206010\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to fetch key "{key}": ' . $e->getMessage(), ['key' => $key, 'exception' => $e, 'cache-adapter' => \get_debug_type($this)]);
        }
    ...
    public function getItems(array $keys = []) : iterable
    {
        ...
        try {
            ...
            \ConfigTransformer202206010\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to fetch items: ' . $e->getMessage(), ['keys' => $keys, 'exception' => $e, 'cache-adapter' => \get_debug_type($this)]);
            $items = [];
        }
    ...
    {
        if (!$item instanceof \ConfigTransformer202206010\Symfony\Component\Cache\CacheItem) {
            return \false;
        }
    ...
    private function generateItems(iterable $items, array &$keys) : \Generator
    {
        ...
        try {
            foreach ($items as $id => $value) {
                if (!isset($keys[$id])) {
                    throw new \ConfigTransformer202206010\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Could not match value id "%s" to keys "%s".', $id, \implode('", "', $keys)));
                }
                $key = $keys[$id];
                unset($keys[$id]);
            ...
            \ConfigTransformer202206010\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to fetch items: ' . $e->getMessage(), ['keys' => \array_values($keys), 'exception' => $e, 'cache-adapter' => \get_debug_type($this)]);
        }
        foreach ($keys as $key) {
    ...
    private function getId(mixed $key)
    {
        ...
        \assert('' !== \ConfigTransformer202206010\Symfony\Component\Cache\CacheItem::validateKey($key));
        $this->ids[$key] = $key;
