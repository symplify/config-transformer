 ...
 */
namespace ConfigTransformer202201268\Symfony\Component\Cache\Traits;
...
trait AbstractAdapterTrait
{
    ...
    public function hasItem(mixed $key) : bool
    {
        ...
        try {
            ...
            \ConfigTransformer202201268\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to check if key "{key}" is cached: ' . $e->getMessage(), ['key' => $key, 'exception' => $e, 'cache-adapter' => \get_debug_type($this)]);
            return \false;
        }
    }
    /**
    ...
    public function clear(string $prefix = '') : bool
    {
        ...
        try {
            ...
            \ConfigTransformer202201268\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to clear the cache: ' . $e->getMessage(), ['exception' => $e, 'cache-adapter' => \get_debug_type($this)]);
            return \false;
        }
    }
    /**
    ...
    public function getItem(mixed $key) : \ConfigTransformer202201268\Symfony\Component\Cache\CacheItem
    {
        ...
        try {
            ...
            \ConfigTransformer202201268\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to fetch key "{key}": ' . $e->getMessage(), ['key' => $key, 'exception' => $e, 'cache-adapter' => \get_debug_type($this)]);
        }
    ...
    public function getItems(array $keys = []) : iterable
    {
        ...
        try {
            ...
            \ConfigTransformer202201268\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to fetch items: ' . $e->getMessage(), ['keys' => $keys, 'exception' => $e, 'cache-adapter' => \get_debug_type($this)]);
            $items = [];
        }
    ...
    private function generateItems(iterable $items, array &$keys) : \Generator
    {
        ...
        try {
            foreach ($items as $id => $value) {
                if (!isset($keys[$id])) {
                    throw new \ConfigTransformer202201268\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Could not match value id "%s" to keys "%s".', $id, \implode('", "', $keys)));
                }
                $key = $keys[$id];
                unset($keys[$id]);
            ...
            \ConfigTransformer202201268\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to fetch items: ' . $e->getMessage(), ['keys' => \array_values($keys), 'exception' => $e, 'cache-adapter' => \get_debug_type($this)]);
        }
        foreach ($keys as $key) {
