...
use ConfigTransformer20220612\Predis\Command\Redis\UNLINK;
use ConfigTransformer20220612\Predis\Connection\Aggregate\ClusterInterface;
use ConfigTransformer20220612\Predis\Connection\Aggregate\RedisCluster;
...
trait RedisTrait
{
    ...
    private $redis;
    private $marshaller;
    private function init(\Redis|\RedisArray|\RedisCluster|\ConfigTransformer20220612\Predis\ClientInterface|RedisProxy|RedisClusterProxy $redis, string $namespace, int $defaultLifetime, ?MarshallerInterface $marshaller)
    {
        parent::__construct($namespace, $defaultLifetime);
        if (\preg_match('#[^-+_.A-Za-z0-9]#', $namespace, $match)) {
            throw new InvalidArgumentException(\sprintf('RedisAdapter namespace contains "%s" but only characters in [-+_.A-Za-z0-9] are allowed.', $match[0]));
        }
        if ($redis instanceof \ConfigTransformer20220612\Predis\ClientInterface && $redis->getOptions()->exceptions) {
     ...
     */
    public static function createConnection(string $dsn, array $options = []) : \Redis|\RedisArray|\RedisCluster|RedisClusterProxy|RedisProxy|\ConfigTransformer20220612\Predis\ClientInterface
    {
        ...
        if (!\extension_loaded('redis') && !\class_exists(\ConfigTransformer20220612\Predis\Client::class)) {
        ...
        if (isset($params['redis_sentinel']) && !\class_exists(\ConfigTransformer20220612\Predis\Client::class) && !\class_exists(\RedisSentinel::class)) {
        ...
        if (null === $params['class'] && \extension_loaded('redis')) {
            ...
            $class = $params['class'] ?? \ConfigTransformer20220612\Predis\Client::class;
            if (isset($params['redis_sentinel']) && !\is_a($class, \ConfigTransformer20220612\Predis\Client::class, \true) && !\class_exists(\RedisSentinel::class)) {
        ...
        } elseif (\is_a($class, \ConfigTransformer20220612\Predis\ClientInterface::class, \true)) {
    ...
    protected function doFetch(array $ids) : iterable
    {
        ...
        $result = [];
        if ($this->redis instanceof \ConfigTransformer20220612\Predis\ClientInterface && $this->redis->getConnection() instanceof ClusterInterface) {
    ...
    protected function doClear(string $namespace) : bool
    {
        ...
        foreach ($hosts as $host) {
            ...
            if (!$host instanceof \ConfigTransformer20220612\Predis\ClientInterface) {
            ...
            if (!\version_compare($info['redis_version'], '2.8', '>=')) {
                ...
                $args = $this->redis instanceof \ConfigTransformer20220612\Predis\ClientInterface ? [0, $pattern] : [[$pattern], 0];
            ...
            $cursor = null;
            do {
                $keys = $host instanceof \ConfigTransformer20220612\Predis\ClientInterface ? $host->scan($cursor, 'MATCH', $pattern, 'COUNT', 1000) : $host->scan($cursor, $pattern, 1000);
    ...
    private function pipeline(\Closure $generator, object $redis = null) : \Generator
    {
        ...
        if ($redis instanceof RedisClusterProxy || $redis instanceof \RedisCluster || $redis instanceof \ConfigTransformer20220612\Predis\ClientInterface && $redis->getConnection() instanceof RedisCluster) {
            ...
            foreach ($generator() as $command => $args) {
                ...
                $ids[] = 'eval' === $command ? $redis instanceof \ConfigTransformer20220612\Predis\ClientInterface ? $args[2] : $args[1][0] : $args[0];
            }
        } elseif ($redis instanceof \ConfigTransformer20220612\Predis\ClientInterface) {
        ...
        if (!$redis instanceof \ConfigTransformer20220612\Predis\ClientInterface && 'eval' === $command && $redis->getLastError()) {
        ...
        if ($this->redis instanceof \ConfigTransformer20220612\Predis\ClientInterface) {
            ...
            if ($connection instanceof ClusterInterface && $connection instanceof \Traversable) {
                ...
                foreach ($connection as $c) {
                    $hosts[] = new \ConfigTransformer20220612\Predis\Client($c);
                }
            }
