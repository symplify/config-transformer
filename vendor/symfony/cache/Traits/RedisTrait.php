...
use ConfigTransformerPrefix202302\Predis\Command\Redis\UNLINK;
use ConfigTransformerPrefix202302\Predis\Connection\Aggregate\ClusterInterface;
use ConfigTransformerPrefix202302\Predis\Connection\Aggregate\RedisCluster;
...
use ConfigTransformerPrefix202302\Symfony\Component\Cache\Exception\CacheException;
...
trait RedisTrait
{
    ...
    private function init($redis, string $namespace, int $defaultLifetime, ?MarshallerInterface $marshaller)
    {
        ...
        if ($redis instanceof \ConfigTransformerPrefix202302\Predis\ClientInterface && $redis->getOptions()->exceptions) {
    ...
    public static function createConnection(string $dsn, array $options = [])
    {
        ...
        if (!\extension_loaded('redis') && !\class_exists(\ConfigTransformerPrefix202302\Predis\Client::class)) {
        ...
        if (isset($params['redis_sentinel']) && !\class_exists(\ConfigTransformerPrefix202302\Predis\Client::class) && !\class_exists(\RedisSentinel::class)) {
        ...
        if (null === $params['class'] && \extension_loaded('redis')) {
            ...
            $class = $params['class'] ?? \ConfigTransformerPrefix202302\Predis\Client::class;
            if (isset($params['redis_sentinel']) && !\is_a($class, \ConfigTransformerPrefix202302\Predis\Client::class, \true) && !\class_exists(\RedisSentinel::class)) {
        ...
        } elseif (\is_a($class, \ConfigTransformerPrefix202302\Predis\ClientInterface::class, \true)) {
    ...
    protected function doFetch(array $ids) : iterable
    {
        ...
        $result = [];
        if ($this->redis instanceof \ConfigTransformerPrefix202302\Predis\ClientInterface && $this->redis->getConnection() instanceof ClusterInterface) {
    ...
    protected function doClear(string $namespace) : bool
    {
        ...
        foreach ($hosts as $host) {
            ...
            if (!$host instanceof \ConfigTransformerPrefix202302\Predis\ClientInterface) {
            ...
            if (!\version_compare($info['redis_version'], '2.8', '>=')) {
                ...
                $args = $this->redis instanceof \ConfigTransformerPrefix202302\Predis\ClientInterface ? [0, $pattern] : [[$pattern], 0];
            ...
            $cursor = null;
            do {
                $keys = $host instanceof \ConfigTransformerPrefix202302\Predis\ClientInterface ? $host->scan($cursor, 'MATCH', $pattern, 'COUNT', 1000) : $host->scan($cursor, $pattern, 1000);
    ...
    private function pipeline(\Closure $generator, object $redis = null) : \Generator
    {
        ...
        if ($redis instanceof \RedisCluster || $redis instanceof \ConfigTransformerPrefix202302\Predis\ClientInterface && $redis->getConnection() instanceof RedisCluster) {
            ...
            foreach ($generator() as $command => $args) {
                ...
                $ids[] = 'eval' === $command ? $redis instanceof \ConfigTransformerPrefix202302\Predis\ClientInterface ? $args[2] : $args[1][0] : $args[0];
            }
        } elseif ($redis instanceof \ConfigTransformerPrefix202302\Predis\ClientInterface) {
        ...
        if (!$redis instanceof \ConfigTransformerPrefix202302\Predis\ClientInterface && 'eval' === $command && $redis->getLastError()) {
        ...
        if ($this->redis instanceof \ConfigTransformerPrefix202302\Predis\ClientInterface) {
            ...
            if ($connection instanceof ClusterInterface && $connection instanceof \Traversable) {
                ...
                foreach ($connection as $c) {
                    $hosts[] = new \ConfigTransformerPrefix202302\Predis\Client($c);
                }
            }
