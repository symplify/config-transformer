...
use ConfigTransformerPrefix202605\Predis\Command\Redis\UNLINK;
use ConfigTransformerPrefix202605\Predis\Connection\Aggregate\ClusterInterface;
use ConfigTransformerPrefix202605\Predis\Connection\Aggregate\RedisCluster;
use ConfigTransformerPrefix202605\Predis\Connection\Aggregate\ReplicationInterface;
use ConfigTransformerPrefix202605\Predis\Connection\Cluster\ClusterInterface as Predis2ClusterInterface;
use ConfigTransformerPrefix202605\Predis\Connection\Cluster\RedisCluster as Predis2RedisCluster;
...
use ConfigTransformerPrefix202605\Relay\Sentinel;
use ConfigTransformerPrefix202605\Symfony\Component\Cache\Exception\CacheException;
...
trait RedisTrait
{
     ...
     * @var mixed[]
     */
    private static $defaultConnectionOptions = ['class' => null, 'auth' => null, 'persistent' => 0, 'persistent_id' => null, 'timeout' => 30, 'read_timeout' => 0, 'retry_interval' => 0, 'tcp_keepalive' => 0, 'lazy' => null, 'redis_cluster' => \false, 'redis_sentinel' => null, 'dbindex' => 0, 'failover' => 'none', 'ssl' => null];
    /**
    ...
    private function init($redis, string $namespace, int $defaultLifetime, ?MarshallerInterface $marshaller) : void
    {
        ...
        if ($redis instanceof \ConfigTransformerPrefix202605\Predis\ClientInterface && $redis->getOptions()->exceptions) {
    ...
    public static function createConnection(string $dsn, array $options = [])
    {
        ...
        if (!\extension_loaded('redis') && !\extension_loaded('relay') && !\class_exists(\ConfigTransformerPrefix202605\Predis\Client::class)) {
            ...
            if (isset($query['host'])) {
                ...
                foreach ($hosts as $host => $parameters) {
                    ...
                    if (\false === ($i = \strrpos($host, ':'))) {
                        ...
                        $hosts[$host] = ['scheme' => $tcpScheme, 'host' => (string) \substr($host, 0, $i), 'port' => $port] + $parameters;
                    } else {
                        $hosts[$host] = ['scheme' => 'unix', 'path' => (string) \substr($host, 0, $i)] + $parameters;
                    }
                }
        ...
        if (isset($params['host']) || isset($params['path'])) {
            if (!isset($params['dbindex']) && isset($params['path'])) {
                if (\preg_match('#/(\\d+)?$#', $params['path'], $m)) {
                    ...
                    $params['path'] = (string) \substr($params['path'], 0, -\strlen($m[0]));
        ...
        } elseif (!\class_exists(\ConfigTransformerPrefix202605\Predis\Client::class) && !\class_exists(\RedisSentinel::class) && !\class_exists(Sentinel::class)) {
        ...
        switch (\true) {
            case $params['redis_cluster']:
                $class = \extension_loaded('redis') ? \RedisCluster::class : \ConfigTransformerPrefix202605\Predis\Client::class;
                break;
            case isset($params['redis_sentinel']):
                switch (\true) {
                    ...
                    case \extension_loaded('relay'):
                        ...
                        break;
                    default:
                        $class = \ConfigTransformerPrefix202605\Predis\Client::class;
                        break;
                }
                break;
        ...
        if (isset($params['redis_sentinel']) && !\is_a($class, \ConfigTransformerPrefix202605\Predis\Client::class, \true) && !\class_exists(\RedisSentinel::class) && !\class_exists(Sentinel::class)) {
        ...
        if ($isRedisExt || $isRelayExt) {
            ...
            $initializer = static function () use($class, $isRedisExt, $connect, $params, $auth, $hosts, $tls) {
                ...
                do {
                    ...
                    } catch (\RedisException|\ConfigTransformerPrefix202605\Relay\Exception $redisException) {
                    }
                ...
                } catch (\RedisException|\ConfigTransformerPrefix202605\Relay\Exception $e) {
            ...
            $initializer = static function () use($isRedisExt, $class, $params, $hosts) {
                ...
                $redis->setOption(\RedisCluster::OPT_SLAVE_FAILOVER, (function () use($params) {
                    switch ($params['failover']) {
                        case 'error':
                            return \RedisCluster::FAILOVER_ERROR;
                        case 'distribute':
                            return \RedisCluster::FAILOVER_DISTRIBUTE;
                        case 'slaves':
                            return \RedisCluster::FAILOVER_DISTRIBUTE_SLAVES;
                        case 'none':
                            return \RedisCluster::FAILOVER_NONE;
                    }
                })());
                return $redis;
            };
            $redis = $params['lazy'] ? RedisClusterProxy::createLazyProxy($initializer) : $initializer();
        } elseif (\is_a($class, \ConfigTransformerPrefix202605\Predis\ClientInterface::class, \true)) {
    ...
    protected function doFetch(array $ids) : iterable
    {
        ...
        $result = [];
        if ($this->redis instanceof \ConfigTransformerPrefix202605\Predis\ClientInterface && ($this->redis->getConnection() instanceof ClusterInterface || $this->redis->getConnection() instanceof Predis2ClusterInterface) || $this->redis instanceof RelayCluster) {
    ...
    protected function doClear(string $namespace) : bool
    {
        ...
        foreach ($hosts as $host) {
            ...
            } elseif (!$host instanceof \ConfigTransformerPrefix202605\Predis\ClientInterface) {
            ...
            if (!\version_compare($info['redis_version'], '2.8', '>=')) {
                ...
                $args = $this->redis instanceof \ConfigTransformerPrefix202605\Predis\ClientInterface ? [0, $pattern] : [[$pattern], 0];
            ...
            $cursor = null;
            do {
                $keys = $host instanceof \ConfigTransformerPrefix202605\Predis\ClientInterface ? $host->scan($cursor ?? 0, 'MATCH', $pattern, 'COUNT', 1000) : $host->scan($cursor, $pattern, 1000);
                ...
                if ($keys) {
                    if ($prefixLen) {
                        foreach ($keys as $i => $key) {
                            $keys[$i] = (string) \substr($key, $prefixLen);
                        }
                    }
                    $this->doDelete($keys);
                }
    ...
    protected function doDelete(array $ids) : bool
    {
        ...
        if ($this->redis instanceof \ConfigTransformerPrefix202605\Predis\ClientInterface && ($this->redis->getConnection() instanceof ClusterInterface || $this->redis->getConnection() instanceof Predis2ClusterInterface)) {
            static $del;
    ...
    private function pipeline(\Closure $generator, ?object $redis = null) : \Generator
    {
        ...
        if ($redis instanceof \RedisCluster || $redis instanceof \ConfigTransformerPrefix202605\Predis\ClientInterface && ($redis->getConnection() instanceof RedisCluster || $redis->getConnection() instanceof Predis2RedisCluster)) {
            ...
            foreach ($generator() as $command => $args) {
                ...
                $ids[] = 'eval' === $command ? $redis instanceof \ConfigTransformerPrefix202605\Predis\ClientInterface ? $args[2] : $args[1][0] : $args[0];
            }
        } elseif ($redis instanceof \ConfigTransformerPrefix202605\Predis\ClientInterface) {
        ...
        if (!$redis instanceof \ConfigTransformerPrefix202605\Predis\ClientInterface && 'eval' === $command && $redis->getLastError()) {
            $e = $redis instanceof Relay ? new \ConfigTransformerPrefix202605\Relay\Exception($redis->getLastError()) : new \RedisException($redis->getLastError());
            $results = \array_map(static function ($v) use($e) {
                return \false === $v ? $e : $v;
            }, (array) $results);
        }
        if (\is_bool($results)) {
        ...
        if ($this->redis instanceof \ConfigTransformerPrefix202605\Predis\ClientInterface) {
            ...
            if (($connection instanceof ClusterInterface || $connection instanceof Predis2ClusterInterface) && $connection instanceof \Traversable) {
                ...
                foreach ($connection as $c) {
                    $hosts[] = new \ConfigTransformerPrefix202605\Predis\Client($c);
                }
            }
