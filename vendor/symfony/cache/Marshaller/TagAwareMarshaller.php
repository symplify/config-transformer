...
class TagAwareMarshaller implements MarshallerInterface
{
    ...
    public function unmarshall(string $value)
    {
        ...
        $meta = (string) \substr($value, 1, 12);
        $meta[8] = "\x00";
        $tagLen = \unpack('Nlen', $meta, 8)['len'];
        $meta = \substr($meta, 0, 8);
        return ['value' => $this->marshaller->unmarshall((string) \substr($value, 13 + $tagLen)), 'tags' => $tagLen ? $this->marshaller->unmarshall((string) \substr($value, 13, $tagLen)) : [], 'meta' => "\x00\x00\x00\x00\x00\x00\x00\x00" === $meta ? null : $meta];
    }
}
