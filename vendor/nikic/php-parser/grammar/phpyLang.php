...
{
    $code = \ConfigTransformer2021090310\resolveNodes($code);
    $code = \ConfigTransformer2021090310\resolveMacros($code);
    $code = \ConfigTransformer2021090310\resolveStackAccess($code);
    return $code;
}
function resolveNodes($code)
{
    return \preg_replace_callback('~\\b(?<name>[A-Z][a-zA-Z_\\\\]++)\\s*' . \PARAMS . '~', function ($matches) {
        // recurse
        $matches['params'] = \ConfigTransformer2021090310\resolveNodes($matches['params']);
        $params = \ConfigTransformer2021090310\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['params']);
        $paramCode = '';
...
function resolveMacros($code)
{
    return \preg_replace_callback('~\\b(?<!::|->)(?!array\\()(?<name>[a-z][A-Za-z]++)' . \ARGS . '~', function ($matches) {
        // recurse
        $matches['args'] = \ConfigTransformer2021090310\resolveMacros($matches['args']);
        $name = $matches['name'];
        $args = \ConfigTransformer2021090310\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['args']);
        if ('attributes' === $name) {
            \ConfigTransformer2021090310\assertArgs(0, $args, $name);
            return '$this->startAttributeStack[#1] + $this->endAttributes';
        }
        if ('stackAttributes' === $name) {
            \ConfigTransformer2021090310\assertArgs(1, $args, $name);
        ...
        if ('push' === $name) {
            \ConfigTransformer2021090310\assertArgs(2, $args, $name);
        ...
        if ('parseEncapsed' === $name) {
            \ConfigTransformer2021090310\assertArgs(3, $args, $name);
...
{
    $pieces = \preg_split(\ConfigTransformer2021090310\regex('(?:(?&string)|(?&comment)|(?&code))(*SKIP)(*FAIL)|' . $regex), $string);
