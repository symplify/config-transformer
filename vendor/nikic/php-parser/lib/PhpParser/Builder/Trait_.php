...
class Trait_ extends \ConfigTransformer202106207\PhpParser\Builder\Declaration
{
    protected $name;
    protected $uses = [];
    ...
    public function addStmt($stmt)
    {
        ...
        if ($stmt instanceof \ConfigTransformer202106207\PhpParser\Node\Stmt\Property) {
            ...
            $this->methods[] = $stmt;
        } elseif ($stmt instanceof \ConfigTransformer202106207\PhpParser\Node\Stmt\TraitUse) {
            $this->uses[] = $stmt;
        } else {
    ...
    {
        return new \ConfigTransformer202106207\PhpParser\Node\Stmt\Trait_($this->name, ['stmts' => \array_merge($this->uses, $this->properties, $this->methods)], $this->attributes);
    }
}
