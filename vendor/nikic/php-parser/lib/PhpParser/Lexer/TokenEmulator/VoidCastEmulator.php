...
class VoidCastEmulator extends TokenEmulator
{
    ...
    public function emulate(string $code, array $tokens) : array
    {
        for ($i = 0, $c = \count($tokens); $i < $c; ++$i) {
            ...
            $text .= ')';
            $numTokens++;
            \array_splice($tokens, $i, $numTokens, [new Token(\ConfigTransformerPrefix202606\T_VOID_CAST, $text, $token->line, $token->pos)]);
            $c -= $numTokens - 1;
        }
        return $tokens;
    }
    public function reverseEmulate(string $code, array $tokens) : array
    {
        for ($i = 0, $c = \count($tokens); $i < $c; ++$i) {
            $token = $tokens[$i];
            if ($token->id !== \ConfigTransformerPrefix202606\T_VOID_CAST) {
                continue;
            }
