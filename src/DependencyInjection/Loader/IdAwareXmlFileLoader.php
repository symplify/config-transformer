...
use ConfigTransformer20210608\Symfony\Component\DependencyInjection\Loader\XmlFileLoader;
...
use ConfigTransformer20210608\Symplify\ConfigTransformer\Naming\UniqueNaming;
use ConfigTransformer20210608\Symplify\ConfigTransformer\ValueObject\SymfonyVersionFeature;
use ConfigTransformer20210608\Symplify\PackageBuilder\Reflection\PrivatesCaller;
/**
 * Mimics https://github.com/symfony/symfony/commit/b8c68da0107a4f433dd414a355ea5589da0da0e8 for Symfony 3.3-
 */
final class IdAwareXmlFileLoader extends \ConfigTransformer20210608\Symfony\Component\DependencyInjection\Loader\XmlFileLoader
{
    /**
     * @var string
     */
    ...
    public function __construct(\ConfigTransformer20210608\Symfony\Component\DependencyInjection\ContainerBuilder $containerBuilder, \ConfigTransformer20210608\Symfony\Component\Config\FileLocatorInterface $fileLocator, \ConfigTransformer20210608\Symplify\ConfigTransformer\Configuration\Configuration $configuration, \ConfigTransformer20210608\Symplify\ConfigTransformer\Naming\UniqueNaming $uniqueNaming, \ConfigTransformer20210608\Symplify\ConfigTransformer\Collector\XmlImportCollector $xmlImportCollector)
    {
        parent::__construct($containerBuilder, $fileLocator);
        $this->privatesCaller = new \ConfigTransformer20210608\Symplify\PackageBuilder\Reflection\PrivatesCaller();
    ...
    private function processAnonymousServices(\DOMDocument $domDocument, string $file) : void
    {
        $this->count = 0;
        $definitions = [];
        $suffix = '~' . \ConfigTransformer20210608\Symfony\Component\DependencyInjection\ContainerBuilder::hash($file);
        ...
        foreach ($inversedDefinitions as $id => [$domElement, $file]) {
            $definition = $this->privatesCaller->callPrivateMethod($this, 'parseDefinition', [$domElement, $file, new \ConfigTransformer20210608\Symfony\Component\DependencyInjection\Definition()]);
            if ($definition !== null) {
    ...
    {
        $serviceName = \ConfigTransformer20210608\Nette\Utils\Strings::replace($class, '#\\\\#', '.');
    ...
    private function createAnonymousServiceId(bool $hasNamedServices, \DOMElement $domElement, string $file) : string
    {
        ...
        if (!$this->configuration->isAtLeastSymfonyVersion(\ConfigTransformer20210608\Symplify\ConfigTransformer\ValueObject\SymfonyVersionFeature::SERVICE_WITHOUT_NAME)) {
            return $this->createServiceNameFromClass($className);
        }
