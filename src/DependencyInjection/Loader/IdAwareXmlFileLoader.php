 ...
 */
final class IdAwareXmlFileLoader extends \ConfigTransformer2021072110\Symfony\Component\DependencyInjection\Loader\XmlFileLoader
{
    /**
     * @var string
     */
    ...
    private function processAnonymousServices(\DOMDocument $domDocument, string $file) : void
    {
        ...
        foreach ($inversedDefinitions as $id => [$domElement, $file]) {
            $definition = $this->privatesCaller->callPrivateMethod($this, 'parseDefinition', [$domElement, $file, new \ConfigTransformer2021072110\Symfony\Component\DependencyInjection\Definition()]);
            if ($definition !== null) {
    ...
    {
        $serviceName = \ConfigTransformer2021072110\Nette\Utils\Strings::replace($class, '#\\\\#', '.');
    ...
    private function createAnonymousServiceId(bool $hasNamedServices, \DOMElement $domElement, string $file) : string
    {
        ...
        if (!$this->configuration->isAtLeastSymfonyVersion(\ConfigTransformer2021072110\Symplify\ConfigTransformer\ValueObject\SymfonyVersionFeature::SERVICE_WITHOUT_NAME)) {
            return $this->createServiceNameFromClass($className);
        }
