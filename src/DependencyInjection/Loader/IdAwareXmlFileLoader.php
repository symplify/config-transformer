...
use ConfigTransformer202112022\Symfony\Component\DependencyInjection\Loader\XmlFileLoader;
use ConfigTransformer202112022\Symplify\ConfigTransformer\Collector\XmlImportCollector;
use ConfigTransformer202112022\Symplify\ConfigTransformer\Naming\UniqueNaming;
use ConfigTransformer202112022\Symplify\ConfigTransformer\ValueObject\DependencyInjection\Extension\AliasAndNamespaceConfigurableExtension;
 ...
 */
final class IdAwareXmlFileLoader extends \ConfigTransformer202112022\Symfony\Component\DependencyInjection\Loader\XmlFileLoader
{
    /**
     * @var string
     */
    ...
    public function __construct(\ConfigTransformer202112022\Symfony\Component\DependencyInjection\ContainerBuilder $containerBuilder, \ConfigTransformer202112022\Symfony\Component\Config\FileLocatorInterface $fileLocator, \ConfigTransformer202112022\Symplify\ConfigTransformer\Naming\UniqueNaming $uniqueNaming, \ConfigTransformer202112022\Symplify\ConfigTransformer\Collector\XmlImportCollector $xmlImportCollector)
    {
        $this->uniqueNaming = $uniqueNaming;
        $this->xmlImportCollector = $xmlImportCollector;
        parent::__construct($containerBuilder, $fileLocator);
        $this->privatesCaller = new \ConfigTransformer202112022\Symplify\PackageBuilder\Reflection\PrivatesCaller();
    }
    /**
     * @return mixed
     */
    ...
    public function load($resource, $type = null)
    {
        ...
        if (!\is_string($path)) {
            throw new \ConfigTransformer202112022\Symfony\Component\Config\Util\Exception\XmlParsingException();
        }
        ...
        // faking extensions
        $aliasAndNamespaceConfigurableExtension = new \ConfigTransformer202112022\Symplify\ConfigTransformer\ValueObject\DependencyInjection\Extension\AliasAndNamespaceConfigurableExtension('doctrine', 'http://symfony.com/schema/dic/doctrine');
    ...
    private function processAnonymousServices(\DOMDocument $domDocument, string $file) : void
    {
        ...
        foreach ($inversedDefinitions as $id => [$domElement, $file]) {
            $definition = $this->privatesCaller->callPrivateMethod($this, 'parseDefinition', [$domElement, $file, new \ConfigTransformer202112022\Symfony\Component\DependencyInjection\Definition()]);
            if ($definition !== null) {
    ...
    {
        $serviceName = \ConfigTransformer202112022\Nette\Utils\Strings::replace($class, '#\\\\#', '.');
    ...
    {
        try {
            return \ConfigTransformer202112022\Symfony\Component\Config\Util\XmlUtils::loadFile($path);
        } catch (\InvalidArgumentException $invalidArgumentException) {
            $errorMessage = \sprintf('Unable to parse file "%s": %s', $path, $invalidArgumentException->getMessage());
            throw new \ConfigTransformer202112022\Symfony\Component\Config\Util\Exception\XmlParsingException($errorMessage, $invalidArgumentException->getCode(), $invalidArgumentException);
        }
    }
}
