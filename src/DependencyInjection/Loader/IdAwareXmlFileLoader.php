...
use ConfigTransformer202109071\Symplify\ConfigTransformer\Naming\UniqueNaming;
use ConfigTransformer202109071\Symplify\ConfigTransformer\ValueObject\DependencyInjection\Extension\AliasAndNamespaceConfigurableExtension;
 ...
 */
final class IdAwareXmlFileLoader extends \ConfigTransformer202109071\Symfony\Component\DependencyInjection\Loader\XmlFileLoader
{
    /**
     * @var string
     */
    ...
    public function __construct(\ConfigTransformer202109071\Symfony\Component\DependencyInjection\ContainerBuilder $containerBuilder, \ConfigTransformer202109071\Symfony\Component\Config\FileLocatorInterface $fileLocator, \ConfigTransformer202109071\Symplify\ConfigTransformer\Naming\UniqueNaming $uniqueNaming, \ConfigTransformer202109071\Symplify\ConfigTransformer\Collector\XmlImportCollector $xmlImportCollector)
    {
        $this->uniqueNaming = $uniqueNaming;
        $this->xmlImportCollector = $xmlImportCollector;
        parent::__construct($containerBuilder, $fileLocator);
        $this->privatesCaller = new \ConfigTransformer202109071\Symplify\PackageBuilder\Reflection\PrivatesCaller();
    }
    ...
    public function load($resource, $type = null) : void
    {
        ...
        if (!\is_string($path)) {
            throw new \ConfigTransformer202109071\Symfony\Component\Config\Util\Exception\XmlParsingException();
        }
        ...
        // faking extensions
        $aliasAndNamespaceConfigurableExtension = new \ConfigTransformer202109071\Symplify\ConfigTransformer\ValueObject\DependencyInjection\Extension\AliasAndNamespaceConfigurableExtension('doctrine', 'http://symfony.com/schema/dic/doctrine');
    ...
    private function processAnonymousServices(\DOMDocument $xml, string $file) : void
    {
        ...
        foreach ($inversedDefinitions as $id => [$domElement, $file]) {
            $definition = $this->privatesCaller->callPrivateMethod($this, 'parseDefinition', [$domElement, $file, new \ConfigTransformer202109071\Symfony\Component\DependencyInjection\Definition()]);
            if ($definition !== null) {
    ...
    {
        $serviceName = \ConfigTransformer202109071\Nette\Utils\Strings::replace($class, '#\\\\#', '.');
    ...
    {
        try {
            return \ConfigTransformer202109071\Symfony\Component\Config\Util\XmlUtils::loadFile($path);
        } catch (\InvalidArgumentException $invalidArgumentException) {
            $errorMessage = \sprintf('Unable to parse file "%s": %s', $path, $invalidArgumentException->getMessage());
            throw new \ConfigTransformer202109071\Symfony\Component\Config\Util\Exception\XmlParsingException($errorMessage, $invalidArgumentException->getCode(), $invalidArgumentException);
        }
    }
}
